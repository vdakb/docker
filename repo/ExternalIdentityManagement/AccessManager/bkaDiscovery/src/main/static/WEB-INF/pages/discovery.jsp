...
-->
<c:set var="returnPRM" value="${pageContext.request.getParameter('returnIDParam')}"/>
<!--the URL where the user should be redirected at the SP -->
<c:set var="returnURL" value="${pageContext.request.getParameter('return')}"/>
<html dir="ltr" lang="${pageContext.request.locale}">
  <head>
    ...
    <!-- Choose automatically partner by Ip Range -->
    <script type="text/javascript">window.onload = function() {if ('${provider.partner}' != '#') { document.getElementById('${provider.id}').click(); }} </script>
  </head>
          ...
          <div class="container">
            <div class="row">
              <c:forEach var="provider" items="${applicationScope.storage.provider}">
                <c:if test="${not fn:contains(provider.partner, 'OIDC')}">
                  <div class="card">
                    <div class="card-body">
