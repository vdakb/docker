...
public class ClaimsSerializer<T extends ClaimsHolder> extends StdSerializer<T> {
  ...
  @Override
  public void serialize(T holder, JsonGenerator gen, SerializerProvider provider)
    throws IOException {
   ...
   */
  protected void writeClaim(Map.Entry<String, Object> entry, JsonGenerator gen)
    throws IOException {
    gen.writeFieldName(entry.getKey());
    handleSerialization(entry.getValue(), gen);
  }

  private static void handleSerialization(Object value, JsonGenerator gen)
    throws IOException {
  ...
  private static void serializeMap(Map<?, ?> map, JsonGenerator gen)
    throws IOException {
    gen.writeStartObject();
  ...
  private static void serializeList(List<?> list, JsonGenerator gen)
    throws IOException {
    gen.writeStartArray();
