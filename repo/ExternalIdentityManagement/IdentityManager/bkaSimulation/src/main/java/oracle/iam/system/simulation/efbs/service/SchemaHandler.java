...
import oracle.iam.system.simulation.NotAllowedException;
...
class SchemaHandler extends AbstractHandler<SchemaResource> {
 ...
 /**
  ...
  public Response<SchemaResource> get(final HttpExchange t)
    ...
    String[]                         segments  = t.getRequestURI().getPath().split("/");
    ...
    if (Server.CONTEXT_SCHEMA.endsWith(last)) {
      return createResponse(200, resourceSchema.size(), 1, resourceSchema);
    }
    else {
      for (SchemaResource cursor : resourceSchema) {
        if (cursor.id().equals(last))
          return createResponse(200, cursor);
      }
      throw new NotFoundException(ServiceBundle.string(ServiceError.OBJECT_NOT_EXISTS, last, Server.CONTEXT_SCHEMA));
    }
  }
