...
public class HttpConnection {
    ...
    public HttpConnection(NextcloudConfiguration config, String path, Map<String, String> params) {
        this.config = config;
        ...
        try {
            ...
            logger.finest("Connection to NextCloud prepared.");

        } catch (IOException e) {
            logger.log(Level.SEVERE, "Failed to create a connection to NextCloud.");
            throw ConnectorException.wrap(e);
        }
    }

    private String buildFinalUrl(String path, Map<String, String> params) {
        StringBuilder url = new StringBuilder().append(this.config.getBaseUrl() + path + "?format=json");
    ...
    public void sendRequest(String method, String body) {
        logger.finer("Sending request to NextCloud. METHOD=" + method);
        try {
            ...
            try {
                ...
                String err = "NextCloud - Unable to establish connection. Server might be down";
            ...
            if (responseCode != 200) {
                String err = "NextCloud - Unexpected response code from the server. This is likely a development error or something has changed with the topology. Response was: " + responseCode;
                logger.severe(err);
            ...
            String err = "NextCloud - Something unexpected went wrong with the request. Something might be unexpected about the actual data we're sending.";
