...
public class ServiceClient extends AbstractLoggable {
   ...
   **
   ** @param  feature            the request feature(s) to register.
   **                            <br>
   **                            Allowed object is array of {@link Feature}.
   **
   ** @return                    the http authentication feature  providing a
   **                            valid access token.
   **                            <br>
   **                            Possible object is array of {@link Feature}.
   **
   ** @throws SystemException    if the authentication flow is secured by SSL
   **                            and the required {@code SSLContext} could not
   **                            be configured.
   */
  protected Feature passwordFlow(final Feature feature)
    ...
    ).register(feature, HttpAuthenticationFeature.basic(this.endpoint.clientIdentifier(), this.endpoint.clientSecret())).build(new SystemConsole(this.logger.category()));
    flow.accessToken();
    return flow.feature();
  }
  ...
  protected Feature credentialFlow()
    ...
    final AuthorizationFlow flow   = AuthorizationClient.credentialFlow(
      ...
      Principal.of(this.endpoint.clientIdentifier(),  this.endpoint.clientSecret())
    , UriBuilder.fromPath(this.endpoint.authorizationServer()).build()
    ).build(new SystemConsole(this.logger.category()));
    flow.accessToken();
    return flow.feature();
  }
}