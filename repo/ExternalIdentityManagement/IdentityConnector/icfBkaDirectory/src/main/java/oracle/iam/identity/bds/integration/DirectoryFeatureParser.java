...
final class DirectoryFeatureParser extends FeatureParser {
  ...
  enum Grammar {
    ...
    , ENTRY_STATUS       (FEATURE,                                                 Directory.OIM.Feature.ENTRY_STATUS)
    , STATUS_ATTRIBUTE   (new Grammar[]{ENTRY_STATUS},                             Directory.OIM.Feature.STATUS_ATTRIBUTE)
    ...
    switch (state) {
     ...
     case GROUPLINK            : switch (this.state.peek()) {
      ...
      case SCOPELINK            : switch (this.state.peek()) {
                                  ...
                                  break;
      case STATUS_ATTRIBUTE     : String value = (String) this.feature.getOrDefault(Directory.OIM.Feature.ENTRY_STATUS, "");
                                  if (!StringUtility.isEmpty(value)) {
                                    value+= "|";
                                  }
                                  value += attributes.getValue(ATTRIBUTE_NAME) + "::" + attributes.getValue(ATTRIBUTE_ENABLED) + "::" + attributes.getValue(ATTRIBUTE_DISABLED);
                                  this.feature.put(Directory.OIM.Feature.ENTRY_STATUS, value);
                                  break;
      default                    : break;
    }
  }
