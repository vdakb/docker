...
package oracle.iam.identity.icf.connector.bds;
...
import oracle.iam.identity.icf.connector.*;
...
public class Main extends DirectoryService {
  ...
  static final String CATEGORY = "JCS.CONNECTOR.BDS";
...
//    SyncStrategy strategy = chooseSyncStrategy(objectClass);
  ...
  public Uid create(final ObjectClass type, final Set<Attribute> attribute, final OperationOptions operation)
    ...
    try {
      ...
      uid = DirectoryCreate.build(this.endpoint, type).execute(attribute.stream().filter(attr -> attr.getValue().size() > 0 &&
                                                                                                 !attr.getValue().get(0).toString().isEmpty()).collect(Collectors.toSet()));
    }
    catch (SystemException e) {
      propagate(e);
    }
  ...
  public Uid update(final ObjectClass type, final Uid uid, final Set<Attribute> replace, final OperationOptions option)
    ...
    try {
      ...
      final String accountPrefix = ((DirectoryConfiguration) getConfiguration()).getAccountPrefix();

      for (Attribute cursor : replace) {
        if (StringUtility.equal(cursor.getName(), accountPrefix)) {
          DirectoryRename.build(this.endpoint, type).execute(cursor, uid);
        } else {
