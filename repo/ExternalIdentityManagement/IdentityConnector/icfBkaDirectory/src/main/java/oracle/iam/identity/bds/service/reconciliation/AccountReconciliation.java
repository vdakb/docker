...
public class AccountReconciliation extends EntryReconciliation {
  ...
  //////////////////////////////////////////////////////////////////////////////
  // Method:   configure (Reconciliation)

  @Override
  protected void configure() throws TaskException {
    parameter(SEARCH_CONTAINER, ObjectClass.ACCOUNT_NAME);
    super.configure();
  }
  ...
  protected OperationOptionsBuilder operationBuilder() {
    // At the begining, get the same behavior of the super method.
    final OperationOptionsBuilder factory = super.operationBuilder();
    
    final Map<String, Object> option = factory.getOptions();
    if (!StringUtility.isBlank(groupContext()))
      option.put("group_base_dn",     groupContext());
    if (!StringUtility.isBlank(roleContext()))
      option.put("role_base_dn", roleContext());
    if (!StringUtility.isBlank(scopeContext()))
      option.put("scope_base_dn",scopeContext());
