...
public abstract class DirectorySearch extends DirectoryOperation {
  ...
  final Map<ObjectClass, String>    objectClassDN;
  ...
  static class Paginated extends DirectorySearch {
     ...
     **                          Allowed object is {@link Set} where each
     **                          element is of type {@link String}.
     ** @param  objectClassDN    the DNs where are located embedded Object
     **                          Classe. If the researched entry contains
     **                          embedded object (i.e group), this attribut
     **                          must contain the DNs of each Object Class.
     **                          <br>
     **                          Allowed object is {@link Map}. Can be null
     **
     ** @throws SystemException  if the appropriate Request Controls cannot be
     **                          initialized.
     */
    private Paginated(final DirectoryEndpoint endpoint, final ObjectClass type, final DirectoryName base, final int scope, final int limit, final String filter, final Set<String> emit, final Set<String> order, final Map<ObjectClass, String> objectClassDN)
      throws SystemException {

      // ensure inheritance
      super(endpoint, type, base, scope, limit, filter, emit, objectClassDN);
  ...
  private static class VirtualList extends DirectorySearch {
   ...
   /**
     ...
     **                          initialized.
     */
    private VirtualList(final DirectoryEndpoint endpoint, final ObjectClass type, final DirectoryName base, final int scope, final int limit, final String filter, final Set<String> emit, final Set<String> order, final Map<ObjectClass, String> objectClassDN)
      throws SystemException {
  ...
  static class Simple extends DirectorySearch {
     ...
     **                          Allowed object is {@link Set} where each
     **                          element is of type {@link String}.
     ** @param  objectClassDN    the DNs where are located Object Classed. If
     **                          the researched entry contains embedded object
     **                          (i.e group), this attribut must contain the
     **                          DN of every Object Class.
     **                          <br>
     **                          Allowed object is {@link Map}. Can be null
     **
     ** @throws SystemException  if the appropriate Request Controls cannot be
     **                          initialized.
     */
    protected Simple(final DirectoryEndpoint endpoint, final ObjectClass type, final DirectoryName base, final int scope, final int limit, final String filter, final Set<String> emit, final Set<String> order, final Map<ObjectClass, String> objectClassDN)
      throws SystemException {
   ...
   */
  private DirectorySearch(final DirectoryEndpoint endpoint, final ObjectClass type, final DirectoryName base, final int scope, final int limit, final String filter, final Set<String> emit, final Map<ObjectClass, String> objectClassDN)
    throws SystemException {
    ...
    this.objectClassDN   = objectClassDN != null ? objectClassDN : new HashMap<ObjectClass, String>();
  ...
  public static DirectorySearch build(final DirectoryEndpoint endpoint, final ObjectClass type, final OperationOptions option, final DirectoryFilter filter)
    ...
    final Map<ObjectClass, String> classDNLoc = new HashMap<ObjectClass, String>();
    if (option.getOptions().containsKey("group_base_dn"))
      classDNLoc.put(ObjectClass.GROUP, (String) option.getOptions().get("group_base_dn"));
    if (option.getOptions().containsKey("role_base_dn"))
      classDNLoc.put(DirectorySchema.ROLE, (String) option.getOptions().get("role_base_dn"));
    if (option.getOptions().containsKey("scope_base_dn"))
      classDNLoc.put(DirectorySchema.SCOPE, (String) option.getOptions().get("scope_base_dn"));
    ...
    if (endpoint.virtualListControl() && endpoint.supported(VirtualListViewControl.OID)) {
      return new VirtualList(endpoint, type, base, scope, control.limit, searchFilter(endpoint, type, entryFilter, nativeFilter, control.filter, incrementalFilter), control.emit, control.order, classDNLoc);
    }
    else if (endpoint.simplePageControl() && endpoint.supported(PagedResultsControl.OID)) {
      return new Paginated(endpoint, type, base, scope, control.limit, searchFilter(endpoint, type, entryFilter, nativeFilter, control.filter, incrementalFilter), control.emit, control.order, classDNLoc);
    }
    // if nothing fits fall back to a simple result set search
    else {
      return new Simple(endpoint, type, base, scope, control.limit, searchFilter(endpoint, type, entryFilter, nativeFilter, control.filter, incrementalFilter), control.emit, control.order, classDNLoc);
    }
  }
  ...
  static DirectorySearch simple(final DirectoryEndpoint endpoint, final int limit, final String filter, final Set<String> emit)
    ...
    return new Simple(endpoint, DirectorySchema.ANY, DirectoryName.build(endpoint.changeLogContainer()), SearchControls.ONELEVEL_SCOPE, limit, filter, emit, null, null);
  }
  ...
  static DirectorySearch strategy(final DirectoryEndpoint context, final ObjectClass type, final DirectoryName base, final int scope, final int limit, final String filter, final Set<String> emit, final Set<String> order)
    ...
    if (context.virtualListControl() && context.supported(VirtualListViewControl.OID)) {
      return new VirtualList(context, type, base, scope, limit, filter, emit, order, null);
    }
    else if (context.simplePageControl() && context.supported(PagedResultsControl.OID)) {
      return new Paginated(context, type, base, scope, limit, filter, emit, order, null);
    }
    // if nothing fits fall back to a simple result set search
    else {
      return new Simple(context, type, base, scope, limit, filter, emit, order, null);
    }
  }
  ...
  public void collectMembership(final ConnectorObjectBuilder builder, final String userDN)
    throws SystemException {
  ...
  public void collectRoleMembership(final ConnectorObjectBuilder builder, final String userDN)
    throws SystemException {
      final OperationOptionsBuilder opBuilder = new OperationOptionsBuilder().setContainer(new QualifiedUid(ObjectClass.GROUP, new Uid("ou=Rollen,ou=TN1,ou=AW1")));
      final List<EmbeddedObject> memberOf  = new ArrayList<EmbeddedObject>();
      final DirectoryFilter      filter    = DirectoryFilter.expression(DirectoryEntry.composeFilter(this.endpoint.roleMember(), userDN));
      final DirectorySearch      search    = DirectorySearch.build(this.endpoint, ObjectClass.GROUP, opBuilder.build(), filter);
      for (SearchResult group : search.list()) {
        memberOf.add(new EmbeddedObjectBuilder().setObjectClass(DirectorySchema.ROLE).addAttribute(Uid.NAME, group.getNameInNamespace()).addAttribute(Name.NAME, group.getNameInNamespace()).build());
      }
      builder.addAttribute(DirectorySchema.ROLE.getObjectClassValue(), memberOf);
  }
  
  public void collectScopeMembership(final ConnectorObjectBuilder builder, final String userDN)
    throws SystemException {
      final OperationOptionsBuilder opBuilder = new OperationOptionsBuilder().setContainer(new QualifiedUid(ObjectClass.GROUP, new Uid("ou=Intermediate,ou=TN1,ou=AW1")));
      final List<EmbeddedObject> memberOf  = new ArrayList<EmbeddedObject>();
      final DirectoryFilter      filter    = DirectoryFilter.expression(DirectoryEntry.composeFilter(this.endpoint.scopeMember(), userDN));
      final DirectorySearch      search    = DirectorySearch.build(this.endpoint, ObjectClass.GROUP, opBuilder.build(), filter);
      for (SearchResult group : search.list()) {
        memberOf.add(new EmbeddedObjectBuilder().setObjectClass(DirectorySchema.SCOPE).addAttribute(Uid.NAME, group.getNameInNamespace()).addAttribute(Name.NAME, group.getNameInNamespace()).build());
      }
      builder.addAttribute(DirectorySchema.SCOPE.getObjectClassValue(), memberOf);
  }

  public void collectGroupMembership(final ConnectorObjectBuilder builder, final String userDN)
    throws SystemException {
      final OperationOptionsBuilder opBuilder = new OperationOptionsBuilder().setContainer(new QualifiedUid(ObjectClass.GROUP, new Uid("ou=Funktionsrechte,ou=AW1")));
      final List<EmbeddedObject> memberOf  = new ArrayList<EmbeddedObject>();
      final DirectoryFilter      filter    = DirectoryFilter.expression(DirectoryEntry.composeFilter(this.endpoint.groupMember(), userDN));
      final DirectorySearch      search    = DirectorySearch.build(this.endpoint, ObjectClass.GROUP, opBuilder.build(), filter);
      for (SearchResult group : search.list()) {
        memberOf.add(new EmbeddedObjectBuilder().setObjectClass(ObjectClass.GROUP).addAttribute(Uid.NAME, group.getNameInNamespace()).addAttribute(Name.NAME, group.getNameInNamespace()).build());
      }
      builder.addAttribute(ObjectClass.GROUP.getObjectClassValue(), memberOf);
  }
  
  public void collectMembersip(final ConnectorObjectBuilder builder, final String userDN, final ObjectClass type, final String contextDN)
    throws SystemException {
    
    final List<EmbeddedObject> memberOf  = new ArrayList<EmbeddedObject>();
    final OperationOptionsBuilder opBuilder = new OperationOptionsBuilder().setContainer(new QualifiedUid(type, new Uid(contextDN))); 
    final DirectoryFilter      filter    = DirectoryFilter.expression(DirectoryEntry.composeFilter(this.endpoint.groupMember(), userDN));
    final DirectorySearch      search    = DirectorySearch.build(this.endpoint, type, opBuilder.build(), filter);
    for (SearchResult group : search.list()) {
      memberOf.add(new EmbeddedObjectBuilder().setObjectClass(type).addAttribute(Uid.NAME, group.getNameInNamespace()).addAttribute(Name.NAME, group.getNameInNamespace()).build());
    }
    builder.addAttribute(type.getObjectClassValue(), memberOf);
  ...
  protected ConnectorObject connectorObject(final SearchResult result)
    ...
    try {
      ...
      if (this.type.equals(ObjectClass.ACCOUNT)) {
        for (Map.Entry<ObjectClass, String> entry : objectClassDN.entrySet()) {
          collectMembersip(builder, result.getNameInNamespace(), entry.getKey(), entry.getValue());
        }
      }
    }
    catch (NamingException e) {
  ...
  private Uid createIdentifier(final SearchResult result)
    ...
    if (this.endpoint.distinguishedName().equalsIgnoreCase(this.entryIdentifier)) {
