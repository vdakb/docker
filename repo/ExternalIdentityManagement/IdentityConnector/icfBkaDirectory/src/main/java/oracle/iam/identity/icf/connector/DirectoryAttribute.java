...
public class DirectoryAttribute {
    ...
    for (Attribute cursor : attribute) {
      if (!cursor.is(Name.NAME)) {
        if (groupEntitlement(cursor.getName()) || posixEntitlement(cursor.getName()) || roleEntitlement(cursor.getName())) {
          ...
          if (cursor.is(OperationalAttributes.ENABLE_NAME)) {
            encodeEnable(schema, cursor, entry);
          }
        }
      }
    }
  ...
  private static void encodeEnable(final DirectorySchema schema, final Attribute attribute, final BasicAttributes entry) {
    Set<DirectoryEndpoint.Bind.Status> statusAttributes = schema.statusAttributes();
    if (CollectionUtility.empty(statusAttributes)) {
      return;
    }

    for (DirectoryEndpoint.Bind.Status cursor : statusAttributes) {
      final BasicAttribute encoded = new BasicAttribute(cursor.name());
      for (Object value : attribute.getValue()) {
        if (value instanceof Boolean) {
          String status = (Boolean) value ? cursor.enabled() : cursor.disabled();
          encoded.add(status);
        }
      }
      entry.put(encoded);
