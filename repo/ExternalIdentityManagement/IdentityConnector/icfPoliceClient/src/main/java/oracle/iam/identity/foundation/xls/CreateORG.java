...
public class WAS extends SystemConsole {
  ...
  private void execute()
    ...
    int i = 0;
    ensureRoot();
    organization(this.identities, "", "NSIS", "Top");
    while (r.hasNext()) {
      ...
      if (i == WAS.skip) {
        ...
        organization(this.identities, code, name, "NSIS").commit();
      }
    }
  ...
  protected final XMLOutputNode organization(final XMLOutputNode node, final String code, final String name, final String parent)
    ...
    parameter(organization, "Organization Status",        "Active");
    parameter(organization, "Organization Customer Type", "System");
    parameter(organization, "Parent Organization Name",   parent);
    parameter(organization, "code_nsis",                  code);
    return organization;
  }
  ...
  private void ensureRoot() {
    if (this.identities == null) {
      try {
        ...
        this.identities = XMLProcessor.marshal(this, dataFile, this.format).element("operation:organization");
        this.identities.attribute("contextRef", "oim-context");
        this.identities.attribute("operation",  "create");
      }
  ...
  // Method:   parameter
  /**
   ** This is used to create parameterized name/value pair element.
   */
  private XMLOutputNode parameter(final XMLOutputNode node, final String name, final String value)
    throws Exception {

    final XMLOutputNode parameter = node.element("parameter");
