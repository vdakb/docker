...
import static oracle.iam.identity.gws.integration.igs.Service.OIM.Resource.TYPE_ACCEPT;
import static oracle.iam.identity.gws.integration.igs.Service.OIM.Resource.TYPE_CONTENT;
import static oracle.iam.identity.gws.integration.igs.Service.OIM.Resource.RESOURCE_OWNER;
import static oracle.iam.identity.gws.integration.igs.Service.OIM.Resource.CLIENT_SECRET;
import static oracle.iam.identity.gws.integration.igs.Service.OIM.Resource.CLIENT_IDENTIFIER;
import static oracle.iam.identity.gws.integration.igs.Service.OIM.Resource.RESOURCE_CREDENTIAL;
import static oracle.iam.identity.gws.integration.igs.Service.OIM.Resource.AUTHENTICATION_SCHEME;
import static oracle.iam.identity.gws.integration.igs.Service.OIM.Resource.AUTHORIZATION_SERVER;
...
public class ServiceResource extends TargetResource {
  ...
  public ServiceResource(final Loggable loggable, final Map<String, String> parameter)
    ...
    // perform a consistency check on authentication properties that are used
    // for different purposes due to dependencies on the defined authentication
    // mechanism and therefore cannot be declared required
  ...
  /**
   ** Perform a consistency check on authentication properties defined on an
   ** <code>IT Resource</code> that are used for different purposes due to
   ** dependencies on the defined authentication mechanism and therefore cannot
  ...
  private void validateAuthentication()
    ...
    if (!"none".equals(attribute(AUTHENTICATION_SCHEME))) {
      // all authentication mechanism requires a principal name and password
      if (StringUtility.isEmpty(attribute(PRINCIPAL_NAME)))
        authnViolated(PRINCIPAL_NAME);
      if (StringUtility.isEmpty(attribute(PRINCIPAL_PASSWORD)))
        authnViolated(PRINCIPAL_PASSWORD);

      // the OAuth 2 Password Credential Flow requires is addition valid client
      // credentials
      if ("oauth-password".equals(attribute(AUTHENTICATION_SCHEME))) {
        if (StringUtility.isEmpty(attribute(CLIENT_IDENTIFIER)))
          authnViolated(CLIENT_IDENTIFIER);
        if (StringUtility.isEmpty(attribute(CLIENT_SECRET)))
          authnViolated(CLIENT_SECRET);
      }
    }
  }

  //////////////////////////////////////////////////////////////////////////////
  // Method:   authnViolated
  /**
   ...
   **                             <code>IT Resource</code>.
   */
  private void authnViolated(final String name)
    throws TaskException {
