...
// ~~~~~ ~~~~~~
/**
 ** The test case lookup operation on users at the target system leveraging the
...
public class Lookup extends Base {
  ...
  @Test
  public void execute() {
    final Set<String> wellknown = CollectionUtility.set("-1", "10");
    try {
      for (String id : wellknown) {
        final AccountResource response = CONTEXT.lookupAccount(id);
