...
import oracle.iam.identity.gws.integration.igs.Service;
...
import oracle.iam.identity.icf.connector.scim.igs.Main;
...
public class TestResource extends TestBaseIntegration {
  ...
  public void testUnmarshalResource() {
    try {
      final ServiceResource resource = ServiceResource.build(CONSOLE, config(INTRANET));
      ...
      assertTrue(resource.containsKey(Service.OIM.Resource.TYPE_ACCEPT));
      assertTrue(resource.containsKey(Service.OIM.Resource.TYPE_CONTENT));

      assertEquals(resource.stringValue(ServiceResource.SERVER_NAME),      "dieters-macbook-pro.fritz.box");
      assertEquals(resource.stringValue(ServiceResource.SERVER_PORT),      "8081");
      assertEquals(resource.stringValue(ServiceResource.ROOT_CONTEXT),      "/igs/scim/v2");
      assertEquals(resource.stringValue(Service.OIM.Resource.TYPE_CONTENT), "application/scim+json");
      assertEquals(resource.stringValue(Service.OIM.Resource.TYPE_ACCEPT),  "application/scim+json");
    }
  ...
  public void testUnmarshalFeature() {
    try {
      ...
      assertNotNull(token);
      assertEquals(token.getBundleName(),    "igs.identity.connector.bundle");
