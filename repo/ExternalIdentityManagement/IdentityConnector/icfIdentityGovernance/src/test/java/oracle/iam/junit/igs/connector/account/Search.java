...
// ~~~~~ ~~~~~~
/**
 ** The test case search operation on users at the target system leveraging the
...
public class Search extends Base {
  ...
  public void execute() {
    try {
      final ListResponse<AccountResource> result = CONTEXT.searchAccount(1, 2000, null).invoke(AccountResource.class);
      assertNotNull(result);
      assertNotEquals(result.total(), 0);

      final List<AccountResource> resource = result.resource();
      assertNotNull(resource);
      assertNotEquals(resource.size(), 0);
      for (AccountResource cursor : resource) {
        assertNotNull(cursor.id());
        assertNotNull(cursor.userName());
        assertNotNull(cursor.metadata());
      }
    }
