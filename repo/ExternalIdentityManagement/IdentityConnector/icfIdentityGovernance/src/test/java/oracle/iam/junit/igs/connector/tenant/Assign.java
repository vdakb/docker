...
public class Assign extends Base {
  ...
  // Method:   execute
  /**
   ** Test grant complete role request leveraging Web Access Context.
   */
  @Test
  public void execute() {
    try {
      final List<Operation> patch = CollectionUtility.list(
        ...
        , CollectionUtility.list(
            TenantResource.Role.of(TenantResource.Role.USER, 5L).scope("uid.generate")
          , TenantResource.Role.of(TenantResource.Role.USER, 5L).scope("uid.register")
          )
        )
      );
