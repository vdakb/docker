...
// ~~~~~ ~~~~~ ~~~~~~~~~~~~
/**
 ** This represents the mapping between the SCIM schemas representing tenants
 ...
 ** @since   1.0.0.0
 */
public final class TenantSchema implements CheckedFunction<Path, String, ProcessingException> {
   ...
   ** Singleton Pattern
   */
  public static final TenantSchema            instance    = new TenantSchema();
  ...
  public enum Attribute {
    ...
    , ACTIVE ("active",            Tenant.Attribute.ACTIVE.id)
    , NAME   ("displayName",       Tenant.Attribute.NAME.id)
    , ROLE   ("roles",             "roles")
    ;
  ...
  // Method:   Ctor
  /**
   ** Constructs a <code>TenantSchema</code> allows use as a JavaBean.
   ** <br>
   ** Zero argument constructor required by the framework.
   ** <br>
   ** Default Constructor
  ...
  public final String apply(final Path path)
    ...
    // any path in a search operation can belongs to one or more elements
  ...
  private static synchronized Map<String, Attribute> permitted() {
    ...
    mapping.put(Attribute.ROLE.outbound,   Attribute.ROLE);
    return mapping;
  }
