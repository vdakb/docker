...
// ~~~~~ ~~~~~~~~~~
/**
 ** The <code>StateView</code> managed bean for maintain {@link State}s.
 **
 ** @author dieter.steding@oracle.com
 ** @version 1.0.0.0
 ** @since   1.0.0.0
...
@ViewScoped
@Named("state")
public class StateView extends AbstractView<State> {
  ...
  // compatible with
  @SuppressWarnings("compatibility:8039628700577510982")
  private static final long serialVersionUID = 5223292879496996354L;
  ...
  // Method:   Ctor
  /**
   ** Constructs a <code>StateView</code> that allows use as a JavaBean.
   **
   ...
   */
  @Inject
  public StateView(final Preference preference) {
    // ensure inheritance
    super(preference);
  }
   ...
   **
   ** @param  entry              the {@link State} to return the identifier for.
   **                            <br>
   **                            Allowed object is {@link State}.
   **
   ** @return                    the identifier of the {@link State}
   **                            <code>entry</code>
   **                            <br>
   **                            Possible object is {@link String}.
   */
  @Override
  public String getRowKey(final State entry) {
    return entry.getId();
  }
  ...
  ** <br>
   ...
   **
   ** @param  identifier         the identifier of an {@link State} bean to
   **                            lookup.
   **                            <br>
   **                            Allowed object is {@link String}.
   **
   ** @return                    the {@link State} bean belonging to the
   ...
   */
  @Override
  public State getRowData(final String identifier) {
    // prevent bogus input
    if (identifier == null)
      throw new FacesException("getRowData");

    for (State cursor : getWrappedData()) {
   ...
   **                            element is of type {@link State}.
   */
  @Override
  public List<State> load(final int start, final int count, final Map<String, SortMeta> sort, final Map<String, FilterMeta> filter) {
   ...
   ** update the {@link State} view with the current state of the persistence
   ** layer.
   */
  @Override
  public void refresh() {
    reset();
    Message.show(Message.information(Message.resourceValue(APP, "action.refresh.label"), Message.resourceValue(UID, "sta.refresh.feedback")));
  }
   ...
   ** delete the selected {@link State}.
   */
  @Override
  public void delete() {
    try {
      this.facade.delete(this.selected);
      Message.show(Message.information(Message.resourceValue(APP, "action.delete.label"), Message.resourceValue(UID, "sta.deleted.feedback"), this.selected.getId()));
      reset();
    }
    catch (Exception e) {
   ...
   ** persist a created or modified {@link State}.
   **
  ...
  public void save(final ActionEvent event) {
    try {
      if (this.modify) {
        ...
        Message.show(Message.information(Message.resourceValue(APP, "action.modify.label"), Message.resourceValue(UID, "sta.modified.feedback"), this.selected.getId()));
      }
      else {
        this.facade.create(this.selected);
        this.modify = true;
        Message.show(Message.information(Message.resourceValue(APP, "action.create.label"), Message.resourceValue(UID, "sta.created.feedback"), this.selected.getId()));
      }
      this.changed = false;
    }
    catch (Exception e) {
