...
////////////////////////////////////////////////////////////////////////////////
// class ParticipantTypeView
// ~~~~~ ~~~~~~~~~~~~~~~~~~~
/**
 ** The <code>ParticipantTypeView</code> managed bean for maintain
 ** {@link ParticipantType}s.
 **
 ** @author dieter.steding@oracle.com
 ** @version 1.0.0.0
 ** @since   1.0.0.0
 */
@ViewScoped
@Named("participantType")
public class ParticipantTypeView extends AbstractView<ParticipantType> {
  ...
  // compatible with
  @SuppressWarnings("compatibility:3455901416000296955")
  private static final long serialVersionUID = 826138365100636975L;
  ...
  private ParticipantTypeFacade facade;
  ...
  // Method:   Ctor
  /**
   ** Constructs a <code>ParticipantTypeView</code> that allows use as a
   ** JavaBean.
   **
   ...
   */
  @Inject
  public ParticipantTypeView(final Preference preference) {
    // ensure inheritance
    super(preference);
  }
   ...
   **
   ** @param  entry              the {@link ParticipantType} to return the
   **                            identifier for.
   **                            <br>
   **                            Allowed object is {@link ParticipantType}.
   **
   ** @return                    the identifier of the {@link ParticipantType}
   **                            <code>entry</code>
   **                            <br>
   **                            Possible object is {@link String}.
   */
  @Override
  public String getRowKey(final ParticipantType entry) {
    return entry.getId();
  }
   ...
   **
   ** @param  identifier         the identifier of an {@link ParticipantType}
   **                            bean to lookup.
   **                            <br>
   **                            Allowed object is {@link String}.
   **
   ** @return                    the {@link ParticipantType} bean belonging to
   **                            the specified <code>identifier</code>.
   **                            <br>
   ...
   */
  @Override
  public ParticipantType getRowData(final String identifier) {
    // prevent bogus input
    if (identifier == null)
      throw new FacesException("getRowData");

    for (ParticipantType cursor : getWrappedData()) {
   ...
   **                            element is of type {@link ParticipantType}.
   */
  @Override
  public List<ParticipantType> load(final int start, final int count, final Map<String, SortMeta> sort, final Map<String, FilterMeta> filter) {
   ...
   ** update the {@link ParticipantType} view with the current state of the
   ** persistence layer.
   */
  @Override
  public void refresh() {
    reset();
    Message.show(Message.information(Message.resourceValue(APP, "action.refresh.label"), Message.resourceValue(UID, "ptt.refresh.feedback")));
  }
   ...
   ** a new {@link ParticipantType}.
   */
  @Override
  public void create() {
    this.modify   = false;
    this.selected = new ParticipantType();
   ...
   ** delete the selected {@link ParticipantType}.
   */
  @Override
  public void delete() {
    try {
      this.facade.delete(this.selected);
      Message.show(Message.information(Message.resourceValue(APP, "action.delete.label"), Message.resourceValue(UID, "ptt.deleted.feedback"), this.selected.getId()));
      reset();
    }
    catch (Exception e) {
   ...
   ** persist a created or modified {@link ParticipantType}.
   **
  ...
  public void save(final ActionEvent event) {
    try {
      if (this.modify) {
        ...
        Message.show(Message.information(Message.resourceValue(APP, "action.modify.label"), Message.resourceValue(UID, "ptt.modified.feedback"), this.selected.getId()));
      }
      else {
        this.facade.create(this.selected);
        this.modify = true;
        Message.show(Message.information(Message.resourceValue(APP, "action.create.label"), Message.resourceValue(UID, "ptt.created.feedback"), this.selected.getId()));
      }
      this.changed = false;
    }
    catch (Exception e) {
