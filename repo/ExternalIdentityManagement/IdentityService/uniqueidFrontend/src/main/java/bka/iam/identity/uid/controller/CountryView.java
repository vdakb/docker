...
// ~~~~~ ~~~~~~~~~~~
/**
 ** The <code>CountryView</code> managed bean for maintain {@link Country}.
 **
 ** @author dieter.steding@oracle.com
 ** @version 1.0.0.0
 ** @since   1.0.0.0
...
@ViewScoped
@Named("country")
public class CountryView extends AbstractView<Country> {
  ...
  // compatible with
  @SuppressWarnings("compatibility:-258644869135350970")
  private static final long serialVersionUID = 6710804144649934888L;
  ...
  // Method:   Ctor
  /**
   ** Constructs a <code>CountryView</code> that allows use as a JavaBean.
   **
   ...
   */
  @Inject
  public CountryView(final Preference preference) {
    // ensure inheritance
    super(preference);
  }
   ...
   **
   ** @param  entry              the {@link Country} to return the identifier
   **                            for.
   **                            <br>
   **                            Allowed object is {@link Country}.
   **
   ** @return                    the identifier of the {@link Country}
   **                            <code>entry</code>
   **                            <br>
   **                            Possible object is {@link String}.
   */
  @Override
  public String getRowKey(final Country entry) {
    return entry.getId();
  }
   ...
   **
   ** @param  identifier         the identifier of an {@link Country} bean to
   **                            lookup.
   **                            <br>
   **                            Allowed object is {@link String}.
   **
   ** @return                    the {@link Country} bean belonging to the
   ...
   */
  @Override
  public Country getRowData(final String identifier) {
    // prevent bogus input
    if (identifier == null)
      throw new FacesException("getRowData");

    for (Country cursor : getWrappedData()) {
   ...
   **                            element is of type {@link Country}.
   */
  @Override
  public List<Country> load(final int start, final int count, final Map<String, SortMeta> sort, final Map<String, FilterMeta> filter) {
   ...
   ** update the {@link Country} view with the current state of the persistence
   ** layer.
   */
  @Override
  public void refresh() {
    reset();
    Message.show(Message.information(Message.resourceValue(APP, "action.refresh.label"), Message.resourceValue(UID, "cnt.refresh.feedback")));
  }
   ...
   ** delete the selected {@link Country}.
   */
  @Override
  public void delete() {
    try {
      this.facade.delete(this.selected);
      Message.show(Message.information(Message.resourceValue(APP, "action.delete.label"), Message.resourceValue(UID, "cnt.deleted.feedback"), this.selected.getId()));
      reset();
    }
    catch (Exception e) {
   ...
   ** persist a created or modified {@link Country}.
   **
