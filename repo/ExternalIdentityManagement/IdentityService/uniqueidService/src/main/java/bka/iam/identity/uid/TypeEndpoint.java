...
// ~~~~~ ~~~~~~~~~~~~
/**
 ** The REST service {@link Endpoint} for {@link Type} transactions.
 **
 ** @author  dieter.steding@oracle.com
 ** @version 1.0.0.0
 ** @since   1.0.0.0
 */
@Path(TypeEndpoint.PATH)
@Consumes(MediaType.APPLICATION_JSON)
@Produces(MediaType.APPLICATION_JSON)
public class TypeEndpoint extends Endpoint {
  ...
  public static final String PATH = "type";
  ...
  // Method:   Ctor
  /**
   ** Constructs a <code>TypeEndpoint</code> allows use as a JavaBean.
   ** <br>
  ...
  // Method:   list
  /**
   ** Service REST request to retrieve all {@link Type}s paginated by the
   ...
   **
   ** @return                    the collection of {@link Type}s.
   **                            <br>
  ...
  public Response list(final @Context UriInfo context) {
    try {
      ...
      final SearchResult<Type> result   = this.facade.list(request);
      ...
      final JsonObject         response = Schema.type(trimmer, request.start() + 1, result);
  ...
  // Method:   lookup
  /**
   ** REST service request to retrieve a certain {@link Type} by its
   ** <code>id</code>.
   ** <p>
   ** If the principal isn't equiped with the role <code>administrator</code>
   ** this method responed with a {@link ForbiddenException}.
   **
   ** @param  id                 the identifier of the {@link Type} to
   **                            lookup.
   **                            <br>
   ...
   **                              for an account type identifier have been
   **                              violated.
  ...
  public Response lookup(final @PathParam("id") String id, final @Context UriInfo context)
    ...
    // prevent bogus input
    SurrogateSchema.validateAccountType(id);

    final Type subject = this.facade.lookup(id);
    if (subject == null)
      throw notFound(Type.NAME, id);

    final Schema trimmer = Schema.build(resourceTrimmer(context));
    return success(trimmer.convert(subject));
  }

  //////////////////////////////////////////////////////////////////////////////
  // Method:   create
  /**
   ** REST service request to create a certain {@link Type} using POST.
   ** <p>
   ** If the principal isn't equiped with the role <code>administrator</code>
   ** this method responed with a {@link ForbiddenException}.
   **
   ...
   **                                   rules for an account type identifier
   **                                   have been violated.
  ...
  public Response create(final JsonObject resource, final @Context UriInfo context)
    ...
    final Type entity = Schema.type(resource);
    // prevent bogus input
    SurrogateSchema.validate(entity);

    if (this.facade.lookup(entity.getId()) != null)
      throw conflict(Type.NAME, entity.getId());
  ...
  // Method:   modify
  /**
   ** REST service request to modify a certain {@link Type} using PUT.
   ** <p>
   ** If the principal isn't equiped with the role <code>administrator</code>
   ** this method responed with a {@link ForbiddenException}.
   **
   ** @param  id                 the identifier of the {@link Type} to modify.
   **                            <br>
   **                            Allowed object is {@link String}.
   ** @param  resource           an object that represents the resource to
   **                            modify.
  ...
  public Response modify(final @PathParam("id") String id, final JsonObject resource, final @Context UriInfo context)
    ...
    final Type entity = this.facade.lookup(id);
    if (entity == null)
      throw notFound(Type.NAME, id);
  ...
  // Method:   delete
  /**
   ** REST service request to delete a certain {@link Type} using DELETE.
   ** <p>
   ** If the principal isn't equiped with the role <code>administrator</code>
   ** this method responed with a {@link ForbiddenException}.
   **
   ** @param  id                 the identifier of the {@link Type} to delete.
   **                            <br>
   **                            Allowed object is {@link String}.
   **
   ** @return                    an object abstracting the response obtained
