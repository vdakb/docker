...
// ~~~~~ ~~~~~~~~~~~~~
/**
 ** The REST service {@link Endpoint} for {@link State} transactions.
 **
 ** @author  dieter.steding@oracle.com
 ** @version 1.0.0.0
 ** @since   1.0.0.0
 */
@Path(StateEndpoint.PATH)
...
public class StateEndpoint extends Endpoint {
  ...
  public static final String PATH = "state";
  ...
  // Method:   Ctor
  /**
   ** Constructs a <code>StateEndpoint</code> allows use as a JavaBean.
   ** <br>
  ...
  // Method:   list
  /**
   ** Service REST request to retrieve all {@link State}s paginated by the
   ...
   **
   ** @return                    the collection of {@link State}s.
   **                            <br>
  ...
  public Response list(final @Context UriInfo context) {
    try {
      ...
      final SearchResult<State> result   = this.facade.list(request);
      ...
      final JsonObject          response = Schema.state(trimmer, request.start() + 1, result);
  ...
  // Method:   lookup
  /**
   ** REST service request to retrieve a certain {@link State} by its
   ** <code>id</code>.
   ** <p>
   ** If the principal isn't equiped with the role <code>administrator</code>
   ** this method responed with a {@link ForbiddenException}.
   **
   ** @param  id                 the identifier of the {@link State} to
   **                            lookup.
   **                            <br>
   ...
   **                              for a state identifier have been violated.
  ...
  public Response lookup(final @PathParam("id") String id, final @Context UriInfo context)
    ...
    // prevent bogus input
    SurrogateSchema.validateState(id);

    final State subject = this.facade.lookup(id);
    if (subject == null)
      throw notFound(State.NAME, id);
  ...
  // Method:   create
  /**
   ** REST service request to create a certain {@link State} using POST.
   ** <p>
   ** If the principal isn't equiped with the role <code>administrator</code>
   ** this method responed with a {@link ForbiddenException}.
   **
   ...
   **                                   rules for a state identifier have been
   **                                   violated.
  ...
  public Response create(final JsonObject resource, final @Context UriInfo context)
    ...
    final State  entity = Schema.state(resource);
    // prevent bogus input
    SurrogateSchema.validate(entity);

    if (this.facade.lookup(entity.getId()) != null)
      throw conflict(State.NAME, entity.getId());
  ...
  // Method:   modify
  /**
   ** REST service request to modify a certain {@link State} using PUT.
   ** <p>
   ** If the principal isn't equiped with the role <code>administrator</code>
   ** this method responed with a {@link ForbiddenException}.
   **
  ...
  public Response modify(final @PathParam("id") String id, final JsonObject resource, final @Context UriInfo context)
    ...
    final State entity = this.facade.lookup(id);
    if (entity == null)
      throw notFound(State.NAME, id);
  ...
  // Method:   delete
  /**
   ** REST service request to delete a certain {@link State} using DELETE.
   ** <p>
   ** If the principal isn't equiped with the role <code>administrator</code>
   ** this method responed with a {@link ForbiddenException}.
   **
