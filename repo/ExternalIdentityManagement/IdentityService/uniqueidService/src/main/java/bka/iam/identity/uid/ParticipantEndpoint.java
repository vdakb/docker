...
// ~~~~~ ~~~~~~~~~~~~~~~~~~~
/**
 ** The REST service {@link Endpoint} for {@link Participant} transactions.
 **
 ** @author  dieter.steding@oracle.com
 ** @version 1.0.0.0
 ** @since   1.0.0.0
 */
@Path(ParticipantEndpoint.PATH)
...
public class ParticipantEndpoint extends Endpoint {
  ...
  public static final String PATH = "participant";
  ...
  // Method:   Ctor
  /**
   ** Constructs a <code>ParticipantEndpoint</code> allows use as a JavaBean.
   ** <br>
  ...
  // Method:   list
  /**
   ** Service REST request to retrieve all {@link Participant}s paginated by the
   ...
   **
   ** @return                    the collection of {@link Participant}s.
   **                            <br>
  ...
  public Response list(final @Context UriInfo context) {
    try {
      ...
      final SearchResult<Participant> result   = this.facade.list(request);
      ...
      final JsonObject                response = Schema.participant(trimmer, request.start() + 1, result);
  ...
  // Method:   lookup
  /**
   ** REST service request to retrieve a certain {@link Participant} by its
   ** <code>id</code>.
   ** <p>
   ** If the principal isn't equiped with the role <code>administrator</code>
   ** this method responed with a {@link ForbiddenException}.
   ...
   **                            Allowed object is {@link UriInfo}.
   **
   ** @return                    the {@link Participant} mapped at <code>id</code>
   ...
   **                              for a participant identifier have been
   **                              violated.
  ...
  public Response lookup(final @PathParam("id") String id, final @Context UriInfo context)
    ...
    // prevent bogus input
    SurrogateSchema.validateParticipant(id);

    final Participant subject = this.facade.lookup(id);
    if (subject == null)
     throw notFound(Participant.NAME, id);
  ...
  // Method:   create
  /**
   ** REST service request to create a certain {@link Participant} using POST.
   ** <p>
   ** If the principal isn't equiped with the role <code>administrator</code>
   ** this method responed with a {@link ForbiddenException}.
   **
  ...
  public Response create(final JsonObject resource, final @Context UriInfo context)
    ...
    final Participant entity = Schema.participant(resource);
    // prevent bogus input
    SurrogateSchema.validate(entity);

    if (this.facade.lookup(entity.getId()) != null)
      throw conflict(Participant.NAME, entity.getId());
  ...
  // Method:   modify
  /**
   ** REST service request to modify a certain {@link Participant} using PUT.
   ** <p>
   ** If the principal isn't equiped with the role <code>administrator</code>
   ** this method responed with a {@link ForbiddenException}.
   **
  ...
  public Response modify(final @PathParam("id") String id, final JsonObject resource, final @Context UriInfo context)
    ...
    final Participant entity = this.facade.lookup(id);
    if (entity == null)
      throw notFound(Participant.NAME, id);
    ...
    return Response
      ...
      .entity(
         trimmer.convert(
           this.facade.modify(
             Schema.participant(entity, resource)
           )
         )
       )
      .build()
  ...
  // Method:   delete
  /**
   ** REST service request to delete a certain {@link Participant} using DELETE.
   ** <p>
   ** If the principal isn't equiped with the role <code>administrator</code>
   ** this method responed with a {@link ForbiddenException}.
   **
