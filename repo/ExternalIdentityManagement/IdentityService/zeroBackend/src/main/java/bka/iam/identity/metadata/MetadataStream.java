 ...
 */
public class MetadataStream extends InputStream {
  ...
  public int read(final byte[] buffer, int offset, int length)
    ...
    if (isLowSurrogate(this.chars[0])) {
      this.surrogatePair[1] = this.chars[0];
      if (isSurrogatePair(this.surrogatePair)) {
        surrogateStr = new String(this.surrogatePair, 0, 2);
      }
      this.surrogatePair = new char[2];
    }

    int isLastHighSurrogate = 0;
    if (isHighSurrogate(this.chars[(this.length - 1)])) {
      this.surrogatePair[0] = this.chars[(this.length - 1)];
  ...
  private boolean isSurrogatePair(char[] surPair) {
    return (isHighSurrogate(surPair[0])) && (isLowSurrogate(surPair[1]));
  }

  private boolean isHighSurrogate(char surChar) {
    return (surChar >= 55296) && (surChar <= 56319);
  }

  private boolean isLowSurrogate(char surChar) {
    return (surChar >= 56320) && (surChar <= 57343);
  }
}