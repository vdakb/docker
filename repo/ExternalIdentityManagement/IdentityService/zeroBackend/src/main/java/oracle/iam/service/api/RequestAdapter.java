...
//  private final Platform provider;
  private final OIMClient provider;
   ...
   ** {@link OIMClient} <code>provider</code> to coordinate the transactions.
   **
   ...
   */
  private RequestAdapter(final OIMClient provider) {
    // ensure inheritance
    super();
   ...
   ** specified {@link OIMClient} provider.
   **
   **
   ** @param  provider           the service provider coordinating the
   **                            transactions.
   ...
   **                              <code>null</code>.
   */
  public static RequestAdapter build(final OIMClient provider) {
   ...
   **                            Allowed object is {@link String}.
   **
   ** @return                    the <code>RequestAdapter</code> to allow method
   **                            chaining.
   **                            <br>
   **                            Possible object is <code>RequestAdapter</code>.
   **
  ...
  public final RequestAdapter prepare(final String beneficiary)
    ...
    if (this.beneficiary != null && StringUtility.equalIgnoreCase(this.beneficiary.getBeneficiaryKey(), beneficiary))
      return this;
  ...
  */
  public String splitSubmit(final List<RequestBeneficiaryEntity> entities)
    ...
    final RequestService service = this.provider.getService(RequestService.class);
  ...
  // Method:   findAccount
  /**
   ** Compares the attributes of the given application with the
   ** attribute mapping of a provisioned {@link ApplicationInstance} that has
   ** the same name as the given application.
   ** <br>
   ...
   ** application with the mapping of the {@link ApplicationInstance}
  ...
  public String findAccount(final Pair<String, String> application, final List<Pair<String, String>> attribute, final Set<String> exclude)
    ...
    final ProvisioningService  provisioning = this.provider.getService(ProvisioningService.class);
  ...
  public String lookupApplication(final String name)
    ...
    try {
      result = this.provider.getService(ApplicationInstanceService.class).findApplicationInstanceByName(name);
    }
   ...
   **                            populated from the sttribute.
   **                            <br>
   **                            Possible object is
   **                            {@link RequestBeneficiaryEntityAttribute}.
   */
  public static RequestBeneficiaryEntityAttribute createAttribute(final String id, final String value, final String type) {
...
//      case Date    : return new RequestBeneficiaryEntityAttribute(attribute.getId(), Date.valueOf(attribute.getValue()), type);
   ...
   **
   ** @param  name               the name of the
   **                            <code>Application Instance</code> to list
   **                            names for.
   **                            <br>
   **                            Allowed object is {@link String}.
   **
   ** @return                    the {@link List} of {@link String} objects
   **                            representing account names for the particular
   **                            <code>Application Instance</code>.
   **                            <br>
   **                            Possible object is {@link List} of {@link String}s.
   **                            <br>
   **                            Never <code>null</code>
   **
   ** @throws RequestException  if list operation failed.
   */
  public List<String> listAccounts(final String name)
    throws RequestException {

    List<String> result = new ArrayList<>();
    List<Account> accountList = null;
    try {
      FormInfo formInfo = this.provider.getService(ApplicationInstanceService.class).findApplicationInstanceByName(name).getAccountForm();
      String namingAttr = null;
      for (FormField field : formInfo.getFormFields()) {
        // we're looking for "AccountName" property with the (string) value of "true"
        // there should be one and only one such property, no need to check
        if ("true".equals((String) field.getProperty("AccountName"))) {
          namingAttr = field.getLabel();
          break;
        }
      }
      if (namingAttr == null) {
        // just in case - should never happen
        throw exception(RequestError.REQUEST_NAMING_ATTR_NOT_FOUND, name);
      } else {
        accountList = this.provider.getService(ProvisioningService.class).getProvisionedAccountsForAppInstance(
            name,
        ...
        for (Account account : accountList) {
          Account detailedAccount = this.provider.getService(ProvisioningService.class).getAccountDetails(Long.valueOf(account.getAccountID()));
          Map<String, List<Object>> data = detailedAccount.getNormalizedData();
          // No need to check for null of emptiness - this one cannot be empty/null by definition
          result.add((String) data.get(namingAttr).get(0));
        }
      }
    }
    catch (ApplicationInstanceNotFoundException e) {
      ...
      // this will never happen, this exception is thrown by getAccountDetails() for an account we just processed
      throw exception(RequestError.REQUEST_FAILED, name);
    }
