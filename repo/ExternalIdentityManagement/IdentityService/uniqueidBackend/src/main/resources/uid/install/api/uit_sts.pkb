...
PROMPT ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
PROMPT Creating API Package Body for Table 'uit_states'
PROMPT ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
--------------------------------------------------------------------------------
-- Name:        cg$uit_states
-- Description: uit_states table API package definitions
--------------------------------------------------------------------------------
CREATE OR REPLACE PACKAGE BODY cg$uit_states
IS

--  Constant default values
C10ACTIVE   CONSTANT uit_states.active%TYPE := 1;
...
BEGIN
  ...
  IF (con_name = 'UIT_STS_PK')
  THEN
    cg$errors.push( NVL(UIT_STS_PK, cg$errors.string(cg$errors.API_PK_CON_VIOLATED, cg$errors.APIMSG_PK_VIOLAT, 'UIT_STS_PK', 'uit_states'))
                  , 'E'
                  , 'API'
                  , cg$errors.API_PK_CON_VIOLATED
                  , loc
                  );
  ELSIF (con_name = 'UIT_STS_UK')
  THEN
    cg$errors.push( NVL(UIT_STS_UK, cg$errors.string(cg$errors.API_UQ_CON_VIOLATED, cg$errors.APIMSG_UK_VIOLAT, 'UIT_STS_UK', 'uit_states'))
                  , 'E'
                  , 'API'
                  , cg$errors.API_UQ_CON_VIOLATED
                  , loc
                  );
  ELSIF (con_name = 'UIT_STS_CK1' AND UIT_STS_CK1 IS NOT NULL)
  THEN
    cg$errors.push(UIT_STS_CK1, 'E', 'API', cg$errors.API_CK_CON_VIOLATED, loc);
  ELSE
...
BEGIN
  cg$errors.push( cg$errors.string(cg$errors.API_UNIQUE_KEY_UPDATE, cg$errors.ERR_UK_UPDATE, uk)
                ...
                , 'cg$uit_states.raise_uk_not_updateable'
                );
...
BEGIN
  cg$errors.push( cg$errors.string(cg$errors.API_FOREIGN_KEY_TRANS, cg$errors.ERR_FK_TRANS, fk)
                ...
                , 'cg$uit_states.raise_fk_not_transferable'
                );
  cg$errors.raise_failure;
...
EXCEPTION
  ...
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$uit_states.up_autogen_columns.denorm');
    cg$errors.raise_failure;
...
EXCEPTION
  ...
  THEN
    validate_mandatory(cg$rec, 'cg$uit_states.ins.mandatory_missing');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.check_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_CHECK_CON, 'cg$uit_states.ins.check_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.fk_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_FOREIGN_KEY, 'cg$uit_states.ins.fk_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.uk_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_UNIQUE_KEY, 'cg$uit_states.ins.uk_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$uit_states.ins.others');
    called_from_package := FALSE;
    cg$errors.raise_failure;
END ins;
...
BEGIN
  ...
  THEN
    sel(cg$upd_rec);
    -- Report error if attempt to update non updateable Primary Key UIT_STS_PK
    IF (cg$ind.id AND cg$rec.id != cg$upd_rec.id)
    THEN
      raise_uk_not_updateable('UIT_STS_PK');
    END IF;
    IF NOT (cg$ind.id)
    THEN
...
EXCEPTION
  ...
  THEN
    validate_mandatory(cg$rec, 'cg$uit_states.upd.upd_mandatory_null');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.check_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_CHECK_CON, 'cg$uit_states.upd.check_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.fk_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_FOREIGN_KEY, 'cg$uit_states.upd.fk_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.uk_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_UNIQUE_KEY, 'cg$uit_states.upd.uk_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$uit_states.upd.others');
    called_from_package := FALSE;
    cg$errors.raise_failure;
END upd;
...
EXCEPTION
  ...
  THEN
    err_msg(SQLERRM, cg$errors.ERR_DELETE_RESTRICT, 'cg$uit_states.del.delete_restrict');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN NO_DATA_FOUND
  THEN
    cg$errors.push(cg$errors.string(cg$errors.API_ROW_DEL, cg$errors.ROW_DEL), 'E', 'ORA', SQLCODE, 'cg$uit_states.del.NO_DATA_FOUND');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$uit_states.del.others');
    called_from_package := FALSE;
    cg$errors.raise_failure;
END del;
...
EXCEPTION
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$uit_states.sel.others');
    cg$errors.raise_failure;
END sel;
...
BEGIN
  ...
  EXCEPTION
    ...
    THEN
      cg$errors.push(cg$errors.string(cg$errors.API_ROW_LCK, cg$errors.ROW_LCK), 'E', 'ORA', SQLCODE, 'cg$uit_states.lck.resource_busy');
      cg$errors.raise_failure;
    WHEN NO_DATA_FOUND
    THEN
      cg$errors.push(cg$errors.string(cg$errors.API_ROW_DEL, cg$errors.ROW_DEL), 'E', 'ORA', SQLCODE, 'cg$uit_states.lck.NO_DATA_FOUND');
      cg$errors.raise_failure;
    WHEN OTHERS
    THEN
      cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$uit_states.lck.others');
      cg$errors.raise_failure;
    END;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.string(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P10ID), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$uit_states.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.string(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P15ROWVERSION), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$uit_states.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.string(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P20CREATED_BY), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$uit_states.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.string(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P25CREATED_ON), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$uit_states.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.string(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P30UPDATED_BY), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$uit_states.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.string(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P35UPDATED_ON), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$uit_states.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.string(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P40ACTIVE), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$uit_states.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  IF (cg$old_ind.name)
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.string(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P45NAME), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$uit_states.lck');
      any_modified := TRUE;
    END IF;
  END IF;
...
--------------------------------------------------------------------------------
-- Name:        cascade_update
-- Description: Updates all child tables affected by a change to uit_states
--
-- Parameters:  cg$rec     the record of uit_states current values
--              cg$old_rec the record of uit_states previous values
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- Name:        cascade_delete
-- Description: Delete all child tables affected by a delete to uit_states
--
-- Parameters:  cg$rec     the record of uit_states current values
--------------------------------------------------------------------------------
