...
PROMPT ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
PROMPT Creating API Package Body for Table 'uit_tenants'
PROMPT ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
--------------------------------------------------------------------------------
-- Name:        cg$uit_tenants
-- Description: uit_tenants table API package definitions
--------------------------------------------------------------------------------
CREATE OR REPLACE PACKAGE BODY cg$uit_tenants
IS

-- Constant default values
C10ACTIVE   CONSTANT uit_tenants.active%TYPE := 1;
...
BEGIN
  ...
  IF (con_name = 'UIT_TNT_PK')
  THEN
    cg$errors.push( NVL(UIT_TNT_PK, cg$errors.string(cg$errors.API_PK_CON_VIOLATED, cg$errors.APIMSG_PK_VIOLAT, 'UIT_TNT_PK', 'uit_tenants'))
                  , 'E'
                  , 'API'
                  , cg$errors.API_PK_CON_VIOLATED
                  , loc
                  );
  ELSIF (con_name = 'UIT_TNT_CK1' AND UIT_TNT_CK1 IS NOT NULL)
  THEN
    cg$errors.push(UIT_TNT_CK1, 'E', 'API', cg$errors.API_CK_CON_VIOLATED, loc);
  ELSE
...
BEGIN
  cg$errors.push( cg$errors.string(cg$errors.API_UNIQUE_KEY_UPDATE, cg$errors.ERR_UK_UPDATE, uk)
                ...
                , 'cg$uit_tenants.raise_uk_not_updateable'
                );
...
BEGIN
  cg$errors.push( cg$errors.string(cg$errors.API_FOREIGN_KEY_TRANS, cg$errors.ERR_FK_TRANS, fk)
                ...
                , 'cg$uit_tenants.raise_fk_not_transferable'
                );
  cg$errors.raise_failure;
...
EXCEPTION
  ...
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$uit_tenants.up_autogen_columns.denorm');
    cg$errors.raise_failure;
...
IS
  ...
  cg$clm_rec cg$uit_claims.cg$row_type;
  cg$clm_ind cg$uit_claims.cg$ind_type;
  -- Constant default values
BEGIN
...
EXCEPTION
  ...
  THEN
    validate_mandatory(cg$rec, 'cg$uit_tenants.ins.mandatory_missing');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.check_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_CHECK_CON, 'cg$uit_tenants.ins.check_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.fk_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_FOREIGN_KEY, 'cg$uit_tenants.ins.fk_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.uk_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_UNIQUE_KEY, 'cg$uit_tenants.ins.uk_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$uit_tenants.ins.others');
    called_from_package := FALSE;
    cg$errors.raise_failure;
END ins;
...
BEGIN
  ...
  THEN
    sel(cg$upd_rec);
    -- Report error if attempt to update non updateable Primary Key UIT_TNT_PK
    IF (cg$ind.id AND cg$rec.id != cg$upd_rec.id)
    THEN
      raise_uk_not_updateable('UIT_TNT_PK');
    END IF;
    IF NOT (cg$ind.id)
    THEN
...
EXCEPTION
  ...
  THEN
    validate_mandatory(cg$rec, 'cg$uit_tenants.upd.upd_mandatory_null');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.check_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_CHECK_CON, 'cg$uit_tenants.upd.check_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.fk_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_FOREIGN_KEY, 'cg$uit_tenants.upd.fk_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.uk_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_UNIQUE_KEY, 'cg$uit_tenants.upd.uk_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$uit_tenants.upd.others');
    called_from_package := FALSE;
    cg$errors.raise_failure;
END upd;
...
EXCEPTION
  ...
  THEN
    err_msg(SQLERRM, cg$errors.ERR_DELETE_RESTRICT, 'cg$uit_tenants.del.delete_restrict');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN NO_DATA_FOUND
  THEN
    cg$errors.push(cg$errors.string(cg$errors.API_ROW_DEL, cg$errors.ROW_DEL), 'E', 'ORA', SQLCODE, 'cg$uit_tenants.del.NO_DATA_FOUND');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$uit_tenants.del.others');
    called_from_package := FALSE;
    cg$errors.raise_failure;
END del;
...
EXCEPTION
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$uit_tenants.sel.others');
    cg$errors.raise_failure;
END sel;
...
BEGIN
  ...
  EXCEPTION
    ...
    THEN
      cg$errors.push(cg$errors.string(cg$errors.API_ROW_LCK, cg$errors.ROW_LCK), 'E', 'ORA', SQLCODE, 'cg$uit_tenants.lck.resource_busy');
      cg$errors.raise_failure;
    WHEN NO_DATA_FOUND
    THEN
      cg$errors.push(cg$errors.string(cg$errors.API_ROW_DEL, cg$errors.ROW_DEL), 'E', 'ORA', SQLCODE, 'cg$uit_tenants.lck.NO_DATA_FOUND');
      cg$errors.raise_failure;
    WHEN OTHERS
    THEN
      cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$uit_tenants.lck.others');
      cg$errors.raise_failure;
    END;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.string(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P10ID), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$uit_tenants.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.string(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P15ROWVERSION), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$uit_tenants.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.string(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P20CREATED_BY), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$uit_tenants.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.string(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P25CREATED_ON), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$uit_tenants.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.string(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P30UPDATED_BY), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$uit_tenants.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.string(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P35UPDATED_ON), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$uit_tenants.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.string(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P40ACTIVE), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$uit_tenants.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  IF (cg$old_ind.name)
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.string(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P45NAME), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$uit_tenants.lck');
      any_modified := TRUE;
    END IF;
  END IF;
...
--------------------------------------------------------------------------------
-- Name:        cascade_update
-- Description: Updates all child tables affected by a change to uit_tenants
--
-- Parameters:  cg$rec     the record of uit_tenants current values
--              cg$old_rec the record of uit_tenants previous values
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- Name:        cascade_delete
-- Description: Delete all child tables affected by a delete to uit_tenants
--
-- Parameters:  cg$rec     the record of uit_tenants current values
--------------------------------------------------------------------------------
