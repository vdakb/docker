-- /Users/dsteding/Project/Bundeskriminalamt12c/ExternalIdentityManagement/IdentityService/uniqueidBackend/src/main/resources/install/uid/api/uit_pts.trg
--
--   Einfuehrung einer personenbezogenen ID im Progamm Polizei 2020
--   Unique Identifier (UID)
--   Version 1.2 | Stand 15.03.2022
--
--   Reference to Page 20 Table Row 1 Vorgabe durch P20 IAM: CodeList: TH=T Partner:P
--
-- Generated for Oracle Database 12c on Fri Mar 11 12.47.21 2022 by Server Generator 10.1.2.93.10

PROMPT ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
PROMPT Creating Trigger Logic for Table 'uit_participants'
PROMPT ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
PROMPT Creating Before Insert Statement Trigger on 'uit_participants'
CREATE OR REPLACE TRIGGER cg$bis_uit_participants
...
/
SHOW ERROR
PROMPT Creating Before Insert Row Trigger on 'uit_participants'
CREATE OR REPLACE TRIGGER cg$bir_uit_participants
BEFORE INSERT ON uit_participants FOR EACH ROW
DECLARE
...
BEGIN
  ...
  THEN
    cg$uit_participants.ins(cg$rec, cg$ind, FALSE);
    cg$uit_participants.called_from_package := FALSE;
  END IF;

  cg$uit_participants.cg$table(cg$uit_participants.idx).id                := cg$rec.id;
  cg$uit_participants.cg$tableind(cg$uit_participants.idx).id             := cg$ind.id;

  cg$uit_participants.cg$table(cg$uit_participants.idx).rowversion        := cg$rec.rowversion;
  cg$uit_participants.cg$tableind(cg$uit_participants.idx).rowversion     := cg$ind.rowversion;

  cg$uit_participants.cg$table(cg$uit_participants.idx).created_by        := cg$rec.created_by;
  cg$uit_participants.cg$tableind(cg$uit_participants.idx).created_by     := cg$ind.created_by;

  cg$uit_participants.cg$table(cg$uit_participants.idx).created_on        := cg$rec.created_on;
  cg$uit_participants.cg$tableind(cg$uit_participants.idx).created_on     := cg$ind.created_on;

  cg$uit_participants.cg$table(cg$uit_participants.idx).updated_by        := cg$rec.updated_by;
  cg$uit_participants.cg$tableind(cg$uit_participants.idx).updated_by     := cg$ind.updated_by;

  cg$uit_participants.cg$table(cg$uit_participants.idx).updated_on        := cg$rec.updated_on;
  cg$uit_participants.cg$tableind(cg$uit_participants.idx).updated_on     := cg$ind.updated_on;

  cg$uit_participants.cg$table(cg$uit_participants.idx).name              := cg$rec.name;
  cg$uit_participants.cg$tableind(cg$uit_participants.idx).name           := cg$ind.name;

  cg$uit_participants.cg$table(cg$uit_participants.idx).description       := cg$rec.description;
  cg$uit_participants.cg$tableind(cg$uit_participants.idx).description    := cg$ind.description;
...
/
SHOW ERROR
PROMPT Creating After Insert Row Trigger on 'uit_participants'
CREATE OR REPLACE TRIGGER cg$air_uit_participants
AFTER INSERT ON uit_participants FOR EACH ROW
DECLARE
  cg$rec cg$uit_participants.cg$row_type;
  cg$ind cg$uit_participants.cg$ind_type;
BEGIN
...
/
SHOW ERROR
PROMPT Creating After Insert Statement Trigger on 'uit_participants'
CREATE OR REPLACE TRIGGER cg$ais_uit_participants
...
/
SHOW ERROR
PROMPT Creating Before Update Statement Trigger on 'uit_participants'
CREATE OR REPLACE TRIGGER cg$bus_uit_participants
BEFORE UPDATE ON uit_participants
BEGIN
...
/
SHOW ERROR
PROMPT Creating Before Update Row Trigger on 'uit_participants'
CREATE OR REPLACE TRIGGER cg$bur_uit_participants
...
/
SHOW ERROR
PROMPT Creating After Update Statement Trigger on 'uit_participants'
CREATE OR REPLACE TRIGGER cg$aus_uit_participants
AFTER UPDATE ON uit_participants
DECLARE
  idx        BINARY_INTEGER := cg$uit_participants.cg$table.FIRST;
  cg$old_rec cg$uit_participants.cg$row_type;
  cg$rec     cg$uit_participants.cg$row_type;
...
BEGIN
  ...
  WHILE idx IS NOT NULL
  LOOP
    cg$old_rec.id             := cg$uit_participants.cg$table(idx).id;
    cg$old_rec.rowversion     := cg$uit_participants.cg$table(idx).rowversion;
    cg$old_rec.created_by     := cg$uit_participants.cg$table(idx).created_by;
    cg$old_rec.created_on     := cg$uit_participants.cg$table(idx).created_on;
    cg$old_rec.updated_by     := cg$uit_participants.cg$table(idx).updated_by;
    cg$old_rec.updated_on     := cg$uit_participants.cg$table(idx).updated_on;
    cg$old_rec.name           := cg$uit_participants.cg$table(idx).name;
    cg$old_rec.description    := cg$uit_participants.cg$table(idx).description;
    ...
    THEN
      ...
      cg$uit_participants.validate_foreign_keys_upd(cg$rec, cg$old_rec, cg$ind);
      cg$uit_participants.cascade_update(cg$rec, cg$old_rec);
...
/
SHOW ERROR
PROMPT Creating Before Delete Statement Trigger on 'uit_participants'
CREATE OR REPLACE TRIGGER cg$bds_uit_participants
...
/
SHOW ERROR
PROMPT Creating Before Delete Row Trigger on 'uit_participants'
CREATE OR REPLACE TRIGGER cg$bdr_uit_participants
...
/
SHOW ERROR
PROMPT Creating After Delete Statement Trigger on 'uit_participants'
CREATE OR REPLACE TRIGGER cg$ads_uit_participants
AFTER DELETE ON uit_participants
DECLARE
  idx        BINARY_INTEGER := cg$uit_participants.cg$table.FIRST;
  cg$rec     cg$uit_participants.cg$row_type;
  cg$old_rec cg$uit_participants.cg$row_type;
BEGIN
  ...
  THEN
    ...
    LOOP
      cg$rec.id := cg$uit_participants.cg$table(idx).id;
      cg$uit_participants.cg$tableind(idx).id := TRUE;

      cg$rec.rowversion := cg$uit_participants.cg$table(idx).rowversion;
      cg$uit_participants.cg$tableind(idx).rowversion := TRUE;

      cg$rec.created_by := cg$uit_participants.cg$table(idx).created_by;
      cg$uit_participants.cg$tableind(idx).created_by := TRUE;

      cg$rec.created_on := cg$uit_participants.cg$table(idx).created_on;
      cg$uit_participants.cg$tableind(idx).created_on := TRUE;

      cg$rec.updated_by := cg$uit_participants.cg$table(idx).updated_by;
      cg$uit_participants.cg$tableind(idx).updated_by := TRUE;

      cg$rec.updated_on := cg$uit_participants.cg$table(idx).updated_on;
      cg$uit_participants.cg$tableind(idx).updated_on := TRUE;

      cg$uit_participants.validate_foreign_keys_del(cg$rec);
      cg$uit_participants.cascade_delete(cg$rec);

      cg$rec.name := cg$uit_participants.cg$table(idx).name;
      cg$uit_participants.cg$tableind(idx).name := TRUE;

      cg$rec.description := cg$uit_participants.cg$table(idx).description;
      cg$uit_participants.cg$tableind(idx).description := TRUE;
