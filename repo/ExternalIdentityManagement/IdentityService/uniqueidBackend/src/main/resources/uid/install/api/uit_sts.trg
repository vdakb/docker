...
PROMPT ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
PROMPT Creating Trigger Logic for Table 'uit_states'
PROMPT ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
PROMPT Creating Before Insert Statement Trigger on 'uit_states'
CREATE OR REPLACE TRIGGER cg$bis_uit_states
...
/
SHOW ERROR
PROMPT Creating Before Insert Row Trigger on 'uit_states'
CREATE OR REPLACE TRIGGER cg$bir_uit_states
...
/
SHOW ERROR
PROMPT Creating After Insert Row Trigger on 'uit_states'
CREATE OR REPLACE TRIGGER cg$air_uit_states
...
END;
/
PROMPT Creating After Insert Statement Trigger on 'uit_states'
CREATE OR REPLACE TRIGGER cg$ais_uit_states
AFTER INSERT ON uit_states
DECLARE
  fk_check   INTEGER;
  idx        BINARY_INTEGER := cg$uit_states.cg$table.FIRST;
  cg$rec     cg$uit_states.cg$row_type;
...
BEGIN
  ...
  THEN
    ...
    LOOP
      cg$rec.id             := cg$uit_states.cg$table(idx).id;
      cg$rec.rowversion     := cg$uit_states.cg$table(idx).rowversion;
      cg$rec.created_by     := cg$uit_states.cg$table(idx).created_by;
      cg$rec.created_on     := cg$uit_states.cg$table(idx).created_on;
      cg$rec.updated_by     := cg$uit_states.cg$table(idx).updated_by;
      cg$rec.updated_on     := cg$uit_states.cg$table(idx).updated_on;
      cg$rec.name           := cg$uit_states.cg$table(idx).name;

      cg$uit_states.validate_foreign_keys_ins(cg$rec);

      idx := cg$uit_states.cg$table.NEXT(idx);
    END LOOP;
  END IF;
...
/
SHOW ERROR
PROMPT Creating Before Update Statement Trigger on 'uit_states'
CREATE OR REPLACE TRIGGER cg$bus_uit_states
...
/
SHOW ERROR
PROMPT Creating Before Update Row Trigger on 'uit_states'
CREATE OR REPLACE TRIGGER cg$bur_uit_states
...
/
SHOW ERROR
PROMPT Creating After Update Statement Trigger on 'uit_states'
CREATE OR REPLACE TRIGGER cg$aus_uit_states
AFTER UPDATE ON uit_states
DECLARE
  idx        BINARY_INTEGER := cg$uit_states.cg$table.FIRST;
  cg$old_rec cg$uit_states.cg$row_type;
  cg$rec     cg$uit_states.cg$row_type;
...
BEGIN
  ...
  WHILE idx IS NOT NULL
  LOOP
    cg$old_rec.id             := cg$uit_states.cg$table(idx).id;
    cg$old_rec.rowversion     := cg$uit_states.cg$table(idx).rowversion;
    cg$old_rec.created_by     := cg$uit_states.cg$table(idx).created_by;
    cg$old_rec.created_on     := cg$uit_states.cg$table(idx).created_on;
    cg$old_rec.updated_by     := cg$uit_states.cg$table(idx).updated_by;
    cg$old_rec.updated_on     := cg$uit_states.cg$table(idx).updated_on;
    cg$old_rec.name           := cg$uit_states.cg$table(idx).name;
    ...
    THEN
      ...
      cg$uit_states.validate_foreign_keys_upd(cg$rec, cg$old_rec, cg$ind);
      cg$uit_states.cascade_update(cg$rec, cg$old_rec);
...
/
SHOW ERROR
PROMPT Creating Before Delete Statement Trigger on 'uit_states'
CREATE OR REPLACE TRIGGER cg$bds_uit_states
...
/
SHOW ERROR
PROMPT Creating Before Delete Row Trigger on 'uit_states'
CREATE OR REPLACE TRIGGER cg$bdr_uit_states
...
/
SHOW ERROR
PROMPT Creating After Delete Statement Trigger on 'uit_states'
CREATE OR REPLACE TRIGGER cg$ads_uit_states
AFTER DELETE ON uit_states
DECLARE
  idx        BINARY_INTEGER := cg$uit_states.cg$table.FIRST;
  cg$rec     cg$uit_states.cg$row_type;
  cg$old_rec cg$uit_states.cg$row_type;
BEGIN
  ...
  THEN
    ...
    LOOP
      cg$rec.id := cg$uit_states.cg$table(idx).id;
      cg$uit_states.cg$tableind(idx).id := TRUE;

      cg$rec.rowversion := cg$uit_states.cg$table(idx).rowversion;
      cg$uit_states.cg$tableind(idx).rowversion := TRUE;

      cg$rec.created_by := cg$uit_states.cg$table(idx).created_by;
      cg$uit_states.cg$tableind(idx).created_by := TRUE;

      cg$rec.created_on := cg$uit_states.cg$table(idx).created_on;
      cg$uit_states.cg$tableind(idx).created_on := TRUE;

      cg$rec.updated_by := cg$uit_states.cg$table(idx).updated_by;
      cg$uit_states.cg$tableind(idx).updated_by := TRUE;

      cg$rec.updated_on := cg$uit_states.cg$table(idx).updated_on;
      cg$uit_states.cg$tableind(idx).updated_on := TRUE;

      cg$uit_states.validate_foreign_keys_del(cg$rec);
      cg$uit_states.cascade_delete(cg$rec);

      cg$rec.name := cg$uit_states.cg$table(idx).name;
      cg$uit_states.cg$tableind(idx).name := TRUE;
