/*
    ...
    ------------+-----------+-----------+-----------------------------------
    1.0.0.0      25.02.2020  SBernet    First release version
*/
 ...
 ** <code>Branding shell Template</code> configuration from Matadata Service.
 **
 ** @author  sylvert.bernet@oracle.com
 ** @version 1.0.0.0
 ** @since   1.0.0.0
 */
public class Serializer extends XMLProcessor {
  ...
  enum Grammar {
    ...
    , COMPONENT          (CONFIGURATION,            ELEMENT_COMPONENT)
    , RULE               (COMPONENT,                ELEMENT_RULE)
    ;
     ...
     ** all the enums are constructed.
      */
    static {
      for (Grammar state : Serializer.Grammar.values()) {
   ...
   */
    ...
    private Grammar              cursor = Serializer.Grammar.INIT;
     ...
     *
     * @param model the {@link bka.iam.identity.ui.shell.branding.model.Configuration} to be unmarshalled by
     * this {@link XMLProcessor}.
     * <br>
     * Allowed object is {@link bka.iam.identity.ui.shell.branding.model.Configuration}.
     *
    ...
    public void startElement(final String uri, final String localName, final String qualifiedName, final Attributes attributes)
      ...
      Grammar state = Serializer.Grammar.lookup(localName);
      if (state == null) {
      ...
      try {
        switch (state) {
          case CONFIGURATION      :
                                    break;
          case COMPONENT          : push(new Component(attributes.getValue(ATTRIBUTE_NAME)));
                                    break;
          case RULE               : push(new Rule(attributes.getValue(ATTRIBUTE_ID), attributes.getValue(ATTRIBUTE_ATTRIBUTE), attributes.getValue(ATTRIBUTE_FILTER)));
    ...
    public void endElement(final String uri, final String localName, final String qualifiedName)
      ...
      // dispatch to handling method
      switch (this.cursor) {
        case COMPONENT          : final Component environment = (Component)pop();
                                  this.model.add(environment);
                                  break;
        case RULE              : final StringBuilder value = (StringBuilder)pop();
                                 final Rule attribute = (Rule)pop();
                                 attribute.setValue(value.toString());
                                 // if an attribute is about to handle there must be
                                 // an attribute container on the stack
                                 ((Component)peek()).add(attribute);
                                 break;
      }
      // change FSA state
    ...
    public void characters(final char[] buffer, final int start, final int length)
      ...
      // where we are?
      switch (this.cursor) {
        case RULE        : // if we have a mapping or an entitlement element
  ...
  public static void unmarshal(final Configuration feature, final Document document)
    ...
    // prevent bogus input
    if (document == null)
      throw new BrandingException("Argument null. Serial unmarshal");
  ...
  // Method:   parse
  /**
   ** Factory method to parse a {@link Configuration} from an {@link Document}.
   **
   ** @param  feature            the {@link Configuration} to be unmarshalled by
   **                            this {@link XMLProcessor}.
   **                            <br>
   **                            Allowed object is {@Configuration}.
   ** @param  document           the configuration source for the descriptor to
   **                            create by parsing the underlying XML.
   **                            <br>
   **                            Allowed object is {@link Document}.
  ...
  private void parse(final Configuration feature, final Document document)
    ...
    // prevent bogus input
    if (document == null)
      throw Adapter.exception(BrandingError.ARGUMENT_IS_NULL, "document");
    ...
    try {
      ...
      throw Adapter.exception(BrandingError.BRANDING_CONFIGURATION_PARSING, e);
    }
  }
}
