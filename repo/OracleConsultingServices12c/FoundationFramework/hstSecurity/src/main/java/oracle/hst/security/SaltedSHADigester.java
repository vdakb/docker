 ...
 */
class SaltedSHADigester extends    Digester
  ...
  // Method:   Ctor
  /**
   ** Constructs a <code>SaltedSHADigester</code> handler with the specified
   ** properties.
   **
   ** @param  digesterTag        the digester algorithm tag like {SSHA-512} to
   **                            prefix the encoded hash value with.
   ** @param  hashSize           the size of a generated hash.
   ** @param  digester           the hash algorithm to use.
   */
  protected SaltedSHADigester(final String digesterTag, final int hashSize, final MessageDigest digester) {
    // ensure inheritance
    super();
  ...
  public boolean match(final String password, final String expected) {
    return match(password.getBytes(), expected);
  }
   ...
   **                            Possible object array of {@link String}.
   */
  protected String digest(final byte[] password) {
    // generate a long random salt using a CSPRNG.
    final byte[] salt = new byte[SALT_SIZE];
    PRNG.nextBytes(salt);
    // hash the password by prepend the salt to the given password and hash it
    // using the same hash function.
    final byte[] plain = new byte[password.length + SALT_SIZE];
    System.arraycopy(password, 0, plain, 0,               password.length);
    System.arraycopy(salt,     0, plain, password.length, SALT_SIZE);
    // create the hash from the concatenated value.
    final byte[] digest = this.digester.digest(plain);
   ...
   **                            <code>false</code> otherwise.
   */
  protected boolean match(final byte[] password, final String expected) {
    ...
    byte[] plain = new byte[password.length + length];
    System.arraycopy(password,  0, plain, 0,               password.length);
    System.arraycopy(salt,      0, plain, password.length, length);
    // create the hash from the concatenated value.
    final byte[] salted = this.digester.digest(plain);
