 ...
 ** @since   1.0.0.0
 */
public class PBEDigester extends Digester {
   ...
   */
  public static final String  HASH_SCHEMA    = "{SHA1}";
  ...
  // Method:   Ctor
  /**
   ** Constructs a <code>PBEDigester</code> handler that allows use as a
   ** JavaBean.
   ** <br>
  ...
  public static String digest(final char[] password) {
    ...
    return HASH_SCHEMA + ":" + HASH_ITERATION + ":" + hash.length + ":" + encode(salt) + ":" + encode(hash);
  }
  ...
  public static boolean match(final char[] password, final String expected) {
    ...
    if (!section[ALGORITHM].equals(HASH_SCHEMA)) {
  ...
  public static void main(String[] args) {
    final String expected = PBEDigester.digest("Welcome1");
    System.out.println(PBEDigester.match("Palaver", expected) ? "match" :"nomatch");
    System.out.println(PBEDigester.match("Welcome1", expected) ? "match" :"nomatch");
  }
}