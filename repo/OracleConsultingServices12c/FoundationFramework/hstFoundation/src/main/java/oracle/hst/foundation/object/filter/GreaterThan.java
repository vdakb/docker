...
// ~~~~~ ~~~~~~~~~~~
/**
 ** Determine if the {@link Entity} {@link Attribute} value is greater than the
 ** one provided in the filter.
 **
 ** @author  dieter.steding@oracle.com
 ** @version 1.1.0.0
 ** @since   1.1.0.0
...
public class GreaterThan<T> extends ComparableAttributeFilter<T> {
  ...
  // Method:   Ctor
  /**
   ** Constructs a <code>GreaterThan</code> w/ the {@link Attribute} to compare.
   **
   ** @param  attribute          the {@link Attribute} this
   **                            {@link ComparableAttributeFilter} belongs to.
   */
  public GreaterThan(final Attribute attribute) {
    // ensure inheritance
    super(attribute);
  }
   ...
   ** has the same name and contains a value that is greater than the value of
   ** the attribute that <code>FilterBuilder</code> placed into this filter.
   **
  ...
  public <R, P> R accept(final FilterVisitor<R, P> visitor, final P parameter) {
    return visitor.greaterThan(parameter, this);
  }
}