...
public abstract class SAXInput extends DefaultHandler {
   ...
   **
   ** @param  parser             the {@link SAXParser} used to handle the
   **                            parsing duties and simply listens to the SAX
   **                            events to construct a document.
   */
  ...
  public void processDocument(final InputStream stream)
    ...
    // prevent bogus input
    if (stream == null)
      throw new SAXException(XMLStreamBundle.format(SystemError.ARGUMENT_IS_NULL, "stream"));
  ...
  public void processDocument(final InputSource source)
    ...
    // prevent bogus input
    if (source == null)
      throw new SAXException(XMLStreamBundle.format(SystemError.ARGUMENT_IS_NULL, "source"));
