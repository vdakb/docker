...
import oracle.hst.foundation.security.IntegerEncryptor;

////////////////////////////////////////////////////////////////////////////////
// class PBEIntegerEncryptor
// ~~~~~ ~~~~~~~~~~~~~~~~~~~
/**
 ** Default implementation of the {@link IntegerEncryptor} interface.
 ** <p>
 ...
 ** @since   1.0.1.0
 */
public class PBEIntegerEncryptor extends    PBENumericEncryptor
                                 implements IntegerEncryptor {
  ...
  // Method:   Ctor
  /**
   ** Constructs a <code>PBEIntegerEncryptor</code> that allows use as a
   ** JavaBean.
   ** <br>
  ...
  // Method:   Ctor
  /**
   ** Constructs a <code>PBEIntegerEncryptor</code> with the specified password
   ** to use.
   ** <p>
   ...
   */
  public PBEIntegerEncryptor(final String password) {
    // ensure inheritance
    super(password);
  }
  ...
  // Method:   Ctor
  /**
   ** Constructs an <code>PBEIntegerEncryptor</code> with the specified
   ** password to use.
   ** <p>
   ...
   ** @param  password           the password to be used.
   */
  public PBEIntegerEncryptor(final char[] password) {
    // ensure inheritance
    super(password);
  }
  ...
  // Method:   Ctor
  /**
   ** Constructs an empty <code>PBEIntegerEncryptor</code> using the
   ** specified byte encryptor (constructor used for cloning).
   **
   ** @param  cryptor            the byte encryptor to use.
   */
  public PBEIntegerEncryptor(final PBEBinaryEncryptor cryptor) {
    // ensure inheritance
    super(cryptor);
  }
  ...
  public BigInteger encrypt(final BigInteger message) {
    ...
    try {
      // the input BigInteger is converted into bytes and the
      // DefaultPBEByteEncryptor does the rest
      final byte[] bytes = this.cryptor.encrypt(message.toByteArray());
   ...
   */
  @Override
  public BigInteger decrypt(final BigInteger message) {
    // prevent bogus inpout
    if (message == null)
      return message;
    ...
    try {
      // the input BigInteger is converted into bytes.
      // the DefaultPBEByteEncryptor does
      byte[] raw = this.cryptor.decrypt(message.toByteArray());
      // process the encrypted byte array (check size, pad if needed...)
      raw = process(raw, message.signum());
      // let the byte encyptor decrypt
      byte[] load = this.cryptor.decrypt(raw);
      // finally, return a new number built from the decrypted bytes
      return new BigInteger(load);
    }
    catch (Exception e) {
      // if encryption fails, it is more secure not to return any information
      // about the cause in nested exceptions. Simply fail.
