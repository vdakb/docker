...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:axf="http://www.antennahouse.com/names/XSL/Extensions" version='1.0'>
    ...
    <fo:block id="{$id}">
      ...
      <xsl:if test="$refentry.generate.name != 0">
        <xsl:choose>
          ...
          <xsl:otherwise>
            <fo:block xmlns:fo="http://www.w3.org/1999/XSL/Format" xsl:use-attribute-sets="refnamediv.titlepage.recto.style" font-family="{$title.fontset}">
  ...
  <xsl:template match="refsynopsisdiv/title
                     ...
                     |refsect1/info/title
                     |refsect2/info/title
                     |refsect3/info/title" mode="titlepage.mode" priority="2">
    <xsl:call-template name="format.refentry.subheading"/>
  </xsl:template>
  <xsl:template name="format.refentry.subheading">
    ...
    <!--     </xsl:call-template> -->
    <!-- -->
    <xsl:param name="section" select="(ancestor::refsynopsisdiv 
                     |ancestor::refsection
                     |ancestor::refsect1
                     |ancestor::refsect2
