...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:axf="http://www.antennahouse.com/names/XSL/Extensions" version='1.0'>
  ...
  <xsl:template match="set">
    ...
    <xsl:variable name="preamble" select="*[not(self::book or self::set or self::setindex)]"/>
  ...
  <xsl:template match="book">
    ...
    <xsl:variable name="content" select="node()[not(self::title or self::subtitle
                            or self::titleabbrev
                            or self::info
                            or self::bookinfo)]"/>
    <xsl:variable name="titlepage-master-reference">
      ...
      <xsl:call-template name="page.sequence">
        ...
        <xsl:with-param name="content">
          <xsl:call-template name="list.of.titles">
            ...
            <xsl:with-param name="nodes" select=".//procedure[title or info/title]"/>
          </xsl:call-template>
        </xsl:with-param>
      </xsl:call-template>
    </xsl:if>
