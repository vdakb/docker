...
-->
    ...
    <xsl:variable name="content">
      <xsl:apply-templates mode="vl.as.list" select="varlistentry
    ...
    <xsl:choose>
      <xsl:when test="ancestor::listitem">
        <fo:list-block id="{$id}" provisional-distance-between-starts=
                        "{$distance-between-starts}" provisional-label-separation="{$label-separation}">
          <xsl:if test="$keep.together != ''">
            <xsl:attribute name="keep-together.within-column">
              <xsl:value-of select="$keep.together"/>
            </xsl:attribute>
      ...
      <xsl:otherwise>
        <fo:list-block id="{$id}" provisional-distance-between-starts=
                        "{$distance-between-starts}" provisional-label-separation="{$label-separation}" xsl:use-attribute-sets="list.block.spacing">
          <xsl:if test="$keep.together != ''">
            <xsl:attribute name="keep-together.within-column">
              <xsl:value-of select="$keep.together"/>
            </xsl:attribute>
  ...
  <xsl:template name="longest.term">
    ...
    <xsl:choose>
      ...
      <xsl:when test="string-length($term.text) &gt; $longest">
        <xsl:call-template name="longest.term">
          <xsl:with-param name="longest" select="string-length($term.text)"/>
  ...
  <xsl:template match="variablelist" mode="vl.as.blocks">
    ...
    <xsl:apply-templates select="*[not(self::varlistentry
            ...
            |processing-instruction()[not(preceding-sibling::varlistentry)]"/>
    <xsl:variable name="content">
      <xsl:apply-templates mode="vl.as.blocks" select="varlistentry
              |comment()[preceding-sibling::varlistentry]
              |processing-instruction()[preceding-sibling::varlistentry]"/>
    </xsl:variable>
    <!-- nested lists don't add extra list-block spacing -->
  ...
  <xsl:template match="varlistentry" mode="vl.as.blocks">
    ...
    <fo:block id="{$id}" xsl:use-attribute-sets="variablelist.term.properties
                                               list.item.spacing" keep-together.within-column="always" keep-with-next.within-column="always">
      <xsl:apply-templates select="term"/>
    </fo:block>
    <fo:block>
  ...
  <xsl:template name="simplelist.table.columns">
    ...
    <fo:table-column column-number="{$curcol}" column-width="proportional-column-width(1)"/>
  ...
  <xsl:template match="procedure">
    ...
    <fo:block id="{$id}" xsl:use-attribute-sets="procedure.properties list.block.spacing">
      ...
      <fo:list-block xsl:use-attribute-sets="list.block.spacing" provisional-distance-between-starts="2em" provisional-label-separation="0.2em">
  ...
  <xsl:template match="stepalternatives">
    <fo:list-block provisional-distance-between-starts="2em" provisional-label-separation="0.2em">
  ...
  <xsl:template match="step/title">
    <fo:block font-weight="bold" keep-together.within-column="always" keep-with-next.within-column="always">
      <xsl:apply-templates/>
    </fo:block>
  </xsl:template>
  ...
  <xsl:template match="calloutlist">
    ...
    <fo:block id="{$id}" text-align="{$alignment}">
      ...
      <!-- Preserve order of PIs and comments -->
      <xsl:apply-templates select="*[not(self::callout or self::title or self::titleabbrev)]
