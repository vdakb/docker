...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:k="java:com.isogen.saxoni18n.Saxoni18nService" exclude-result-prefixes="k" version="1.0">
  <!-- ********************************************************************
  ...
  <xsl:template name="generate-kimber-index">
    ...
    <xsl:variable name="terms" select="//indexterm[count(.|key('k-group',
                   k:getIndexGroupKey(&lang;, &primary;))
                   [&scope;][1]) = 1
                   and not(@class = 'endofrange')]"/>
    <xsl:variable name="alphabetical" select="$terms[not(starts-with(
                k:getIndexGroupKey(&lang;, &primary;),
                '#NUMERIC'
                ))]"/>
    <xsl:variable name="others" select="$terms[starts-with(
                k:getIndexGroupKey(&lang;, &primary;),
                '#NUMERIC'
                )]"/>
    <fo:block>
      <xsl:if test="$others">
        ...
        <fo:block>
          <xsl:apply-templates select="$others" mode="index-symbol-div">
            ...
            <xsl:sort lang="{$sort.lang}" select="k:getIndexGroupSortKey(&lang;,
                      k:getIndexGroupKey(&lang;, &primary;))"/>
          </xsl:apply-templates>
        </fo:block>
      </xsl:if>
  ...
  <xsl:template match="indexterm" mode="index-div-kimber">
    ...
    <xsl:variable name="key" select="k:getIndexGroupKey(&lang;, &primary;)"/>
    <xsl:variable name="label" select="k:getIndexGroupLabel(&lang;, $key)"/>
