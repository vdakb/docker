...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="admon.graphic">
    ...
    <xsl:variable name="filename">
      ...
      <xsl:choose>
        <xsl:when test="local-name($node)='note'">icon-note-16</xsl:when>
        <xsl:when test="local-name($node)='warning'">icon-warning-16</xsl:when>
        <xsl:when test="local-name($node)='caution'">icon-caution-16</xsl:when>
        <xsl:when test="local-name($node)='tip'">icon-tip-16</xsl:when>
        <xsl:when test="local-name($node)='important'">icon-important-16</xsl:when>
  ...
  <xsl:template name="nongraphical.admonition">
    ...
    <fo:block id="{$id}" background-color="{$background}" padding="5mm" border-left="2px solid {$bordercolor}">
