...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!--
  =====================================================================
  == Give anchor zwsp content, otherwise FOP discards the empty inline
  == and the link target is lost
  =====================================================================
  -->
  ...
  <xsl:template match="ulink//text()">
    ...
    <xsl:choose>
      <xsl:when test="$me = $url-attr or $me = substring-after($url-attr, ':') or $me = substring-after($url-attr, '//')">
      ...
      <xsl:choose>
        <xsl:when test="$ulink.footnotes != 0 and not(ancestor::footnote)">
          <fo:footnote>
            ...
            <fo:footnote-body font-family="{$body.font.family}" font-size="{$footnote.font.size}">
              <fo:block>
                <xsl:call-template name="ulink.footnote.number"/>
                <xsl:text> </xsl:text>
                <fo:inline>
  ...
  <xsl:template match="link" name="link">
    ...
    <xsl:choose>
      ...
      -->
      <xsl:when test="starts-with(normalize-space($xrefstyle), 'select:') and contains($xrefstyle, 'nopage')">
      </xsl:when>
      <!--
      so does a negative insert.link.page.number
      -->
      <xsl:when test="$insert.link.page.number = 'no' or $insert.link.page.number = '0'">
      </xsl:when>
