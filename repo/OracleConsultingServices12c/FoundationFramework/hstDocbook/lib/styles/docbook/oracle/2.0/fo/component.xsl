...
<xsl:stylesheet xmlns:xsl               ="http://www.w3.org/1999/XSL/Transform"
  ...
  ==
  == TODO:
  == The whole possibly-labeled-title / label / title business could be
  == organized more transparently!
  =====================================================================
  -->
  <xsl:template name="component.title">
    ...
    <xsl:variable name="title">
      <xsl:choose>
        <!--
        For a chapter, $title contains just the unlabeled title, with
        ...
        <!--
        For a chapter-level appendix, we include the (conditional) autolabel
        and the title together in the $title variable, with
        chapter.title.properties for the two
        -->
        ...
        <!--
        For all other components, $title contains the possibly labeled title
        -->
        <xsl:otherwise>
    ...
    <xsl:if test="$passivetex.extensions != 0">
      <fotex:bookmark xmlns:fotex="http://www.tug.org/fotex" fotex-bookmark-level="2" fotex-bookmark-label="{$id}">
    ...
    <xsl:choose>
      <xsl:when test="$node/self::chapter or $is-chapterlevel-appendix = 1">
        <!--
        dsteding: Put a heading bar on top of the chapter title
        -->
        <fo:block text-align="start" start-indent="0pt" border-top="6pt solid black" padding-bottom="2mm"/>
        <fo:block text-align="start" start-indent="0pt">
      ...
      <xsl:otherwise> <!-- $node is no chapter or chapter-level appendix: -->
        ...
        -->
        <fo:block text-align="start" start-indent="0pt" border-top="6pt solid black" padding-bottom="2mm"/>
        <fo:block text-align="start" start-indent="0pt" xsl:use-attribute-sets="chapter.title.properties">
          <xsl:copy-of select="$title"/>
        </fo:block>
      </xsl:otherwise>
    </xsl:choose>
  ...
  <!-- with coverpage etc. -->
  <xsl:template match="article" mode="article.toplevel.mode">
    <xsl:variable name="id">
      <xsl:call-template name="object.id"/>
    </xsl:variable>
    <xsl:variable name="preamble" select="title | subtitle | titleabbrev | artheader | articleinfo"/>
    <xsl:variable name="content"  select="*[not(self::title or self::subtitle or self::titleabbrev or self::artheader or self::articleinfo or self::index)]"/>
    ...
    <xsl:if test="contains($toc.params, 'toc')">
      <fo:page-sequence hyphenate="{$hyphenate}" format="i" master-reference="{$lot-master-reference}">
        <xsl:attribute name="language">
          <xsl:call-template name="l10n.language"/>
        </xsl:attribute>
        <xsl:attribute name="format">
        ...
        <fo:flow flow-name="xsl-region-body">
          <!--
          set has set.toc here, book division.toc
          -->
    ...
    <fo:page-sequence hyphenate="{$hyphenate}" master-reference="{$master-reference}">
      <xsl:attribute name="language">
        <xsl:call-template name="l10n.language"/>
      </xsl:attribute>
      <xsl:attribute name="format">
  ...
  <xsl:template match="article" mode="article.nontoplevel.mode">
    ...
    <fo:page-sequence hyphenate="{$hyphenate}" master-reference="{$master-reference}">
      ...
      <xsl:choose>
        <xsl:when test="not(preceding::chapter or preceding::preface or preceding::appendix or preceding::article or preceding::dedication or parent::part or parent::reference)">
          <!--
  ...
  <xsl:template match="article/appendix">
    ...
    <!-- break-before is our addition -->
    <fo:block id='{$id}' break-before="page">
      <xsl:if test="$passivetex.extensions != 0">
        <fotex:bookmark xmlns:fotex="http://www.tug.org/fotex" fotex-bookmark-level="{count(ancestor::*)+2}" fotex-bookmark-label="{$id}">
          <xsl:value-of select="$titleabbrev"/>
        </fotex:bookmark>
      </xsl:if>
  ...
  ==
  == Preface is a preface or forward in a Book. The Preface element may
  == appear more than once and should be used for all introductory
  == chapter-like material. For example, a Book might have both a
  == Foreward and an Introduction. Both should be tagged as Prefaces in
  == DocBook.
  ==
  == By Me: Thats why it requires a title element 
  =====================================================================
  -->
  <!--
