  ...
  <xsl:template name="hyphenate-special">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:choose>
          <xsl:when test="$this-lastchar = $rest-firstchar and contains($not-between, $this-lastchar)"/>
          <xsl:when test="contains($after, $this-lastchar) and not (contains($not-before, $rest-firstchar))">
            <xsl:copy-of select="$hyph-char"/>
          </xsl:when>
          <xsl:when test="contains($before, $rest-firstchar) and not (contains($not-after, $this-lastchar))">
            <xsl:copy-of select="$hyph-char"/>
          </xsl:when>
          <xsl:otherwise/>
        </xsl:choose>
