 ...
 ** @since   1.0.0.0
 */
public abstract class AttributeFilter<T> implements Filter<T> {
   ...
   **                            Allowed object is <code>T</code>.
   */
  protected AttributeFilter(final Type type, final Path path, final T value) {
    // ensure inheritance
    super();
   ...
   ** <p>
   ** Two <code>AttributeFilter</code>s are considered equal if and only if they
   ** represent the same properties. As a consequence, two given
   ** <code>AttributeFilter</code>s may be different even though they contain
