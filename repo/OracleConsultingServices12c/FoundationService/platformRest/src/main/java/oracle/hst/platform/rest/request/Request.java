...
package oracle.hst.platform.rest.request;
...
public abstract class Request<T extends Request> {
   ...
   ** <br>
   ** The primary purpose of this method is the ability to override this method.
   **
  ...
  protected Invocation.Builder buildRequest() {
    return build().request(this.accept.toArray(new String[this.accept.size()])).headers(this.headers);
  }
  ...
  protected WebTarget build() {
    for (Map.Entry<String, List<Object>> query : this.parameter.entrySet()) {
      this.target = this.target.queryParam(query.getKey(), query.getValue().toArray());
    }
    return this.target;
  }
}