...
// ~~~~~ ~~~~~~~~~~~~~~~~
/**
 ** Deserializes REST DateTime values to {@link Date} objects.
 **
 ** @author  dieter.steding@oracle.com
 ** @version 1.0.0.0
 ** @since   1.0.0.0
 */
public class DateDeserializer extends DateTimeDeserializer<Date> {
  ...
  // Method:   Ctor
  /**
   ** Constructs an empty <code>DateDeserializer</code> REST DateTime that
   ...
   **
   ** @return                    the deserialized {@link Date} value.
   **
   ** @throws IOException             if the {@link JsonParser} detects an issue
   **                                 to get the token text.
   ** @throws JsonProcessingException if string parameter does not conform to
  ...
  public Date deserialize(final JsonParser parser, final DeserializationContext context)
    ...
    try {
      ...
      throw InvalidFormatException.from(parser, "Expected a field containing a date", context, Date.class);
    }
  }
}