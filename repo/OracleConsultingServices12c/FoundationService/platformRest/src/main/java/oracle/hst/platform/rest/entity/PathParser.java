...
public class PathParser {
  ...
  private static final class StringReader extends Reader {
     ...
     ** @param  off           the offset at which to start storing
     **                          characters.
     **                          <br>
  ...
  public static Path path(final String expression)
    ...
    if (isUrn(trimmed)){
      ...
      int i;
      if (j >= 0){
        i = trimmed.substring(0, j).lastIndexOf(':');
      }
      else{
        i = trimmed.lastIndexOf(':');
      }
      String schemaUrn     = trimmed.substring(0, i++);
      String attributeName = trimmed.substring(i, trimmed.length());
      try{
        path = Path.root(schemaUrn);
      }
      catch (IllegalArgumentException e){
        throw new ParseException(SystemBundle.string(SystemError.PATH_INVALID_URN, schemaUrn),0);
      }
  ...
  /**
   * Parse Date time in the <a href="https://datatracker.ietf.org/doc/html/rfc3339">RFC3339</a> format. 
   * @param rfcDateTime
   * @return
   */
  public static Date parseRfc3339(String rfcDateTime) {
      return Date.from(ZonedDateTime.parse(rfcDateTime, rfc3339Formatter).toInstant());
  }
  ...
  private static Filter<JsonNode> filter(final StringReader reader, final boolean isValueFilter)
    throws ParseException {
    ...
    final Stack<Filter<JsonNode>> output = new Stack<Filter<JsonNode>>();
    ...
    while (current != null) {
      if (current.equals("(") && expectsNewFilter(previous)) {
        ...
        final LinkedList<Filter<JsonNode>> and = new LinkedList<Filter<JsonNode>>();
        while (!token.isEmpty()) {
        ...
        if (op.equalsIgnoreCase(Filter.Type.PR.value())) {
          ...
          JsonNode value = valueToken(reader);
          if (value == null)
            throw new ParseException(SystemBundle.string(SystemError.PATH_UNEXPECTED_EOF_FILTER), reader.pos);
   ...
   */
  private static JsonNode valueToken(final StringReader reader)
    throws ParseException {
    ...
    if(b.length() > 0){
      if(isString){
        return Support.nodeFactory().textNode(b.toString());
      }
      if(isLong){
        if("true".equals(b.toString()) || "false".equals(b.toString())){
          return Support.nodeFactory().booleanNode(Boolean.parseBoolean(b.toString()));
        }
        else{
          return Support.nodeFactory().numberNode(Long.parseLong(b.toString()));
        }
      }
    }
  ...
  public static void main(String args[]) throws ParseException {
    String pathString = "dedko[userName co \"i\" and (userName co \"a\" or userName eq false)]";
    Path path = PathParser.path(pathString);
    System.out.println("urn:"+path.schemaUrn());
