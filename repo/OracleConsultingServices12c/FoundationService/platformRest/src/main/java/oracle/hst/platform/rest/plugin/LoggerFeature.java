...
public class LoggerFeature implements Feature {
  ...
  class Interceptor implements WriterInterceptor {
    ...
    public void aroundWriteTo(final WriterInterceptorContext writerInterceptorContext)
      ...
      final Stream stream = (Stream)writerInterceptorContext.getProperty(LOGGER_ENTITY);
      writerInterceptorContext.proceed();
      if (this.logger.isLoggable(this.level) && printable(this.verbosity, writerInterceptorContext.getMediaType())) {
        if (stream != null) {
          log(stream.capture(charset(writerInterceptorContext.getMediaType())));
        }
      }
    }
    ...
    // Method: printRequest
    /**
     ** Logs HTTP request to a {@link StringBuilder} at required level.
     **
     ** @param  capture          the message capture buffer to log.
     **                          <br>
     **                          Allowed object is {@link StringBuilder}.
     ** @param  payload          the HTTP content body transfered.
     **                          <br>
     ...
     **                          <br>
     **                          Allowed object is {@link String}.
     ** @param  uri              the HTTP endpoint accessed.
     **                          <br>
     **                          Allowed object is {@link URI}.
     */
    void printRequest(final StringBuilder capture, final String payload, final long sequence, final String method, final URI uri) {
      print(capture, sequence).append(ServiceBundle.string(ServiceMessage.LOGGER_THREAD_NAME, payload, Thread.currentThread().getName()));
      print(capture, sequence).append(PREFIX_REQUEST).append(method).append(" ").append(uri.toASCIIString()).append('\n');
    }

    ////////////////////////////////////////////////////////////////////////////
    // Method: printResponse
    /**
     ** Logs HTTP response to a {@link StringBuilder} at required level.
     **
     ** @param  capture          the message capture buffer to log.
     **                          <br>
     **                          Allowed object is {@link StringBuilder}.
     ...
     */
    void printResponse(final StringBuilder capture, final String payload, final long sequence, final int status) {
      print(capture, sequence).append(ServiceBundle.string(ServiceMessage.LOGGER_THREAD_NAME, payload, Thread.currentThread().getName()));
      print(capture, sequence).append(PREFIX_RESPONSE).append(Integer.toString(status)).append('\n');
    }

    ////////////////////////////////////////////////////////////////////////////
    // Method: printHeader
    /**
     ** Logs HTTP request/response headers to a {@link StringBuilder} at
     ** required level.
     **
     ** @param  capture          the message capture buffer to log.
     **                          <br>
     ...
     **                          <br>
     **                          Allowed object is <code>long</code>.
     ** @param  prefix           the prefix to capture to visualize inbound or
     **                          outbound direction.
     **                          <br>
     **                          Allowed object is {@link String}.
     ** @param  collection       the collection of HTTP hedares of the request
     **                          or response.
     **                          <br>
     **                          Allowed object is {@link MultivaluedMap}.
     */
    void printHeader(final StringBuilder capture, final long sequence, final String prefix, final MultivaluedMap<String, String> collection) {
      for (final Map.Entry<String, List<String>> headerEntry : sorted(collection.entrySet())) {
        final List<?> value  = headerEntry.getValue();
        final String  header = headerEntry.getKey();
        if (value.size() == 1) {
          print(capture, sequence).append(prefix).append(header).append(": ").append(value.get(0)).append('\n');
        }
        else {
          ...
          print(capture, sequence).append(prefix).append(header).append(": ").append(sb.toString()).append('\n');
        }
      }
    }
    ...
    // Method: printEntity
    /**
     ** Logs HTTP request/response entity to a {@link StringBuilder} at
     ** required level.
     **
     ** @param  capture          the message capture buffer to log.
     **                          <br>
     ...
     **                          <br>
     **                          Allowed object is {@link String}.
     ** @param  stream           the stream to read the entity from.
     **                          <br>
     **                          Allowed object is {@link InputStream}.
     ** @param  charset          the {@link Charset charset} to be used to
     **                          decode the {@code byte} stream.
     **                          <br>
     **                          Allowed object is {@link Charset}.
     **
     ** @return                  the stream the entity is gotten from.
     **                          <br>
     **                          The stream pointer is resseted to the marked
     **                          position as the method was entered.
     **                          <br>
     **                          Possible object is {@link InputStream}.
     */
    InputStream printEntity(final StringBuilder capture, final long sequence, final String prefix, InputStream stream, final Charset charset)
      throws IOException {
      ...
      print(capture, sequence).append(prefix).append(new String(entity, 0, Math.min(size, this.size), charset));
      if (size > this.size)
        print(capture, sequence).append(prefix).append(ServiceBundle.string(ServiceMessage.LOGGER_ELIPSE_MORE));
    ...
    // Method: sorted
    /**
     ** Returns the headers of a request or a response sorted lexicographically.
     **
     ** @param  collection       the {@link Set} of HTTP headers.
     **                          <br>
     **                          Allowed object is {@link Set} where each
     **                          element is of type {@link Map.Entry} with type
     **                          {@link String} for the key and {@link List} of
     **                          {@link String} as he value.
     **
     ...
     **                          {@link String} as he value.
     */
    Set<Map.Entry<String, List<String>>> sorted(final Set<Map.Entry<String, List<String>>> collection) {
   ...
   **
   ** @param  logger             the {@link Logger} to log requests and
   **                            responses.
   **                            <br>
   **                            Allowed object is {@link Logger}.
   */
  public LoggerFeature(final Logger logger) {
    // ensure inheritance
  ...
  /**
   ** Constructs a <code>LoggerFeature</code> which is associated with
   ** the specified {@link Logger} and verbosity.
   **
   ** @param  logger             the {@link Logger} to log requests and
   **                            responses.
   **                            <br>
   ...
   */
  public LoggerFeature(final Logger logger, final Verbosity verbosity) {
    // ensure inheritance
  ...
  /**
   ** Constructs a <code>LoggerFeature</code> which is associated with
   ** the specified {@link Logger} and maximum number of bytes of entity to
   ** log.
   **
   ** @param  logger             the {@link Logger} to log requests and
   **                            responses.
   ...
   */
  public LoggerFeature(final Logger logger, final Integer size) {
    // ensure inheritance
    this(logger, null, VERBOSITY_DEFAULT, size);
  }
  ...
  /**
   ** Constructs a <code>LoggerFeature</code> which is associated with
   ** the specified {@link Logger}, it's level, message verbosity and maximum
   ...
   **                            Allowed object is {@link Integer}.
   */
  public LoggerFeature(final Logger logger, final Level level, final Verbosity verbosity, final Integer size) {
    // ensure inheritance
    super();
  ...
  private Interceptor interceptor(final FeatureContext context) {
    ...
    final String         category   = PropertyUtility.value(properties, runtime == RuntimeType.SERVER ? LOGGER_CATEGORY_SERVER    : LOGGER_CATEGORY_CLIENT,    PropertyUtility.value(properties, LOGGER_CATEGORY,    this.logger != null ? this.logger.getName() : NAME_DEFAULT));
    ...
    if (runtime == RuntimeType.SERVER) {
      return new Server(this.logger != null ? this.logger : Logger.getLogger(category), this.level != null ? this.level : loggerLevel, this.verbosity != null ? this.verbosity : verbosity, this.size != null ? this.size : entitySize);
    }
    else {
      return new Client(this.logger != null ? this.logger : Logger.getLogger(category), this.level != null ? this.level : loggerLevel, this.verbosity != null ? this.verbosity : verbosity, this.size != null ? this.size : entitySize);
    }
  }
}