...
public interface Cache<K extends Object, V extends Object> extends ConcurrentMap<K, V> {
  ...
  static final String CATEGORY                 = Cache.class.getName();
  static final Logger LOGGER                   = Logger.create(CATEGORY);
  ...
  interface Store<K, V> extends Cache<K, V> {
     ...
     ** An in-memory cache providing thread safety and atomicity guarantees.
     ** <br>
     ** This interface provides an extension to {@link Cache} for use with
     ** skeletal implementations.
     */
    interface Local<K, V> extends Store<K, V> {
      ...
      default V compute(final K key, final BiFunction<? super K, ? super V, ? extends V> function) {
        return compute(key, function, expiry(), /* recordLoad */ true, /* recordFail */ true);
      }
       ...
       ** <b>Note</b>:
       ** <br>
       ** This method differs by accepting parameters indicating whether to
       ** record miss and load metrics based on the success of this operation.
       **
       ** @param  key            the key with which the specified value is to be
       **                        associated.
       **                        <br>
       ...
       ** @param recordLoad      whether load success metrics recorded.
       **                        <br>
       **                        Allowed object is <code>boolean</code>.
       ** @param recordFail      whether load failure metrics recorded.
       **                        <br>
       **                        Allowed object is <code>boolean</code>.
       **
       ** @return                the new value associated with the specified
       ...
       **                        Possible object is <code>V</code>.
       */
      V compute(final K key, final BiFunction<? super K, ? super V, ? extends V> function, final Expiry<? super K, ? super V> expiry, final boolean recordLoad, final boolean recordFail);
    }
    ...
    interface Manual<K, V> extends Store<K, V> {
      ...
      default void bulk(final Set<K> filter, final Map<K, V> result, final Function<? super Iterable<? extends K>, Map<K, V>> function) {
        ...
        try {
          final Map<K, V> loaded = function.apply(filter);
     ...
     ** @param  record           whether to record the hit and miss metrics
     ...
     */
     V getIfPresent(final Object key, final boolean record);
    ...
    @Override
    default V compute(final K key, final BiFunction<? super K, ? super V, ? extends V> function) {
      return compute(key, function, expiry(), /* recordMiss */false, /* recordLoad */true, /* recordFail */true);
    }
     ...
     **                          Possible object is <code>V</code>.
     */
    V compute(final K key, final BiFunction<? super K, ? super V, ? extends V> function, final Expiry<K, V> expiry, final boolean recordMiss, final boolean recordLoad, final boolean recordFail);
  ...
  // ~~~~~ ~~~~~ ~~~~~~~
  /**
   ** A builder of {@link Cache}, {@link Loading}, {@link Async}, and
   ** {@link LoadingAsync} cache instances having a combination of the following
   ** features:
   ** <ul>
   **   <li>automatic loading of entries into the cache, optionally
   **       asynchronously
   **   <li>size-based eviction when a maximum is exceeded based on frequency
   **       and recency
   **   <li>time-based expiration of entries, measured since last access or last
   **       write
   **   <li>asynchronously refresh when the first stale request for an entry
   **       occurs
   **   <li>keys automatically wrapped in {@linkplain WeakReference weak}
   **       references
   ...
   ** </ul>
   ** These features are all optional; caches can be created using all or none
   ** of them. By default, cache instances created by <code>Builder</code> will
   ** not perform any type of eviction.
   ** <p>
   ** Usage example:
   ** <pre>
   **   Loading&lt;Key, Graph&gt; graphs = Cache.Builder.of()
   **   .maximumSize(10.000)
   ...
   ** characteristics to {@link ConcurrentHashMap}. The <code>asMap</code> view
   ** (and its collection views) have <i>weakly consistent iterators</i>.
   ** <br>
   ...
   ** these changes, if any, are reflected in that iterator. These iterators
   ** never throw {@link ConcurrentModificationException}.
   ** <p>
   ** <b>Note</b>:
   ** <br>
   ** bby default, the returned cache uses equality comparisons (the
   ** {@link Object#equals equals} method) to determine equality for keys or
   ** values. However, if {@link #weakKeys} was specified, the cache uses
   ** identity (<code>==</code>) comparisons instead for keys. Likewise, if
   ** {@link #weakValues} or {@link #softValues} was specified, the cache uses
   ** identity comparisons for values.
   ** <p>
   ** Entries are automatically evicted from the cache when any of
   ** {@link #maximumSize(long) maximumSize},
   ** {@link #maximumWeight(long) maximumWeight},
   ** {@link #expireAfter(Expiry) expireAfter},
   ** {@link #expireAfterWrite expireAfterWrite},
   ** {@link #expireAfterAccess expireAfterAccess}, {@link #weakKeys weakKeys},
   ** {@link #weakValues weakValues}, or {@link #softValues softValues} are
   ** requested.
   ** <p>
   ** If {@link #maximumSize(long) maximumSize} or
   ** {@link #maximumWeight(long) maximumWeight} is requested entries may be
   ** evicted on each cache modification.
   ** <p>
   ** If {@link #expireAfter(Expiry) expireAfter},
   ** {@link #expireAfterWrite expireAfterWrite}, or
   ** {@link #expireAfterAccess expireAfterAccess} is requested then entries may
   ** be evicted on each cache modification, on occasional cache accesses, or on
   ** calls to {@link Cache#cleanUp}. A {@link #scheduler(Scheduler)} may be
   ** specified to provide prompt removal of expired entries rather than waiting
   ** until activity triggers the periodic maintenance. Expired entries may be
   ** counted by {@link Cache#estimatedSize()}, but will never be visible to
   ** read or write operations.
   ** <p>
   ** If {@link #weakKeys weakKeys}, {@link #weakValues weakValues}, or
   ** {@link #softValues softValues} are requested, it is possible for a key or
   ** value present in the cache to be reclaimed by the garbage collector.
   ...
   ** {@link Cache#cleanUp}; such entries may be counted in
   ** {@link Cache#estimatedSize()}, but will never be visible to read or write
   ** operations.
   ** <p>
   ...
   ** cache. Only caches built with {@link #maximumSize maximumSize},
   ** {@link #maximumWeight maximumWeight},
   ** {@link #expireAfter(Expiry) expireAfter},
   ** {@link #expireAfterWrite expireAfterWrite},
   ** {@link #expireAfterAccess expireAfterAccess}, {@link #weakKeys weakKeys},
   ** {@link #weakValues weakValues}, or {@link#softValues softValues} perform
   ** periodic maintenance.
   ** <p>
   ** The caches produced by <code>Builder</code> are serializable, and the
   ** deserialized caches retain all the configuration properties of the
   ** original cache. Note that the serialized form does <i>not</i> include
   ** cache contents, but only configuration.
   **
   ** @param  <K>                the most general type of keys this builder
   **                            will be able to create caches for.
   **                            <br>
   **                            This is normally {@link Object} unless it is
   **                            constrained by using a method like
   **                            {@link #removalListener}.
   **                            <br>
   **                            Allowed object is <code>&lt;K&gt;</code>.
   ** @param  <V>                the most general value type this builder will
   **                            be able to create caches for.
   **                            <br>
   **                            This is normally {@link Object} unless it is
   **                            constrained by using a method like
   **                            {@link #removalListener}.
  ...
  static final class Builder<K extends Object, V extends Object> {
    ...
    // Method: bounded
    boolean bounded() {
      return (this.maximumSize       != UNSET)
          || (this.maximumWeight     != UNSET)
          || (this.expireAfterAccess != UNSET)
          || (this.expireAfterWrite  != UNSET)
          || (this.expiry            != null)
          || (this.strengthKey       != null)
    ...
    /**
     ** Sets the initial capacity allocated for the internal data structures.
     ** <br>
    ...
    // Method:   initialCapacity
    /**
     ** Retruns the initial capacity allocated for the internal data structures.
     ** <br>
     ** Providing a large enough estimate at construction time avoids the need
     ** for expensive resizing operations later, but setting this value
     ** unnecessarily high wastes memory.
     ...
     **                          <br>
     **                          Allowed object is <code>&lt;K1&gt;</code>.
     ** @param  <V1>             the value type of the weighter.
     **                          <br>
     **                          Allowed object is <code>&lt;V1&gt;</code>.
    ...
    public <K1 extends K, V1 extends V> Cache<K1, V1> build() {
      requireWeightWithWeighter();
      requireNonLoadingCache();

//      return bounded() ? new BoundedMemory.BoundedLocalManualCache<>(this) : new UnboundedMemory.UnboundedLocalManualCache<>(this);
      return Unbounded.<K1, V1>of(this, false);
    }

    ////////////////////////////////////////////////////////////////////////////
    // Method:   build
    /**
     ** Factory method to create a {@link Loading} <code>Cache</code> which
     ** either returns a {@link CompletableFuture} already loaded value or
     ** currently computing the value for a given key, or atomically computes
     ** the value asynchronously through a supplied mapping function or the
     ** supplied {@link Loader}.
     ** <br>
     ** If the asynchronous computation fails or computes a <code>null</code>
     ** value then the entry will be automatically removed.
     ** <b>Note</b>:
     ** <br>
     ** Multiple threads can concurrently load values for distinct keys.
     ** <p>
     ...
     ** caches.
     ** <p>
     ** This factory method cannot be used with {@link #weakValues()},
     ** {@link #softValues()}, or when {@link #weakKeys()} are combined with
     ...
     */
    @SuppressWarnings({"unchecked", "cast"}) 
    public <K1 extends K, V1 extends V> Cache<K1, V1> build(final Loader.Default<? super K1, V1> loader) {
      requireWeightWithWeighter();
      Builder<K, V> self = (Builder<K, V>)this;
      bounded();
//      return bounded() || this.refreshAfterWrite ? new BoundedLocalCache.BoundedLocalLoadingCache<>(self, loader) : new UnboundedLocalCache.UnboundedLocalLoadingCache<>(self, loader);
      return Unbounded.<K1, V1>of(this, false);
    }

    ////////////////////////////////////////////////////////////////////////////
    // Method: requireWeightWithWeighter
    void requireWeightWithWeighter() {
      if (this.weighter == null) {
        requireState(this.maximumWeight == UNSET, "maximumWeight requires weighter");
      }
      else if (strictParsing) {
        requireState(this.maximumWeight != UNSET, "weighter requires maximumWeight");
      }
      else if (this.maximumWeight == UNSET) {
        LOGGER.warn("Ignoring weighter specified without maximumWeight");
      }
    }

    ////////////////////////////////////////////////////////////////////////////
    // Method: requireNonLoadingCache
    void requireNonLoadingCache() {
      requireState(this.refreshAfterWrite == UNSET, "refreshAfterWrite requires a LoadingCache");
    }
   ...
   ** @param  function           the function to compute a value.
   **                            <br>
   **                            Allowed object is {@link Function} with
   **                            type <code>K</code> key and <code>V</code> as
   **                            the value.
   **
   ** @return                    the current (existing or computed) value
   **                            associated with the specified key, or
