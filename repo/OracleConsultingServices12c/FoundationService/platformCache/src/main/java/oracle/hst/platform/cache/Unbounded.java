...
final class Unbounded<K, V> implements Cache.Store.Manual<K, V> {
   ...
   */
  private Unbounded(final Builder<? super K, ? super V> builder, final boolean asynchron) {
    // ensure inheritance
    super();
  ...
  public final boolean remove(final Object key, final Object value) {
    ...
    K   casted = (K)Objects.requireNonNull(key);
  ...
  public final V replace(final K key, final V value) {
    ...
    if ((old[0] != null) && (old[0] != value)) {
      notifyRemove(key, Objects.requireNonNull(value), Cache.Event.REPLACED);
    }
    return old[0];
  }
  ...
  public final boolean replace(final K key, final V origin, final V value) {
    ...
    final V[] old = (V[])new Object[1];
    this.store.computeIfPresent(key, (k, v) -> {
      if (v.equals(Objects.requireNonNull(origin))) {
        old[0] = v;
        discardRefresh(k);
        return value;
      }
    ...
    boolean replaced = (old[0] != null);
    if (replaced && (old[0] != value)) {
      notifyRemove(key, old[0], Cache.Event.REPLACED);
    }
    return replaced;
  }
  ...
  public final V compute(final K key, final BiFunction<? super K, ? super V, ? extends V> function, final Expiry<K, V> expiry, final boolean recordMiss, final boolean recordLoad, final boolean recordFailure) {
    return remap(key, metricAware(Objects.requireNonNull(function), recordMiss, recordLoad, recordFailure));
  }
  ...
  public final V computeIfAbsent(K key, Function<? super K, ? extends V> function, boolean recordMetric, boolean recordLoad) {
    ...
    , k -> {
        ...
        missed[0] = true;
        return recordMetric ? this.metricAware(Objects.requireNonNull(function), recordLoad).apply(key) : function.apply(key);
      }
    );
