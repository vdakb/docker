...
package oracle.iam.identity.dbs.service.reconciliation.openfire;
 ...
 ** Identity Manager to reconcile metadata information from a
...
public class LookupReconciliation extends oracle.iam.identity.connector.service.LookupReconciliation {
  ...
  // Method:   Ctor
  /**
   ** Constructs an empty <code>LookupReconciliation</code> job task that allows
   ** use as a JavaBean.
   ** <br>
  ...
  protected TargetResource populateResource()
    ...
    try {
      ...
      if(!StringUtility.isBlank(resourceName())){
        resource = DatabaseResource.build(this, resourceName());
      }
      else {
        final ApplicationInstance instance = populateInstance(applicationInstance());
        resource = DatabaseResource.build(this, instance.getItResourceKey());
      }
    }
    finally {
      timerStop(method);
   ...
   ** te special features of the Service Provider endpoint from an
   ** {@link InputSource}.
   **
   ...
   */
  @Override
  protected TargetFeature populateFeature(final InputSource source)
    throws TaskException {

    return DatabaseFeature.build(this, source);
  }
}
