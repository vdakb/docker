...
final class DirectoryFeatureParser extends FeatureParser {
  ...
  public static final String ELEMENT_SCOPE_LINK          = "scope-link";
  ...
  enum Grammar {
    ...
    , MULTIVALUED        (new Grammar[]{ROLE, GROUP, SCOPE, ACCOUNT},              ELEMENT_MULTIVALUED)
    , GROUPLINK          (new Grammar[]{ACCOUNT},                                  ELEMENT_GROUP_LINK)
    , ROLELINK           (new Grammar[]{ACCOUNT},                                  ELEMENT_ROLE_LINK)
    , SCOPELINK          (new Grammar[]{ACCOUNT},                                  ELEMENT_SCOPE_LINK)
  ...
  public void startElement(final String uri, final String localName, final String qualifiedName, final Attributes attributes)
    ...
    switch (state) {
      ...
      case SERVER              : processServer(attributes);
                                 break;
      case ROLE                : this.feature.put(Directory.OIM.Feature.ROLE_CLASS,                    attributes.getValue(ATTRIBUTE_OBJECT_CLASS));
                                 this.feature.put(Directory.OIM.Feature.ROLE_PREFIX,                   attributes.getValue(ATTRIBUTE_OBJECT_PREFIX));
                                 this.feature.put(Directory.OIM.Feature.ROLE_MEMBER_PREFIX,            attributes.getValue(ATTRIBUTE_OBJECT_MEMBER));
                                 break;
      case GROUP               : this.feature.put(Directory.OIM.Feature.GROUP_CLASS,                   attributes.getValue(ATTRIBUTE_OBJECT_CLASS));
                                 ...
                                 break;
      case ORGANIZATION        : this.feature.put(Directory.OIM.Feature.ORGANIZATION_CLASS,            attributes.getValue(ATTRIBUTE_OBJECT_CLASS));
                                 this.feature.put(Directory.OIM.Feature.ORGANIZATION_PREFIX,           attributes.getValue(ATTRIBUTE_OBJECT_PREFIX));
                                 break;    
      case ORGANIZATIONAl_UNIT : this.feature.put(Directory.OIM.Feature.ORGANIZATIONALUNIT_CLASS,      attributes.getValue(ATTRIBUTE_OBJECT_CLASS));
                                 this.feature.put(Directory.OIM.Feature.ORGANIZATIONALUNIT_PREFIX,     attributes.getValue(ATTRIBUTE_OBJECT_PREFIX));
                                 break;    
      case SCOPE               : this.feature.put(Directory.OIM.Feature.SCOPE_CLASS,                   attributes.getValue(ATTRIBUTE_OBJECT_CLASS));
                                 this.feature.put(Directory.OIM.Feature.SCOPE_PREFIX,                  attributes.getValue(ATTRIBUTE_OBJECT_PREFIX));
                                 this.feature.put(Directory.OIM.Feature.SCOPE_MEMBER_PREFIX,           attributes.getValue(ATTRIBUTE_OBJECT_MEMBER));
                                 break;
     ...
     case GROUPLINK            : switch (this.state.peek()) {
                                   case ACCOUNT             : this.feature.put(Directory.OIM.Feature.ACCOUNT_GROUP_PREFIX, attributes.getValue(ATTRIBUTE_ATTRIBUTE));
                                                              break;
                                 }
                                 break;
      case ROLELINK            : switch (this.state.peek()) {
                                   case ACCOUNT             : this.feature.put(Directory.OIM.Feature.ACCOUNT_ROLE_PREFIX,  attributes.getValue(ATTRIBUTE_ATTRIBUTE));
                                                              break;
                                 }
                                 break;
      case SCOPELINK            : switch (this.state.peek()) {
                                   case ACCOUNT             : this.feature.put(Directory.OIM.Feature.ACCOUNT_SCOPE_PREFIX, attributes.getValue(ATTRIBUTE_ATTRIBUTE));
                                                              break;
                                 }
                                 break;
      default                  : break;
  ...
  public void characters(final char[] buffer, final int start, final int length)
    ...
    // where we are?
    switch (this.state.peek()) {
      case DISTINGUISHED_NAMES : List<String> list = feature.distinguishedName();
                                 if (list == null) {
