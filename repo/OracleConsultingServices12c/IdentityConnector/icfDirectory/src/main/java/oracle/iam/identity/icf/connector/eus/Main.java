/*
    ...
    Subsystem   :   Enterprise User Connector
...
package oracle.iam.identity.icf.connector.eus;
...
public class Main extends DirectoryService {
  ...
  static final String CATEGORY = "JCS.CONNECTOR.EUS";
  ...
  public Schema schema() {
    ...
    Schema schema = null;
    try {
      schema = DirectorySchema.build(this.endpoint).schema(getClass());
    }
...
//    SyncStrategy strategy = chooseSyncStrategy(objectClass);
   ...
   **                            converted from the {@link DirectoryFilter}
   **                            passed to
   ...
   */
  @Override
  public Uid create(final ObjectClass objectClass, final Set<Attribute> attribute, final OperationOptions operation) {
   ...
   */
  @Override
  public Uid update(final ObjectClass objectClass, final Uid uid, final Set<org.identityconnectors.framework.common.objects.Attribute> replace, final OperationOptions option)
    throws ConnectorException {
  ...
  public Uid removeAttributeValues(final ObjectClass type, final Uid uid, final Set<Attribute> remove, final OperationOptions option)
    ...
    try {
      // dispatch to the appropriate method
      DirectoryModify.build(this.endpoint, type).assign(uid, remove);
    }
    catch (SystemException e) {
      // report the exception in the log spooled for further investigation
      fatal(method, e);
