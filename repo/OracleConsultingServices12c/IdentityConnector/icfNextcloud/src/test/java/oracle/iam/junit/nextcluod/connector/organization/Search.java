...
import oracle.iam.identity.icf.connector.nextcloud.schema.Organization;
...
public class Search extends TestFixture {
  ...
  public void paginated() {
    ...
    int count = 500;
    try {
      final List<Organization> resource = CONTEXT.searchOrganization(index, count, null);
      ...
      for (int i = 0; i < resource.size(); i++) {
        ...
        assertNotNull(resource.get(i).name());
        System.out.println(resource.get(i).name());
        index += count;
      }
    }
