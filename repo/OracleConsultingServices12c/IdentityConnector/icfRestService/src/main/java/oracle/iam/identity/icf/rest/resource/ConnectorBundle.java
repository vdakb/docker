...
public class ConnectorBundle extends ListResourceBundle  {
  ...
  , { Connector.Endpoint.SECURE_HINT,                 "Secure Socket (SSL/TLS) is a standard security technology for establishing an encrypted link between a server and a client - typically a web server (website) and a browser, or a mail server and a mail client (e.g., Outlook).\nSSL/TLS allows sensitive information such as credit card numbers, social security numbers, and login credentials to be transmitted securely." }
  ...
  , { Connector.Endpoint.AUTHENTICATION_SCHEME_HINT,  "An authentication scheme is a method or recipe to pass credentials from a principal to an authenticating system.\nThese schemes vary in their security and complexity with some only handling authentication while others can also handle authorization." }
  , { Connector.Endpoint.AUTHORIZATION_SERVER_LABEL,  "Authorization Server" }
  , { Connector.Endpoint.AUTHORIZATION_SERVER_HINT,   "An entity responsible for authenticating user's identity and gives an authorization token. This token is accepted by resource server and validate the identity." }
