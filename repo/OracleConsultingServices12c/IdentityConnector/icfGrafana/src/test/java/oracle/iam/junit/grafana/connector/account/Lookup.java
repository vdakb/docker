...
public class Lookup extends TestFixture {
  ...
  protected void lookup(final String identity) {
    try {
      ...
      assertEquals(User.ID,    beneficiary.id(),    origin.id());
      assertEquals(User.LOGIN, beneficiary.login(),  origin.login());
      assertEquals(User.EMAIL, beneficiary.email(), origin.email());
    }
