...
// ~~~~~ ~~~~~~~~~~~~~~~
/**
 ** An interface to convert Grafana REST error responses object in transferrable
...
public class ExceptionParser {
    ...
    try {
      ...
      while (!stop && parser.nextToken() != JsonToken.END_OBJECT) {
        ...
        if (JsonToken.NOT_AVAILABLE != parser.nextToken()) {
          ...
          if (field != null) {
            switch (field) {
              // Grafana use sometimes "message" as the property name of the
              // detailed, human readable message in a REST error response
              case "message" : error.detail(parser.getValueAsString());
                               break;
            }
          }
        }
  ...
  public static SystemException from(final Error error) {
    ...
    switch (error.status()) {
      ...
      case 412 : return ServiceException.conflict(error.detail());
