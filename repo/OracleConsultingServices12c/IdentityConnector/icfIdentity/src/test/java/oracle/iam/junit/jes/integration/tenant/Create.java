...
// ~~~~~ ~~~~~~
/**
 ** The test case delete operation of tenants at the target system
...
public class Create extends TestFixture {
  ...
  public void createUnknown() {
    try {
      ...
      final Map<String, Object>  data = data("igd/create-tenant-unknown.conf");

      final Descriptor descriptor = DescriptorFactory.configure(Descriptor.buildProvisioning(CONSOLE), Tenant.PROVISIONING);
  ...
  public void createXellerate() {
    assertEquals(createOrganization("igd/create-tenant-xellerate.conf"), Tenant.LEGACY.value);
  }
  ...
  public void createTop() {
    assertEquals(createOrganization("igd/create-tenant-top.conf"), Tenant.TOP.value);
  }
  ...
  /**
   ** Performs all actions to create an organization in the target system by
   ...
   **
   ** @return                    the system identifier of the organization
   **                            created.
   **                            <br>
   **                            Possible object is {@link UID}.
   */
  private Uid createOrganization(final String path) {
    try {
      // the map simulates how Identity Governance provides the data after the
      // adapter task createProcessData applied on the process data
      ...
      final Set<Attribute> dataSet = DescriptorTransformer.build(descriptor, data);
      assertNotNull(dataSet);
      return FACADE.create(ServiceClass.TENANT, dataSet, null);
    }
    catch (ConnectorException e) {
      failed(e);
    }
