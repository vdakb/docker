...
public class Search extends TestReconciliation {
  ...
  public void testSearch() {
    try {
      final Descriptor              mapping   = DescriptorFactory.configure(Descriptor.buildProvisioning(CONSOLE), TestFixture.Account.RECONCILIATION);
      ...
      facade(endpoint()).search(ObjectClass.ACCOUNT, null, new Handler("testSearch", mapping), option.build());
    }
  ...
  public void testFuture() {
    try {
      ...
      facade(endpoint()).search(ObjectClass.ACCOUNT, null, new Handler("testFuture", mapping), option.build());
    }
    catch (ConnectorException e) {
      failed(e);
    }
