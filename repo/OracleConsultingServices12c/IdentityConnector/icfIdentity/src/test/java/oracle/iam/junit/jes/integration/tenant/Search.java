...
// ~~~~~ ~~~~~~
/**
 ** The test case for search operation on tenants (aka Organization) at the
...
public class Search extends TestReconciliation {
  ...
  public void testSearch() {
    try {
      final Descriptor              mapping   = DescriptorFactory.configure(Descriptor.buildProvisioning(CONSOLE), TestFixture.Tenant.RECONCILIATION);
      ...
      facade(endpoint()).search(ServiceClass.TENANT, null, new Handler("testSearch", mapping), option.build());
    }
  ...
  public void testFuture() {
    try {
      ...
      facade(endpoint()).search(ServiceClass.TENANT, null, new Handler("testFuture", mapping), option.build());
    }
    catch (ConnectorException e) {
      failed(e);
    }
