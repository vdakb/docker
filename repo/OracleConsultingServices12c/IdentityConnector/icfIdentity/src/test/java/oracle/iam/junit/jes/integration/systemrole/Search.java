...
// ~~~~~ ~~~~~~
/**
 ** The test case for search operation on system roles (aka Role)s at the
...
public class Search extends TestReconciliation {
  ...
  public void testSearch() {
    try {
      final Descriptor              mapping   = DescriptorFactory.configure(Descriptor.buildProvisioning(CONSOLE), TestFixture.SystemRole.RECONCILIATION);
      ...
      facade(endpoint()).search(ObjectClass.GROUP, null, new Handler("testSearch", mapping, true), option.build());
    }
  ...
  public void testFuture() {
    try {
      ...
      facade(endpoint()).search(ObjectClass.GROUP, null, new Handler("testFuture", mapping, true), option.build());
    }
    catch (ConnectorException e) {
      failed(e);
    }
