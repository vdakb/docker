...
import oracle.iam.identity.icf.rest.utility.ParserFactory;
...
class Parser {
   ...
   **                            Possible object is {@link Option}.
   */
  static Option option(final Option option) {
   ...
   **
   ** @throws ServiceException  if the path string could not be parsed.
   */
  static Path path(final String expression)
    ...
    while ((token = readPathToken(reader)) != null) {
      if (token.isEmpty()) {
        ...
        if (path.empty() || path.element(path.size() - 1).filter() == null)
          throw ServiceException.invalidPath(ServiceBundle.string(ServiceError.PATH_EXPECT_ATTRIBUTE_NAME, reader.mark));
      }
      else {
        ...
        try {
          ...
          throw ServiceException.invalidPath(ServiceBundle.string(ServiceError.PATH_INVALID_FILTER, e.getMessage()));
        }
        catch(Exception e) {
          throw ServiceException.invalidPath(ServiceBundle.string(ServiceError.PATH_INVALID_ATTRIBUTE_NAME, reader.mark, e.getMessage()));
        }
      }
    }
    return path;
   ...
   **                            Possible object is {@link Path}.
   **
   ** @throws ServiceException   if the filter string could not be parsed.
   */
  static Filter filter(final String expression)
    throws ServiceException {
    ...
    while (c > 0) {
      if (c == '.') {
        if (reader.pos >= reader.string.length()) {
          ...
          throw ServiceException.invalidPath(ServiceBundle.string(ServiceError.PATH_UNEXPECTED_EOF_STRING));
        }
        // terminating period
      ...
      if (c == '-' || c == '_' || c == '$' || Character.isLetterOrDigit(c) || option.is(c)) {
        b.append((char)c);
      }
      else {
        throw ServiceException.invalidPath(ServiceBundle.string(ServiceError.PATH_UNEXPECTED_CHARACTER, (char)c, reader.pos - 1, reader.mark));
      }
      c = reader.read();
    }
  ...
  private static Filter readFilter(final StringReader reader, final boolean valueFilter)
    ...
    while ((token = readFilterToken(reader, valueFilter)) != null) {
      if (token.equals("(") && expectNew(previous)) {
        ...
        if (nextToken == null) {
          throw ServiceException.invalidFilter(ServiceBundle.string(ServiceError.PATH_UNEXPECTED_EOF_FILTER));
        }
        if (!nextToken.equals("(")) {
          throw ServiceException.invalidFilter(ServiceBundle.string(ServiceError.PATH_EXPECT_PARENTHESIS, reader.mark));
        }
        precedence.push(token);
      }
        ...
        if (operator == null) {
          throw ServiceException.invalidFilter(ServiceBundle.string(ServiceError.PATH_INVALID_PARENTHESIS, reader.mark));
        }
        ...
        try {
          ...
          throw ServiceException.invalidFilter(ServiceBundle.string(ServiceError.PATH_INVALID_ATTRIBUTE_PATH, reader.mark, e.getMessage()));
        }

        if (filterAttribute.empty())
          throw ServiceException.invalidFilter(ServiceBundle.string(ServiceError.PATH_EXPECT_ATTRIBUTE_PATH, reader.mark));
        ...
        if (op.equalsIgnoreCase(Filter.Type.PR.value())) {
          ...
          try {
            ...
            reader.mark(0);
            final ParserFactory factory = (ParserFactory)Support.objectReader().getFactory();
            final JsonParser  parser  = factory.create(reader);
            ...
            throw ServiceException.invalidFilter(ServiceBundle.string(ServiceError.PATH_INVALID_COMPARISON_VALUE, reader.mark, e.getMessage()));
          }
          ...
          if (op.equalsIgnoreCase(Filter.Type.EQ.value())) {
            ...
            throw ServiceException.invalidFilter(ServiceBundle.string(ServiceError.PATH_UNRECOGNOIZED_OPERATOR, op, reader.mark));
          }
        }
      }
      else {
        throw ServiceException.invalidFilter(ServiceBundle.string(ServiceError.PATH_UNEXPECTED_CHARACTER, token, reader.pos - 1, reader.mark));
      }
      previous = token;
    }
  ...
  private static String readFilterToken(final StringReader reader, final boolean valueFilter)
    ...
    while (c > 0) {
      ...
      if (c == '-' || c == '_' || c == '.' || c == ':' || c == '$' || Character.isLetterOrDigit(c) || option.is(c)) {
        ...
        throw ServiceException.invalidFilter(ServiceBundle.string(ServiceError.PATH_UNEXPECTED_CHARACTER, (char)c, reader.pos - 1, reader.mark));
      }
      c = reader.read();
    }
    return (b.length() > 0)  ? b.toString() : null;
   ...
   ** @param  eof                whether the end of the filter string was
   **                            reached.
   **                            <br>
   **                            Allowed object is <code>boolean</code>.
   **
   ...
   ** @throws ServiceException   if the filter string could not be parsed.
   */
  private static String closeGrouping(final Stack<String> operators, final Stack<Filter> output, final boolean eof)
    throws ServiceException {
