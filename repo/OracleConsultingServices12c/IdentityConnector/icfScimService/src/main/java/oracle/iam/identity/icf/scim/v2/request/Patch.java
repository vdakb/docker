 ...
 ** to SCIM 2 interface.
 ** <br>
 ** The core schema for <code>PatchResource</code> is identified using the URI:
...
public class Patch<T extends Resource> extends Return<Patch> {
  ...
  public static class Request extends    oracle.iam.identity.icf.scim.schema.Entity
    ...
    @Attribute(description="Patch Operations", required=true, multiValueClass=Operation.class)
     ...
     */
    @JsonCreator
    public Request(final @JsonProperty(value=OPERATIONS, required=true) List<Operation> operation) {
      // ensure inheritance
      super();
  ...
  public T match(final String version) {
  ...
  // Method:   invoke
  /**
   ** Invoke the SCIM 2 modify request using PATCH.
   **
  ...
  public <R> R invoke(final Class<R> clazz)
    ...
    try {
      response = buildRequest().method("PATCH", Entity.entity(this.request, typeContent()));
