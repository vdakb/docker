...
public abstract class CompositeFilter implements Filter {
   ...
   **                            Allowed object is {@link Filter}.
   */
  protected CompositeFilter(final Type type, final Filter lhs, final Filter rhs) {
    // ensure inheritance
   ...
   */
  public CompositeFilter(final Type type, final Collection<Filter> filter) {
    // ensure inheritance
    super();
   ...
   ** <p>
   ** Two <code>CompositeFilter</code> filter are considered equal if and only
   ** if they represent the same properties. As a consequence, two given
   ** <code>CompositeFilter</code> filter may be different even though they
