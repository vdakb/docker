 ...
 ** manipulating SCIM resources using the <code>GenericScimResource</code> class.
 **
 ** @author  dieter.steding@oracle.com
 ** @version 1.0.0.0
 ** @since   1.0.0.0
...
public class Path implements Iterable<Path.Element> {
  ...
  protected final List<Element> element;
  ...
  public static final class Element {
     ...
     ** <p>
     ** Two <code>Path</code>s filter are considered equal if and only if
     ** they represent the same properties. As a consequence, two given
     ** <code>Path</code>s filter may be different even though they
   ...
   */
  protected Path(final String namespace, final List<Element> element) {
    // ensure inheritance
    super();
  ...
  // only for lnow as long SCIM V1 PatchOperation relies on path
   ...
   ** the contract specified in {@link Object#equals(Object)}.
   ** <p>
   ** Two <code>Path</code>es are considered equal if and only if they represent
   ** the same properties. As a consequence, two given <code>Path</code>es may
