...
public class ListResponseHandler<T> implements ResponseHandler<T> {
   ...
   */
  @Override
  public final void extension(final String namespace, final ObjectNode extension) {
  ...
  public ListResponse<T> build() {
    ...
    // Especially Pivotal Cloud Foundry (PCF) replies on a search request with
    // values mirrored from the request itselfif the query parameter exceeds
    // the existing resource metrics, e.g.
    ...
    return new ListResponse<T>(
      this.total           == null ? this.resource.size() : this.total
    , this.start           == null ? 1 : this.start
    , this.resource.size() ==    0 ? 0 : this.items == null ? this.resource.size() : this.items
    , this.resource);
  }
}