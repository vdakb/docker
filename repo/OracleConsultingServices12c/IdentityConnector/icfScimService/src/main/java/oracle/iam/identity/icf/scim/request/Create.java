...
public class Create<T extends Resource> extends Return<Create> {
  ...
  // Method:   Ctor
  /**
   ** Constructs a new SCIM create request that will POST the given resource to
   ** the given web target.
   ** <br>
   ** This constructor is private to prevent other classes to use
   ** "new Create()" and enforces use of the public method below.
   **
   ** @param  target             the {@link WebTarget} to send the request.
   **                            <br>
   **                            Allowed object is {@link WebTarget}.
   ** @param  resource           the SCIM 1 resource to create by POST.
   **                            <br>
  ...
  // Method:   build
  /**
   ** Factory method to create a new SCIM create request that will POST the
   ** given resource to the given web target.
   **
   ...
   **                            <br>
   **                            Possible object is <code>Create</code> for type
   **                            <code>T</code>.
   */
  ...
  public <R> R invoke(final Class<R> clazz)
    ...
    try {
      response = buildRequest().post(Entity.entity(MapperFactory.instance.writeValueAsString(this.resource), typeContent()));
