...
public class Lookup extends TestFixture {
   ...
   */
  @Test
  public void pidadmin() {
    lookup(RealmGroup.PID);
  }
   ...
   */
  @Test
  public void uidadmin() {
    lookup(RealmGroup.UID);
  }
   ...
   ** Test lookup request leveraging server context.
   */
  protected void lookup(final Pair<String, File> identity) {
    final Group origin = group(identity.value);
    assertNotNull(origin);
    assertEquals(Service.NAME, origin.name(), identity.tag);
    try {
      Group target = CONTEXT.resolveGroup(identity.tag);
      target = CONTEXT.lookupGroup(target.id());
      assertNotNull(target);
      assertEquals(Service.NAME, target.name(), identity.tag);
    }
