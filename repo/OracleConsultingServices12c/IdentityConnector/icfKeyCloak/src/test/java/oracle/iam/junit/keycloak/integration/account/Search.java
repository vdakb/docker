...
import oracle.iam.junit.keycloak.integration.TestReconciliation;
...
public class Search extends TestReconciliation {
  ...
  public void searchAll() {
    try {
      final Descriptor              mapping   = DescriptorFactory.configure(Descriptor.buildProvisioning(CONSOLE), TestFixture.User.RECONCILIATION);
      final OperationOptionsBuilder option    = searchControl(CollectionUtility.union(mapping.source(), mapping.referenceSource()));

      facade(endpoint()).search(ObjectClass.ACCOUNT, null, new Handler("searchAll", mapping), option.build());
    }
  ...
  public void searchStartsWith() {
    try {
      final Descriptor              mapping   = DescriptorFactory.configure(Descriptor.buildProvisioning(CONSOLE), TestFixture.User.RECONCILIATION);
      final OperationOptionsBuilder option    = searchControl(CollectionUtility.union(mapping.source(), mapping.referenceSource()));
      final Filter                  filter    = FilterBuilder.startsWith(new Name("an"));

      facade(endpoint()).search(ObjectClass.ACCOUNT, filter, new Handler("searchStartsWith", mapping), option.build());
    }
    catch (ConnectorException e) {
      failed(e);
    }
