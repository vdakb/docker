...
package oracle.iam.junit.keycloak.integration.role;
...
import oracle.iam.identity.icf.connector.keycloak.schema.Keycloak;
...
// ~~~~~ ~~~~~~
/**
 ** The test case for search operation on global roles (aka AdminRole)s at the
...
public class Search extends TestReconciliation {
  ...
  public void searchAll() {
    try {
      final Descriptor              mapping   = DescriptorFactory.configure(Descriptor.buildProvisioning(CONSOLE), TestFixture.Role.RECONCILIATION);
      final Set<String>             returning = CollectionUtility.union(mapping.source(), mapping.referenceSource());
      final OperationOptionsBuilder option    = searchControl(returning);

      facade(endpoint()).search(Keycloak.ROLE.clazz, null, new Handler(mapping), option.build());
    }
