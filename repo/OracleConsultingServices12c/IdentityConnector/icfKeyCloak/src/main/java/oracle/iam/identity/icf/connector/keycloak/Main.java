 ...
 ** {@link ServiceConnector} for a Red Hat Keycloak server.
 ** <p>
 ...
 ** @since   1.0.0.0
 */
@ConnectorClass(configurationClass=ServiceConfiguration.class, displayNameKey="name", messageCatalogPaths="oracle.iam.identity.icf.connector.keycloak.Main.properties")
public class Main extends    ServiceConnector
  ...
  private static final String CATEGORY = "JCS.CONNECTOR.RKC";
  ...
  /**
   ** Place holder for the {@link Context} passed into the callback
   ** {@link ServiceClient#build(ServiceEndpoint)}.
   */
  ...
  // Method:   Ctor
  /**
   ** Constructs a default <code>Main</code> Red Hat Keycloak Connector that
  ...
  public final void init(final Configuration configuration)
    ...
    try {
      this.context = Context.build(ServiceClient.build(this.config.endpoint(), true));
    }
    catch (SystemException e) {
      // report the exception in the log spooled for further investigation
      fatal(method, e);
      // send back the exception occurred
      propagate(e);
    }
    finally {
      exiting(method);
  ...
  public void executeQuery(final ObjectClass type, final String criteria, final ResultsHandler handler, final OperationOptions option) {
    ...
    try {
      if(ObjectClass.ACCOUNT.equals(type)) {
        ...
        searchGroup(control, criteria, handler);
      }
      else if(Keycloak.ROLE.clazz.equals(type)) {
        searchRole(control, criteria, handler);
      }
      else if(Keycloak.CLIENTROLE.clazz.equals(type)) {
        searchClientRole(control, criteria, handler);
      }
      else {
  ...
  public Uid create(final ObjectClass type, final Set<Attribute> attribute, final OperationOptions option)
    ...
    if (name == null) {
      name = new Name(SchemaUtility.notNullStringValue(accessor.find(Service.USERNAME)));
    }
    // prevent bogus state
    if (name == null) {
      exiting(method);
      propagate(SystemError.NAME_IDENTIFIER_REQUIRED, Service.USERNAME);
    }
  ...
  public Uid update(final ObjectClass type, final Uid uid, final Set<Attribute> replace, final OperationOptions option)
    ...
    try {
      // dispatch to the appropriate method
      if (ObjectClass.ACCOUNT.equals(type)) {
        result = modifyUser(identifier, replace);
      }
      else {
        unsupportedType(type, method);
      }
      ...
      if (ObjectClass.ACCOUNT.equals(type)) {
        for (Attribute cursor : addition) {
          ...
          for (int i = 0; i < value.size(); i++) {
            final EmbeddedObject subject = (EmbeddedObject)value.get(i);
            final ObjectClass    target  = subject.getObjectClass();

            if (ObjectClass.GROUP.equals(target)) {
              assignGroup(identifier, subject.getAttributes());
            }
            else if (Keycloak.ROLE.clazz.equals(target)) {
              assignRole(identifier, subject.getAttributes());
            }
            else if (Keycloak.CLIENTROLE.clazz.equals(target)) {
              assignClientRole(identifier, subject.getAttributes());
            }
            else {
              unsupportedType(target, method);
            }
          }
        }
      }
        ...
        for (Attribute cursor : remove) {
          ...
          for (int i = 0; i < value.size(); i++) {
            ...
            if (ObjectClass.GROUP.equals(target)) {
              ...
              revokeClientRole(identifier, subject.getAttributes());
            }
            else {
              unsupportedType(target, method);
            }
  ...
  protected void searchUser(final OperationContext control, final String criteria, final ResultsHandler handler)
    ...
    // 1-based; Keycloak is an exception here and expects the index to be
    // 0-based
    int index = control.start - 1;
    try {
      final long total = this.context.countUser(criteria);
      do {
  ...
  protected String createUser(final Set<Attribute> attribute)
    ...
    try {
      ...
      // execute operation
      final User entity = this.context.createUser(Service.transformUser(attribute));
      final AttributesAccessor acc = new AttributesAccessor(attribute);
      final GuardedString      pwd = acc.getPassword();
      if (pwd != null) {
        final Credential credential = this.context.resetPassword(entity.id(), CredentialAccessor.string(pwd), false);
//        this.context.labelPassword(entity.id(), credential.id(), false);
      }
      return entity.id();
    }
    finally {
   ...
   ** @throws SystemException    if the operation fails.
   */
  protected Uid modifyUser(final String identifier, final Set<Attribute> attribute)
    throws SystemException {
    ...
    try {
      ...
      User resource = null;
      if (attribute.size() > 0) {
        User user = this.context.lookupUser(identifier, false);
        resource = this.context.modifyUser(identifier, Service.transformUser(user, attribute));
      }
      // take care about the password
      if (password != null)
        this.context.resetPassword(identifier, CredentialAccessor.string(GuardedString.class.cast(password)), false);

      return new Uid(resource == null || resource.id() == null ? identifier : resource.id());
    }
    finally {
      exiting(method);
    }
  ...
  // Method:   assignRole
  /**
   ** Assigns a <code>User</code> to a realm role in Keycloak.
   **
   ...
   ** @throws SystemException    if the operation fails.
   */
  protected final void assignRole(final String beneficiary, final Set<Attribute> attribute)
    throws SystemException {

    final AttributesAccessor accessor = new AttributesAccessor(attribute);
    // prevent bogus input
    final String id = accessor.findString(Uid.NAME);
    if (StringUtility.empty(id))
      AbstractConnector.identifierRequired();

    this.context.assignRole(beneficiary, this.context.lookupRole(id));
  }

  //////////////////////////////////////////////////////////////////////////////
  // Method:   revokeRole
  /**
   ** Revokes a <code>User</code> from a realm role in Keycloak.
   **
   ...
   ** @throws SystemException    if the operation fails.
   */
  protected final void revokeRole(final String beneficiary, final Set<Attribute> attribute)
    throws SystemException {

    final AttributesAccessor accessor = new AttributesAccessor(attribute);
    this.context.revokeRole(beneficiary, this.context.lookupRole(accessor.findString(Uid.NAME)));
  }
   ...
   ** @throws SystemException    if the operation fails.
   */
  protected void searchClientRole(final OperationContext control, final String criteria, final ResultsHandler handler)
          throws SystemException {

    final String method = "searchClientRole";
    entering(method);
    ...
    try {
      do {
        final List<Client> response = this.context.searchClientRole(index, control.limit, criteria);
        if (response.size() == 0)
          break;

        for (Client cursor : response) {
          for (Role role : cursor.role()) {
            final ConnectorObject payload = transform(cursor, role);
            if (!handler.handle(payload))
              break;
          }
        }
   ...
   ** @throws SystemException    if the operation fails.
   */
  protected final void assignClientRole(final String beneficiary, final Set<Attribute> attribute)
          throws SystemException {
    ...
    this.context.assignClientRole(beneficiary, ids[0], this.context.lookupRole(ids[1]));
  }
   ...
   ** @throws SystemException    if the operation fails.
   */
  protected final void revokeClientRole(final String beneficiary, final Set<Attribute> attribute)
          throws SystemException {
    ...
    this.context.revokeClientRole(beneficiary, ids[0], this.context.lookupRole(ids[1]));
  }
   ...
   ** @throws SystemException    if the operation fails.
   */
  protected void searchGroup(final OperationContext control, final String criteria, final ResultsHandler handler)
    throws SystemException {
    ...
    try {
      do {
        final List<Group> response = this.context.searchGroup(index, control.limit, criteria);
        if (response.size() == 0)
          break;

        for (Group cursor : response) {
          final ConnectorObject payload = transform(cursor);
          if (!handler.handle(payload))
            break;
        }
  ...
  // Method:   assignGroup
  /**
   ** Assigns a <code>User</code> to a realm group in Keycloak.
   **
   ** @param  beneficiary        the identifier of a <code>User</code> account.
   **                            <br>
   **                            <b>Note</b>:
   ...
   ** @throws SystemException    if the operation fails.
   */
  protected final void assignGroup(final String beneficiary, final Set<Attribute> attribute)
    throws SystemException {
    ...
    this.context.assignGroup(beneficiary, subject);
  }

  //////////////////////////////////////////////////////////////////////////////
  // Method:   revokeGroup
  /**
   ** Revokes a <code>User</code> from a realm group in Keycloak.
   **
   ** @param  beneficiary        the identifier of a <code>User</code> identity.
   **                            <br>
   **                            <b>Note</b>:
   ...
   ** @throws SystemException    if the operation fails.
   */
  protected final void revokeGroup(final String beneficiary, final Set<Attribute> attribute)
    throws SystemException {
    ...
    this.context.revokeGroup(beneficiary, subject);
  }
  ...
  private ConnectorObject transform(final User user) {
    ...
    // developer
    builder.setObjectClass(ObjectClass.ACCOUNT).setUid(user.id()).setName(user.username());
    try {
   ...
   ** to the {@link ConnectorObject} transfer object.
   **
   ** @param  role               the specified REST {@link Role} to
   **                            transform.
   **                            <br>
  ...
  private ConnectorObject transform(final Role role) {
    ...
    // always set __UID__ and __NAME__ attribute to convince stupid framework
    // developer
    builder.setObjectClass(Keycloak.ROLE.clazz).setUid(role.id()).setName(role.name());
    try {
  ...
  /**
   ** Transforms the data received from the code>Service Provider</code> and
   ** wrapped in the specified REST {@link Client} <code>role</code>
   ** to the {@link ConnectorObject} transfer object.
   **
   ** @param  client             the specified REST {@link Client} to
   **                            transform.
   **                            <br>
   ...
   **                            Possible object is {@link ConnectorObject}.
   */
  private ConnectorObject transform(final Client client, final Role role) {
    final String method = "transform";
    entering(method);
    final ConnectorObjectBuilder builder = new ConnectorObjectBuilder();
    // always set __UID__ and __NAME__ attribute to convince stupid framework
    // developer
    builder.setObjectClass(Keycloak.CLIENTROLE.clazz)
            .setUid(String.format("%s__%s", client.id(), role.id() ))
            .setName(String.format("%s - %s", client.name(), role.name() ));
  ...
  /**
   ** Transforms the data received from the code>Service Provider</code> and
   ** wrapped in the specified REST {@link Group} <code>group</code>
   ** to the {@link ConnectorObject} transfer object.
   **
   ** @param  group               the specified REST {@link Group} to
   **                            transform.
   **                            <br>
   ...
   **                            Possible object is {@link ConnectorObject}.
   */
  private ConnectorObject transform(final Group group) {
    final String method = "transform";
    entering(method);
    final ConnectorObjectBuilder builder = new ConnectorObjectBuilder();
    // always set __UID__ and __NAME__ attribute to convince stupid framework
    // developer
    builder.setObjectClass(ObjectClass.GROUP).setUid(group.id()).setName(group.name());
    try {
      Service.transform(builder, group);
    }
    catch (IllegalArgumentException | IllegalAccessException e) {
      fatal(method, e);
    }
