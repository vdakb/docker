...
public class Lookup extends TestFixture {
  ...
  public void lookupTop() {
    try {
      final Organization identity = CONTEXT.lookupTenant(OrganizationManagerConstants.TOP_ORGANIZATION_NAME);
      assertNotNull(identity);
      assertNotNull(identity.getEntityId());
      assertEquals(identity.getEntityId(), "3");
      assertEquals(identity.getAttribute(OrganizationManagerConstants.AttributeName.ORG_NAME.getId()), OrganizationManagerConstants.TOP_ORGANIZATION_NAME);
    }
    catch (SystemException e) {
      failed(e);
    }
  ...
  public void lookupXellerate() {
    try {
      final Organization identity = CONTEXT.lookupTenant("Xellerate Users");
      assertNotNull(identity);
      assertNotNull(identity.getEntityId());
      assertEquals(identity.getEntityId(), "1");
      assertEquals(identity.getAttribute(OrganizationManagerConstants.AttributeName.ORG_NAME.getId()), "Xellerate Users");
    }
    catch (SystemException e) {
      failed(e);
    }
