 ...
 ** @since   1.0.0.0
 */
class ScimTranslator extends FilterTranslator<String> {
  ...
  // Method:   Ctor
  /**
   ** Constructs a PCF SCIM filter <code>ScimTranslator</code> that allows use
   ** as a JavaBean.
   ** <br>
   ...
   **                            <br>
   **                            Possible object is {@link ScimTranslator}.
   */
  static ScimTranslator build() {
    return new ScimTranslator();
  }
  ...
  static String filter(final String name, final String value, final String operator, final boolean not) {
    ...
    if (Uid.NAME.equals(name)) {
      normalized = ScimMarshaller.IDENTIFIER;
    }
    else if (Name.NAME.equals(name)) {
      normalized = ScimMarshaller.USERNAME;
    }
    else if (OperationalAttributeInfos.ENABLE.equals(name)) {
      normalized = ScimMarshaller.STATUS;
    }
