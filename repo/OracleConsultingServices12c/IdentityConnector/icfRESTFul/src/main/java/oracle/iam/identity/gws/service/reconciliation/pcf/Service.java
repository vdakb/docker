...
package oracle.iam.identity.gws.service.reconciliation.pcf;
 ...
 ** <code>Pivotal Cloud Foundry</code> Service Provider.
 **
 ** @author  dieter.steding@oracle.com
 ** @version 1.0.0.0
 ** @since   1.0.0.0
 */
abstract class Service extends EntityReconciliation {
  ...
  private static final String CATEGORY = "JCS.PCF.RECONCILIATION";
  ...
  // Method:   Ctor
  /**
   ** Constructs an empty <code>Service</code> scheduler that allows use as a
   ** JavaBean.
   ** <br>
  ...
  protected TargetResource populateResource()
    ...
    try {
      ...
      if(!StringUtility.isBlank(resourceName())){
        resource = PivotalResource.build(this, resourceName());
      }
      else {
        final ApplicationInstance instance = populateInstance(applicationInstance());
        resource = PivotalResource.build(this, instance.getItResourceKey());
      }
    }
    finally {
      timerStop(method);
