...
// ~~~~~ ~~~~~~~~~~~~~~
/**
 ** The test case to create accounts leveraging the
 ...
 ** @since   1.0.0.0
 */
public class TestUserCreate extends TestUser {
  ...
  // Method:   Ctor
  /**
   ** Constructs a <code>TestUserCreate</code> that allows use as a JavaBean.
   ** <br>
   ...
   */
    ...
    try {
      ...
      final Uid             uid        = Network.facade(Network.intranet()).create(ObjectClass.ACCOUNT, dataSet, null);
   ...
   */
    ...
    try {
      final Descriptor      descriptor = DescriptorFactory.configure(Descriptor.buildProvisioning(Network.CONSOLE), PROVISIONING);
      final Set<Attribute>  dataSet    = DescriptorTransformer.build(descriptor, bwgaertmax.CREATE);
      final Uid             uid        = Network.facade(Network.intranet()).create(ObjectClass.ACCOUNT, dataSet, null);
      Network.CONSOLE.debug(uid.toString());
    }
    catch (TaskException e) {
