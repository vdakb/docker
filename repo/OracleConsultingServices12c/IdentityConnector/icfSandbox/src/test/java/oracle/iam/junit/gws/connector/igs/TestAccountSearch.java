...
// ~~~~~ ~~~~~~~~~~~~~~~~~
/**
 ** The test case to obtain the accounts from the target system leveraging the
 ...
 ** @since   1.0.0.0
 */
public class TestAccountSearch extends TestFixture {
  ...
  public void testSearch() {
    try {
      final ListResponse<UserResource> result = context.searchAccount(1, 2000, null).invoke(UserResource.class);
      ...
      for (UserResource cursor : resource) {
        Assert.assertNotNull(cursor.id());
        Assert.assertNotNull(cursor.userName());
        Assert.assertNotNull(cursor.metadata());
      }
    }
