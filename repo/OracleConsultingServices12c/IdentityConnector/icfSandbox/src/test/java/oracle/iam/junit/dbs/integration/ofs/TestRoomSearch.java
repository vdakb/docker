 ...
 ** @since   1.0.0.0
 */
public class TestRoomSearch extends TestCaseIntegration {
  ...
  private static class Handler implements ResultsHandler {
    ...
    public boolean handle(final ConnectorObject object) {
      // verify
      assertEquals(object.getObjectClass(), Marshaller.ROOM);
  ...
  // Method:   Ctor
  /**
   ** Constructs a <code>TestRoomSearch</code> that allows use as a JavaBean.
   ** <br>
   ** Zero argument constructor required by the framework.
   ** <br>
   ** Default Constructor
   */
  public TestRoomSearch() {
    // ensure inheritance
    super();
  }
   ...
   */
    ...
    try {
      ...
      Network.CONSOLE.info("TestRoomSearch::fullReconciliation");
      Network.facade(Network.intranet()).search(Marshaller.ROOM, null, new Handler(), option.build());
    }
  ...
  public void futureReconciliation() {
    try {
      ...
      Network.CONSOLE.info("TestRoomSearch::futureReconciliation");
      Network.facade(Network.intranet()).search(Marshaller.ROOM, null, new Handler(), option.build());
    }
    catch (ConnectorException e) {
      failed(e);
  ...
  public void filteredReconciliationUID() {
    ...
    try {
      final OperationOptionsBuilder option = searchControl("19700101010000.000Z").setAttributesToGet(CollectionUtility.set(Uid.NAME, "description"));
      Network.CONSOLE.info("TestRoomSearch::filteredReconciliationUID :: " + filter.toString());
      Network.facade(Network.intranet()).search(Marshaller.ROOM, filter, new Handler(), option.build());
    }
    catch (ConnectorException e) {
      failed(e);
    }
  ...
  public void filteredReconciliationNAME() {
    ...
    try {
      final OperationOptionsBuilder option     = searchControl("19700101010000.000Z").setAttributesToGet(CollectionUtility.set(Uid.NAME, "description"));
      Network.CONSOLE.info("TestRoomSearch::filteredReconciliationNAME :: " + filter.toString());
      Network.facade(Network.intranet()).search(Marshaller.ROOM, filter, new Handler(), option.build());
    }
    catch (ConnectorException e) {
      failed(e);
