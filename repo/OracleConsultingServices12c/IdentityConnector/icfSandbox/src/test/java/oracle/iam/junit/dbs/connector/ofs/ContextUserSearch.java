...
////////////////////////////////////////////////////////////////////////////////
// class ContextUserSearch
// ~~~~~ ~~~~~~~~~~~~~~~~~
/**
 ** The test case belonging to user searches leveraging the the
 ...
 ** @since   1.0.0.0
 */
public class ContextUserSearch extends Transaction {
  ...
  // Method:   Ctor
  /**
   ** Constructs a <code>ContextUserSearch</code> that allows use as a JavaBean.
   ** <br>
   ** Zero argument constructor required by the framework.
   ** <br>
   ** Default Constructor
   */
  public ContextUserSearch() {
    // ensure inheritance
    super();
  }
  ...
  public void fullReconciliation() {
    ...
    try {
      final List<User> result = this.context.userSearch(searchFilter(control, null), control.start, control.limit);
      notNull(result);
  ...
  public void filteredReconciliation() {
    ...
    final OperationContext control = searchControl("19700101010000.000Z");
    try {
      final List<User> result = this.context.userSearch(searchFilter(control, DatabaseFilter.build("userName", "a", DatabaseFilter.Operator.STARTS_WITH)), control.start, control.limit);
      notNull(result);
      Assert.assertTrue("At least one entry was expected", result.size() > 0);
      Assert.assertTrue("Exactly one entry was expected", result.size() == 1);
      for (User cursor : result) {
        System.out.println(cursor)  ;
        equals("User admin was expected", "admin", cursor.uid());
      }
    }
    catch (SystemException e) {
      failed(e);
   ...
   ** @param  criteria           the user defined search criteria.
   **                            <br>
