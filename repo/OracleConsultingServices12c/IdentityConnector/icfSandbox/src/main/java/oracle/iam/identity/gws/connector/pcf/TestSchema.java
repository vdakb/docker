...
public class TestSchema {
  ...
  public static void main(String[] args) {
    try {
      final ListResponse<SchemaResource> response = Network.loginContext(Network.intranet()).schemas();
      Iterator i = response.iterator();
      for (SchemaResource resource : response) {
        for (Definition definition : resource.attribute()) {
           final String                 primary = definition.name();
          final Collection<Definition> layered = definition.attributes();
          if (layered != null && layered.size() > 0) {
             for (Definition sub : layered) {
               System.out.println(resource.id() + "::" + resource.name() + " --> " + String.format("%s.%s", primary, sub.name()));
             }
          }
          else
            System.out.println(resource.id() + "::" + resource.name() + " --> " + primary);
        }
      }
    }
