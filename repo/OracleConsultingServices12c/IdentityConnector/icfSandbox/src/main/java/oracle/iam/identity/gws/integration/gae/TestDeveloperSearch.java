...
public class TestDeveloperSearch extends TestDeveloper {
  ...
  public static void main(String[] args) {
    final Set<String>             target = CollectionUtility.set("__UID__", "__NAME__", "__ENABLE__", "lastName", "firstName");
    final OperationOptionsBuilder option = new OperationOptionsBuilder().setAttributesToGet(target);
    final QualifiedUid            uid    = new QualifiedUid(TENANT, new Uid("phoenix"));
    option.setContainer(uid);
    try {
      Network.facade(Network.localnet()).search(DEVELOPER, null, new Handler(), option.build());
    }
    catch (ConnectorException e) {
      // the exception thrown provides a special format in the message
      // containing a error code and a detailed text
      // this message needs to be plitted by a "::" character sequence
