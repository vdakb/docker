...
class Network {
   ...
   ** Intranet
   */
  static final String               NIRVANA  = "nirvana.vm.oracle.com";
  /**
   ** The DNS name of the server if the Test Cases are executed inside of the
   ** Intranet
   */
  ...
  // Method:   nirvana
  /**
   ** Factory method to create a new {@link Context} if the test case
   ** is executed from nirvana connectivity.
   **
   ** @return                    the {@link Context} configured for
   **                            intranet.
   */
  protected static DatabaseContext nirvana() {
    return endpoint(NIRVANA);
  }

  //////////////////////////////////////////////////////////////////////////////
  // Method:   intranet
  /**
   ** Factory method to create a new {@link DatabaseContext} if the test case
   ** is executed from intranet connectivity.
   **
   ** @return                    the {@link DatabaseContext} configured for
   **                            intranet.
   */
  protected static DatabaseContext intranet() {
   ...
   **                            extranet.
   */
  public static DatabaseContext extranet() {
  ...
  // Method:   endpoint
  /**
   ** Factory method to create a new {@link DatabaseContext} for the specified
   ...
   */
  private static DatabaseContext endpoint(String serverName) {
    DatabaseEndpoint.Driver    driver    = DatabaseEndpoint.Driver.from("oracle.jdbc.OracleDriver");
    AbstractEndpoint.Server    primary   = new AbstractEndpoint.Server(serverName, 7021);
    AbstractEndpoint.Principal principal = new AbstractEndpoint.Principal("systemXXX", new GuardedString(Password.read(PASSWORD).toCharArray()));
    return DatabaseContext.build(DatabaseEndpoint.build(CONSOLE, driver, primary, "mdr.vm.oracle.com", principal, false, "en", "US", "GMT+01:00"));
  }
}