...
public class TestGroupSearch {
  ...
  public static void main(String[] args) {
    ...
    int               index = 1;
    final int         size  = 500;
    final Set<String> returning = CollectionUtility.set("id", "displayName", "meta");
    try {
      final Context context = Network.intranet();
      do {
        final ListResponse<GroupResource> response = context.searchGroup().page(index, size).emit(returning).invoke(GroupResource.class);
