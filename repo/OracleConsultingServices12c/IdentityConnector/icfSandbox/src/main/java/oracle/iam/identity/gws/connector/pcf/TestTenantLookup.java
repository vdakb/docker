...
// ~~~~~ ~~~~~~~~~~~~~~~~
/**
 ** The test case to fetch a single tenant from the target system.
 **
 ** @author  dieter.steding@oracle.com
 ** @version 1.0.0.0
 ** @since   1.0.0.0
...
public class TestTenantLookup {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      final Result.Tenant response = context.lookupTenant(UriBuilder.fromUri(RestContext.ENDPOINT_TENANT).path("4bd93aee-1236-4af8-bee2-9fcea298fc75").build());
      if (response == null) {
        System.out.println("No Records available to be fetched from Target.");
      }
      else {
        System.out.println("guid: " + response.guid());
