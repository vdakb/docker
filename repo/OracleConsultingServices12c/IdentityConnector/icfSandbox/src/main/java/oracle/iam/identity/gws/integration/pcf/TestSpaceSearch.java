 ...
 ** @since   1.0.0.0
 */
public class TestSpaceSearch extends TestCase {
  ...
  public static void main(String[] args) {
    final Set<String>             target = CollectionUtility.set("__UID__", "__NAME__", "entity.tenant");
    final OperationOptionsBuilder option = new OperationOptionsBuilder().setAttributesToGet(target);
    option.getOptions().put(BATCH_SIZE_OPTION, new Integer(100));
    try {
      // create the object class required to search for tenants
      Network.facade( Network.intranet()).search(SPACE, null, new Handler(), option.build());
    }
