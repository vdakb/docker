...
public class TestAccountSearch {
  ...
  public static void main(String[] args) {
    ...
    int               index = 1;
    final int         size  = 500;
    final Set<String> returning = CollectionUtility.set("id", "userName", "meta", "name");
    try {
      final Context context = Network.intranet();
      do {
        final ListResponse<UserResource> response = context.searchAccount().page(index, size).emit(returning).invoke(UserResource.class);
