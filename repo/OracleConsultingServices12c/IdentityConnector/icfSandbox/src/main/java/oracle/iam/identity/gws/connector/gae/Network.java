...
class Network {
   ...
   ** local network.
   */
  static final AbstractEndpoint.Server LOCALNET = new AbstractEndpoint.Server("dsteding-de.de.oracle.com", 8015);
  /**
   ** The DNS name of the server if the Test Cases are executed inside of the
   ** intranet network.
   */
  static final AbstractEndpoint.Server INTRANET = new AbstractEndpoint.Server("buster.vm.oracle.com", 8015);
//  static final AbstractEndpoint.Server INTRANET = new AbstractEndpoint.Server("172.16.182.90", 8080);
  /**
   ** The DNS name of the server if the Test Cases are executed inside of the
   ** extranet network
   */
  static final AbstractEndpoint.Server EXTRANET = new AbstractEndpoint.Server("www.cinnamonstar.net", 8015);

  //////////////////////////////////////////////////////////////////////////////
  // Method:   localnet
  /**
   ** Factory method to create a new {@link Context} if the test case is
   ** executed from local connectivity.
   **
   ** @return                    the {@link Context} configured for local
   **                            network.
   */
   ...
   */
  private static Context endpoint(final AbstractEndpoint.Server server)
    throws SystemException {

    final AbstractEndpoint.Principal principal = new AbstractEndpoint.Principal("apigee@bka.bund.de", new GuardedString("37DDH3tWse9GF8XR5uDtgBGm5SM5dcxH".toCharArray()));
