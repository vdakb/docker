...
public class Search extends AbstractSearchState {
  ...
  // compatible with
  @SuppressWarnings("compatibility:-1307856671562271514")
  private static final long serialVersionUID = 5810011709861922492L;
  ...
  public Search() {
    // ensure inheritance
    super(OrchestrationProcessAdapter.PK, OrchestrationProcessAdapter.NAME);
  }
  ...
  /**
   ** Whether the selected row (aka Orchestration Process) can be opened in a
   ** detail page or not.
   **
   ...
   ** Orchestration Process taskflow to render the details of the selected
  ...
  public String openDetail(final @SuppressWarnings("unused") ActionEvent event) {
    ...
    final String              key        = row.getAttribute(OrchestrationProcessAdapter.PK).toString();
    final String              taskFlowId = "orp" + key.toString();
    final Map<String, Object> parameter  = new HashMap<String, Object>();
    parameter.put(OrchestrationProcessAdapter.PK, key);
    parameter.put(PARAMETER_MODE,                 MODE_VIEW);
    parameter.put(PARAMETER_TASKFLOW,             taskFlowId);
    raiseTaskFlowLaunchEvent(taskFlowId, "/WEB-INF/oracle/iam/identity/sysadmin/orp/flow/train-tf.xml#train-tf", key, null, key, "orp-detail", false, parameter);
    return null;
  }
}