...
public class Search extends AbstractSearchState {
  ...
  // compatible with
  @SuppressWarnings("compatibility:-3503374665068690001")
  private static final long   serialVersionUID = -8275606122310906548L;
  ...
  public Search() {
    // ensure inheritance
    super(JobAdapter.PK, JobAdapter.PK, JobAdapter.STATUS);
  }
  ...
  /**
   ** Whether the selected row (aka Job) can be opened in a detail page or not.
   **
  ...
  /**
   ** Whether the selected row(s) (aka Job) can be deleted or not.
   **
  ...
  public String getLocalizedMessage() {
    ...
    switch (localized) {
      case GENERIC_DELETE : return (selection.size() == 1)
                            ? String.format(ADF.resourceBundleValue(BUNDLE, "JOB_DELETE_CONFIRM_SINGLE"), selection.get(0).getAttribute(JobAdapter.PK))
                            : String.format(ADF.resourceBundleValue(BUNDLE, "JOB_DELETE_CONFIRM_MULTIPLE"), selection.size());
  ...
  /**
   ** Raise a <code>Contextual Event</code> programmatically to launch the
   ** taskflow to render the details of the selected <code>Scheduler Job</code>.
   **
   ** @return                    the outcome of the action to apply control
   **                            flow rules in the task flow.
   */
  ...
  public void delete(final List<Row> selection) {
    if (selection != null && !selection.isEmpty()) {
      ...
      final Object name = selection.get(0).getAttribute(JobAdapter.PK);
      ...
      if (selection.size() == 1)
        raiseTaskFlowFeedbackEvent(String.format(ADF.resourceBundleValue(BUNDLE, "JOB_DELETE_FEEDBACK_SINGLE"), name));
      else
        raiseTaskFlowFeedbackEvent(String.format(ADF.resourceBundleValue(BUNDLE, "JOB_DELETE_FEEDBACK_MULTIPLE"), selection.size()));
  ...
  private void launch(final String mode) {
    ...
    if (MODE_EDIT.equals(mode)) {
      ...
      jobName = (String)selection.get(0).getAttribute(JobAdapter.PK);
      jobTask = (String)selection.get(0).getAttribute(JobAdapter.TASK);
    }

    final String              taskFlowId  = "job" + jobName;
    final String              regionTitle = MODE_EDIT.equals(mode) ? jobName : ADF.resourceBundleValue(BUNDLE, "JOB_CREATE_TITLE");
    final Map<String, Object> parameter   = new HashMap<String, Object>();
    parameter.put(JobAdapter.PK,      jobName);
    parameter.put(JobAdapter.TASK,    jobTask);
    parameter.put(PARAMETER_MODE,     mode);
    parameter.put(PARAMETER_TASKFLOW, taskFlowId);
    raiseTaskFlowLaunchEvent(taskFlowId, "/WEB-INF/oracle/iam/identity/sysadmin/job/flow/train-tf.xml#train-tf", regionTitle, null, null, ConstantsDefinition.HELP_TOPIC_ID_ADM_SYSMAN_SCHED, false, parameter);
  }
}