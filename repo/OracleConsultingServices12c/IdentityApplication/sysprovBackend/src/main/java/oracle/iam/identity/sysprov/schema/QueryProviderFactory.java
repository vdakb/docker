...
public class QueryProviderFactory {
  ...
  private static final String              REPOSITORY         = "META-INF/sql/sysprov.sql";
  ...
  private static final String              PAGINATED_NOT_SORT = "SELECT * FROM (%s WHERE %s ORDER BY %s) WHERE rownumber BETWEEN ? AND ?";
  private static final String              PAGINATED_AND_SORT = "SELECT * FROM (%s AND %s ORDER BY %s) WHERE rownumber BETWEEN ? AND ?";
   ...
   ** @param  criteria           the filter criteria the WHERE-Clause has to
   **                            build for.
   **
   ** @return                    a {@link ResultSet} containing backend objects.
   */
   ...
   ** @param  criteria           the filter criteria the WHERE-Clause has to
   **                            build for.
   ** @param  orderby            the sort criteria the ORDER-BY-Clause has to
   **                            build for.
   **
   ** @return                    a {@link PreparedStatement} ready for
   **                            execution.
   */
  public PreparedStatement preparedStatement(final String property, final DatabaseFilter.Operator conjunction, final Connection connection, final DatabaseFilter criteria, final DatabaseSort orderby) {
    return preparedStatement(property, conjunction, connection, criteria, orderby, 1, 500);
  }
   ...
   **                            execution.
   */
  public PreparedStatement preparedStatement(final String property, final DatabaseFilter.Operator conjunction, final Connection connection, final DatabaseFilter criteria, final DatabaseSort orderby, final int start, final int count) {
    try {
      // prepare the filter criteria
      final List<DatabaseParameter> arguments = new ArrayList<DatabaseParameter>();
      final String                  filter = DatabaseStatement.prepareFilter(criteria, arguments);
      final String                  sort   = DatabaseStatement.prepareOrder(orderby);
      final String                  query  = String.format(conjunction != null && conjunction == DatabaseFilter.Operator.AND ? PAGINATED_AND_SORT : PAGINATED_NOT_SORT, QueryProviderFactory.instance.query(property), filter, sort);
      arguments.add(DatabaseParameter.build(start, Types.INTEGER));
      arguments.add(DatabaseParameter.build(count, Types.INTEGER));
      return preparedStatement(connection, query, arguments);
    }
