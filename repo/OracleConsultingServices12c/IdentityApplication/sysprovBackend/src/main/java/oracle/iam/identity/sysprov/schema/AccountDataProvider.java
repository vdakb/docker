...
public class AccountDataProvider extends AbstractDataProvider<AccountAdapter> {
  ...
  // compatible with
  @SuppressWarnings("compatibility:6995868964334087593")
  ...
  static {
    synchronized(FILTER) {
      ...
      FILTER.put(ResourceObjectAdapter.PK,                  "obj." + OBJ_KEY);
  ...
  // Method:   Ctor
  /**
   ** Constructs an empty <code>AccountDataProvider</code> data
   ...
   **
   ** @param  searchCriteria     the OIM search criteria to apply.
...
//        mab.setProcessInstanceArchived(resultSet.getString(ORC_ARCHIVED));
   ...
   ** to the names declared in Identity Manager for entity Account Instance.
   **
  ...
  private DatabaseFilter extractCriteria(final SearchCriteria criteria) {
    ...
    if (!(lh instanceof SearchCriteria && rh instanceof SearchCriteria)) {
      ...
      if (((String)lh).equalsIgnoreCase("Bind_objectsKey") || ((String)lh).equalsIgnoreCase(ResourceObjectAdapter.PK)) {
        partial = DatabaseFilter.build(FILTER.get(ResourceObjectAdapter.PK), rh, DatabaseFilter.Operator.EQUAL);
      }
