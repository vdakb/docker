/*
    ...
    File        :   EndpointTypeDataProvider.java
...
////////////////////////////////////////////////////////////////////////////////
// class EndpointTypeDataProvider
// ~~~~~ ~~~~~~~~~~~~~~~~~~~~~~~~
/**
 ...
 ** @since   2.0.0.0
 */
public class EndpointTypeDataProvider extends AbstractDataProvider<EndpointTypeAdapter> {
  ...
  // compatible with
  @SuppressWarnings("compatibility:-8795114834023217617")
  private static final long   serialVersionUID = 8249820411550072016L;
  ...
  // Method:   Ctor
  /**
   ** Constructs an empty <code>EndpointTypeDataProvider</code> data access
   ...
   ** Default Constructor
   */
  public EndpointTypeDataProvider() {
    // ensure inheritance
    super();
  }
   ...
   **                            element is of type {@link EndpointTypeAdapter}.
   */
  @Override
  public List<EndpointTypeAdapter> search(final SearchCriteria searchCriteria, final Set<String> requested, final HashMap<String, Object> control) {
    ...
    try {
      final tcResultSet resultSet = service.getITResourceDefinition(filter);
  ...
  private Map<String, String> criteriaToMap(final SearchCriteria searchCriteria, Map<String, String> map) {
    ...
    if (criteria == null) {
      criteria = new SearchCriteria(EndpointTypeAdapter.NAME, "%", SearchCriteria.Operator.EQUAL);
    }
    ...
    switch (op) {
      case EQUAL       : if (ITResourceType.MULTIPLE.equals(lhs))
                           map.put(lhs.toString(), Boolean.valueOf(rhs.toString()) ? "1" : "0");
                         else
                           map.put(lhs.toString(), rhs.toString());
                         break;
      case NOT_EQUAL   : if (ITResourceType.MULTIPLE.equals(lhs))
                           map.put(lhs.toString(), Boolean.valueOf(rhs.toString()) ? "0" : "1");
                         else
                           map.put(lhs.toString(), rhs.toString());
                         break;
  ...
  private String convertAttributeName(final String outboundName) {
    ...
    if (inboundName.equals(EndpointTypeAdapter.PK))
      inboundName = ITResourceType.KEY;
    else if (inboundName.equals(EndpointTypeAdapter.NAME))
      inboundName = ITResourceType.NAME;
    else if (inboundName.equals(EndpointTypeAdapter.MULTIPLE))
      inboundName = ITResourceType.MULTIPLE;
    else
   ...
   ** {@link tcResultSet}.
   ** <b>
   ** Each element in the returned list is an {@link EndpointTypeAdapter}.
   **
   ** @param  control            contains pagination, sort attribute, and sort
   **                            direction.
   **                            <br>
   ...
   **                            element is of type [@link EndpointTypeAdapter}.
   */
  private List<EndpointTypeAdapter> paginate(final HashMap<String, Object> parameter, final tcResultSet resultSet, final int totalRowCount) {
    final List<EndpointTypeAdapter> batch = new ArrayList<EndpointTypeAdapter>();
    if (resultSet == null)
      return batch;
    ...
    return new PagedArrayList<EndpointTypeAdapter>(batch, rangeStart, totalRowCount);
  }
  ...
  // Methode:   build
  /**
   ** Factory method to create a <code>EndpointTypeAdapter</code> from a
   ** {@link tcResultSet}.
   ** <p>
   ** <b>Note</b>:
   ** <br>
   ...
   */
  private static EndpointTypeAdapter build(final tcResultSet resultSet) {
    final EndpointTypeAdapter mab = EndpointTypeAdapter.build();
    try {
      mab.setEndpointTypeKey(resultSet.getLongValue(ITResourceType.KEY));
      mab.setEndpointTypeName(resultSet.getStringValue(ITResourceType.NAME));
      mab.setMultiple(resultSet.getBooleanValue(ITResourceType.MULTIPLE));
     }
