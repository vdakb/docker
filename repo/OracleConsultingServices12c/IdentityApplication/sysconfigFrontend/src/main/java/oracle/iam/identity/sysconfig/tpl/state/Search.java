...
public class Search extends AbstractSearchState {
  ...
  // compatible with
  @SuppressWarnings("compatibility:4525212788609299418")
  private static final long   serialVersionUID = 4817832054091891164L;
  ...
  public Search() {
    // ensure inheritance
    super(NotificationTemplateAdapter.PK, NotificationTemplateAdapter.NAME);
  }
  ...
  /**
   ** Whether the selected row (aka Notification Template) can be opened in a
   ** detail page or not.
   **
  ...
  /**
   ** Whether the selected row(s) (aka ) can be deleted or not.
   **
  ...
  public String getLocalizedMessage() {
    ...
    switch (localized) {
      case GENERIC_DELETE : return (selection.size() == 1)
                            ? String.format(ADF.resourceBundleValue(BUNDLE, "TPL_DELETE_CONFIRM_SINGLE"), selection.get(0).getAttribute(NotificationTemplateAdapter.NAME))
                            : String.format(ADF.resourceBundleValue(BUNDLE, "TPL_DELETE_CONFIRM_MULTIPLE"), selection.size());
  ...
  public String refresh() {
    refresh("NotificationTemplateIterator");
    return null;
  }
   ...
   ** <code>Notification Template</code>s.
   **
   ** @return                    the outcome of the action to apply control
   **                            flow rules in the task flow.
   */
  ...
  public void delete(final List<Row> selection) {
    if (selection != null && !selection.isEmpty()) {
      ...
      final Object name = selection.get(0).getAttribute(NotificationTemplateAdapter.NAME);
      ...
      if (selection.size() == 1)
        raiseTaskFlowFeedbackEvent(String.format(ADF.resourceBundleValue(BUNDLE, "TPL_DELETE_FEEDBACK_SINGLE"), name));
      else
        raiseTaskFlowFeedbackEvent(String.format(ADF.resourceBundleValue(BUNDLE, "TPL_DELETE_FEEDBACK_MULTIPLE"), selection.size()));
  ...
  private void launch(final String mode) {
    ...
    if (MODE_EDIT.equals(mode)) {
      ...
      key  = (Long)row.getAttribute(NotificationTemplateAdapter.PK);
      name = (String)row.getAttribute(NotificationTemplateAdapter.NAME);
    }

    final String              taskFlowId  = "tpl" + key;
    final String              regionTitle = MODE_EDIT.equals(mode) ? name : ADF.resourceBundleValue(BUNDLE, "TPL_CREATE_TITLE");
    final Map<String, Object> parameter   = new HashMap<String, Object>();
    parameter.put(NotificationTemplateAdapter.PK,   key);
    parameter.put(NotificationTemplateAdapter.NAME, name);
    parameter.put(PARAMETER_MODE,     mode);
    parameter.put(PARAMETER_TASKFLOW, taskFlowId);
    raiseTaskFlowLaunchEvent(taskFlowId, "/WEB-INF/oracle/iam/identity/sysconfig/tpl/flow/train-tf.xml#train-tf", regionTitle, null, null, ConstantsDefinition.HELP_TOPIC_ID_ADM_SYSMAN_CONF, false, parameter);
  }
}