...
public class Search extends AbstractSearchState {
  ...
  // compatible with
  @SuppressWarnings("compatibility:4207208063909725407")
  private static final long   serialVersionUID = 2528590837098552228L;
  ...
  public Search() {
    // ensure inheritance
    super(LookupAdapter.PK, LookupAdapter.NAME);
  }
  ...
  public String getLocalizedMessage() {
    ...
    switch (localized) {
      case GENERIC_DELETE : return (selection.size() == 1)
                            ? String.format(ADF.resourceBundleValue(BUNDLE, "LKU_DELETE_CONFIRM_SINGLE"), selection.get(0).getAttribute(LookupAdapter.NAME))
                            : String.format(ADF.resourceBundleValue(BUNDLE, "LKU_DELETE_CONFIRM_MULTIPLE"), selection.size());
  ...
  public void delete(final List<Row> selection) {
    if (selection != null && !selection.isEmpty()) {
      // latch the name of the first lookup to delete for feedback purpose
      final Object name = selection.get(0).getAttribute(LookupAdapter.NAME);
      ...
      if (selection.size() == 1)
        raiseTaskFlowFeedbackEvent(String.format(ADF.resourceBundleValue(BUNDLE, "LKU_DELETE_FEEDBACK_SINGLE"), name));
      else
        raiseTaskFlowFeedbackEvent(String.format(ADF.resourceBundleValue(BUNDLE, "LKU_DELETE_FEEDBACK_MULTIPLE"), selection.size()));
  ...
  private void launch(final String mode) {
    ...
    if (MODE_EDIT.equals(mode)) {
      ...
      lookupKey  = (String)row.getAttribute(LookupAdapter.PK);
      lookupName = (String)row.getAttribute(LookupAdapter.NAME);
    }

    final String              taskFlowId  = "lku" + lookupKey;
    final String              regionTitle = MODE_EDIT.equals(mode) ? lookupName : ADF.resourceBundleValue(BUNDLE, "LKU_CREATE_TITLE");
    final Map<String, Object> parameter   = new HashMap<String, Object>();
    parameter.put(LookupAdapter.PK,   lookupKey);
    parameter.put(LookupAdapter.NAME, lookupName);
    parameter.put(PARAMETER_MODE,     mode);
    parameter.put(PARAMETER_TASKFLOW, taskFlowId);
    raiseTaskFlowLaunchEvent(taskFlowId, "/WEB-INF/oracle/iam/identity/sysconfig/lku/flow/train-tf.xml#train-tf", regionTitle, null, null, ConstantsDefinition.HELP_TOPIC_ID_ADM_CONFIG_LOOKUP, false, parameter);
  }
}