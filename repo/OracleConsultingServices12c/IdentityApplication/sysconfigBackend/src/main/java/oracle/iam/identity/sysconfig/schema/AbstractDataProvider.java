...
public abstract class AbstractDataProvider<T extends ModelAdapterBean> extends oracle.iam.ui.platform.model.common.AbstractDataProvider<T> {
  ...
  private static final String BUNDLE           = "oracle.iam.identity.sysconfig.bundle.Backend";

  // the official serial version ID which says cryptically which version we're
  // compatible with
  @SuppressWarnings("compatibility:-2683424692778353911")
  private static final long   serialVersionUID = 7804368650410536971L;
   ...
   ** @param  qualified          the fullqualifiwd name of the column the
   **                            created filter will take in account.
   **                            <br>
   ...
   */
  public static DatabaseFilter createFilter(final SearchCriteria.Operator operator, final String qualified, final Object value) {
    switch (operator) {
      ...
      case DOES_NOT_CONTAIN : return DatabaseFilter.build(qualified, value, DatabaseFilter.Operator.NOT_CONTAINS);
      default               : return DatabaseFilter.build(qualified, value, DatabaseFilter.Operator.STARTS_WITH);
    }
  }

  //////////////////////////////////////////////////////////////////////////////
  // Methode:   booleanFilter
  /**
   ** Factory method to create a {@link DatabaseFilter}.
   **
   ** @param  operator           the filter operator.
   ...
   **                            Possible object is {@link DatabaseFilter}.
   */
   public static DatabaseFilter booleanFilter(final SearchCriteria.Operator operator, final String qualified, final Object value) {
    if ((value instanceof Boolean))
      return DatabaseFilter.build(qualified, ((Boolean)value) ? "1" : "0", operator == SearchCriteria.Operator.EQUAL ?  DatabaseFilter.Operator.EQUAL : DatabaseFilter.Operator.NOT_EQUAL);
    else
      return DatabaseFilter.build(qualified, value, operator == SearchCriteria.Operator.EQUAL ?  DatabaseFilter.Operator.EQUAL : DatabaseFilter.Operator.NOT_EQUAL);
  }
  ...
  /**
   ** Return the search criteria list of sysconfig objects based on the given
   ** filter criteria.
   **
   ** @param  criteria           the OIM search criteria to invastigate.
   ** @param  name               the name (aka view attribute) to lookup.
