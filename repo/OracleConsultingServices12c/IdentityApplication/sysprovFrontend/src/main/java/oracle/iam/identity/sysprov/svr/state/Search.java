...
public class Search extends AbstractSearchState {
  ...
  // compatible with
  @SuppressWarnings("compatibility:3910837517385662465")
  private static final long   serialVersionUID = 4911823938417900868L;
  ...
  public Search() {
    // ensure inheritance
    super(EndpointAdapter.PK, EndpointAdapter.NAME);
  }
  ...
  /**
   ** Whether the selected row (aka IT Resource) can be opened in a
   ** detail page or not.
   **
  ...
  /**
   ** Whether the selected row(s) (aka IT Resource) can be deleted or not.
   **
  ...
  public String getLocalizedMessage() {
    ...
    switch (localized) {
      case GENERIC_DELETE : return (selection.size() == 1)
                            ? String.format(ADF.resourceBundleValue(BUNDLE, "SVR_DELETE_CONFIRM_SINGLE"), selection.get(0).getAttribute(EndpointAdapter.NAME))
                            : String.format(ADF.resourceBundleValue(BUNDLE, "SVR_DELETE_CONFIRM_MULTIPLE"), selection.size());
  ...
  public void delete(final List<Row> selection) {
    if (selection != null && !selection.isEmpty()) {
      // latch the name of the first IT Resource to delete for feedback purpose
      final Object name = selection.get(0).getAttribute(EndpointAdapter.NAME);
      ...
      if (selection.size() == 1)
        raiseTaskFlowFeedbackEvent(String.format(ADF.resourceBundleValue(BUNDLE, "SVR_DELETE_FEEDBACK_SINGLE"), name));
      else
        raiseTaskFlowFeedbackEvent(String.format(ADF.resourceBundleValue(BUNDLE, "SVR_DELETE_FEEDBACK_MULTIPLE"), selection.size()));
  ...
  private void launch(final String mode) {
    Long   endpointKey  = Long.valueOf(-1L);
    ...
    if (MODE_EDIT.equals(mode)) {
      ...
      endpointKey  = (Long)row.getAttribute(EndpointAdapter.PK);
      endpointName = (String)row.getAttribute(EndpointAdapter.NAME);
    }

    final String              taskFlowId  = "svr" + endpointKey;
    final String              regionTitle = MODE_EDIT.equals(mode) ? endpointName : ADF.resourceBundleValue(BUNDLE, "SVR_CREATE_TITLE");
    ...
    raiseTaskFlowLaunchEvent(taskFlowId, "/WEB-INF/oracle/iam/identity/sysprov/svr/flow/train-tf.xml#train-tf", regionTitle, null, regionTitle, ConstantsDefinition.HELP_TOPIC_ID_ADM_CONFIG_APPINST, false, parameter);
  }
}