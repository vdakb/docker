...
public class AdministratorState extends AbstractSearch {
  ...
  private static final String PICK_ROLE        = "catalog_role";
  ...
  // compatible with
  @SuppressWarnings("compatibility:-3075030561460266631")
  private static final long   serialVersionUID = -3526459712544333675L;
  ...
  // Method:   Ctor
  /**
   ** Constructs a <code>AdministratorState</code> backing bean that allows use
   ** as a JavaBean.
   ** <br>
  ...
  /**
   ** Whether the delete button of an Resource Object Administrator relationship
   ** is disabled.
   **
  ...
  public String getLocalizedMessage() {
    ...
    switch (localized) {
      case GENERIC_DELETE : return ADF.resourceBundleValue(BUNDLE, "OBJ_ADMINISTRATOR_DELETE_CONFIRM");
      case GENERIC_UNDO   : return ADF.resourceBundleValue(BUNDLE, "OBJ_ADMINISTRATOR_UNDO_CONFIRM");
   ...
   ** <ol>
   **   <li>the hardcoded prefix <code>obj</code>
  ...
  public static String entityDistinguisher() {
    return String.format("obj#%s", ADF.pageFlowScopeStringValue(ResourceObjectAdapter.PK));
  }
  ...
  public void pickListener(final @SuppressWarnings("unused") ActionEvent event) {
    ...
    raiseTaskFlowLaunchEvent(PICK_ROLE, PICK_FLOW, ADF.resourceBundleValue(BUNDLE, "OBJ_ADMINISTRATOR_PICKER"), "/images/add_ena.png", null, null, true, parameter);
  }
  ...
  public void createListener(final ActionEvent event) {
    ...
    final ResourceObjectAdministratorAdapter mab  = ResourceObjectAdministratorAdapter.build(Long.valueOf(ADF.pageFlowScopeStringValue(ResourceObjectAdapter.PK)));

    // keep the row created above in the cache so that the row survives the next
    // refresh request
    final Map<Long, ResourceObjectAdministratorAdapter> assign = train.administratorAssign();
  ...
  /**
   ** Refresh the administrator belonging to a certain
   ** <code>Resource Object</code>.
   **
   ** @param  event              the {@link ActionEvent} object that
   **                            characterizes the action to perform.
  ...
  public void assign(final List<Serializable> selection) {
    if (!CollectionUtility.empty(selection)) {
      final Train                                         train   = train();
      final Map<Long, ResourceObjectAdministratorAdapter> assign  = train.administratorAssign();
      final Map<Long, ResourceObjectAdministratorAdapter> remove  = train.administratorRemove();
      final Map<Long, ResourceObjectAdministratorAdapter> modify  = train.administratorModify();
      ...
      for (int i = 0; i < selection.size(); i++) {
        ...
        mab.setPendingAction(ResourceObjectAdministratorAdapter.ADD);
        final Long scope = mab.getGroupKey();
        if (remove.containsKey(scope)) {
          remove.remove(scope);
          mab.setPendingAction(ResourceObjectAdministratorAdapter.MOD);
   ...
   */
  public void writeAccessChanged(final ValueChangeEvent event) {
    ...
    final Long                                          object     = (Long)attributes.get(ResourceObjectAdministratorAdapter.OBJ_PK);
    final Long                                          scope      = (Long)attributes.get(ResourceObjectAdministratorAdapter.UGP_PK);
    final Map<Long, ResourceObjectAdministratorAdapter> assign     = train.administratorAssign();
    final Map<Long, ResourceObjectAdministratorAdapter> modify     = train.administratorModify();
    if (assign.containsKey(scope)) {
      assign.get(scope).setWriteAccess((Boolean)event.getNewValue());
    }
    else if (modify.containsKey(scope)) {
      modify.get(scope).setWriteAccess((Boolean)event.getNewValue());
    }
    else {
      final ResourceObjectAdministratorAdapter mab = ResourceObjectAdministratorAdapter.build(object);
      mab.setGroupKey(scope);
      mab.setWriteAccess((Boolean)event.getNewValue());
      mab.setPendingAction(ResourceObjectAdministratorAdapter.MOD);
      modify.put(scope, mab);
    }
    train.markDirty();
   ...
   **                            Allowed object is {@link ValueChangeEvent}.
   */
  public void deleteAccessChanged(final ValueChangeEvent event) {
    if (event.getNewValue() == null || (event.getOldValue() != null && event.getNewValue().toString().equals(event.getOldValue().toString())))
      return;
    ...
    final Map<Long, ResourceObjectAdministratorAdapter> modify     = train.administratorModify();
    if (assign.containsKey(scope)) {
      assign.get(scope).setDeleteAccess((Boolean)event.getNewValue());
    }
    else if (modify.containsKey(scope)) {
      modify.get(scope).setDeleteAccess((Boolean)event.getNewValue());
    }
    else {
      final ResourceObjectAdministratorAdapter mab = ResourceObjectAdministratorAdapter.build(object);
      mab.setGroupKey(Long.valueOf(scope));
      mab.setDeleteAccess((Boolean)event.getNewValue());
      mab.setPendingAction(ResourceObjectAdministratorAdapter.MOD);
      modify.put(scope, mab);
    }
    train.markDirty();
  ...
  // Methode:   remove
  /**
   ** Removes the given administrators from the collection of values which
   ** this <code>Resource Object</code> administerate.
   **
  ...
  public void remove(final List<Row> selection) {
    if (selection != null && !selection.isEmpty()) {
      ...
      for (Row cursor : selection) {
        final String action = (String)cursor.getAttribute(ResourceObjectAdministratorAdapter.ACTION);
        if (StringUtility.isEmpty(action) || ResourceObjectAdministratorAdapter.MOD.equals(action)) {
          cursor.setAttribute(ResourceObjectAdministratorAdapter.ACTION, ResourceObjectAdministratorAdapter.DEL);
          final ResourceObjectAdministratorAdapter value = new ResourceObjectAdministratorAdapter(cursor);
          remove.put(value.getGroupKey(), value);
          touched = true;
        }
        else if (ResourceObjectAdministratorAdapter.ADD.equals(action)) {
          cursor.remove();
          assign.remove(cursor.getAttribute(ResourceObjectAdministratorAdapter.UGP_PK));
          touched = true;
        }
      }
      if (touched) {
  ...
  public void undo(final List<Row> selection) {
    if (!CollectionUtility.empty(selection)) {
      ...
      for (Row cursor : selection) {
        // no need to cast
        final Object administrator = cursor.getAttribute(ResourceObjectAdministratorAdapter.UGP_PK);
        if (assign.containsKey(administrator)) {
          assign.remove(administrator);
          touched = true;
        }
        else if (modify.containsKey(administrator)) {
          modify.remove(administrator);
          touched = true;
        }
        else if (remove.containsKey(administrator)) {
...
//        ADF.partialRender(getSearchTable());
  ...
  // Method:   refresh
  /**
   ** Refreshs the administrator region of the <code>Resource Object</code>.
   */
  private void refresh() {
    final Train                                                      train   = train();
    final Map<String, Map<Long, ResourceObjectAdministratorAdapter>> pending = new HashMap<String, Map<Long, ResourceObjectAdministratorAdapter>>();

    pending.put(ResourceObjectAdministratorAdapter.ADD, train.administratorAssign());
    pending.put(ResourceObjectAdministratorAdapter.DEL, train.administratorRemove());
    pending.put(ResourceObjectAdministratorAdapter.MOD, train.administratorModify());
    ...
    administrator.put("identifier", Long.valueOf(ADF.pageFlowScopeStringValue(ResourceObjectAdapter.PK)));
    administrator.put("pending",    pending);
    ADF.executeOperation("refreshAdministrator", administrator);
  }
