...
public class Search extends AbstractSearchState {
  ...
  // compatible with
  @SuppressWarnings("compatibility:-7991177422515391085")
  private static final long   serialVersionUID = -390941102628425239L;
  ...
  public Search() {
    // ensure inheritance
    super(ResourceObjectAdapter.PK, ResourceObjectAdapter.NAME);
  }
  ...
  /**
   ** Whether the selected row (aka Lookup Definition) can be opened in a
   ** detail page or not.
   **
  ...
  public String getLocalizedMessage() {
    ...
    switch (localized) {
      case GENERIC_DELETE : return (selection.size() == 1)
                            ? String.format(ADF.resourceBundleValue(BUNDLE, "OBJ_DELETE_CONFIRM_SINGLE"), selection.get(0).getAttribute(ResourceObjectAdapter.NAME))
                            : String.format(ADF.resourceBundleValue(BUNDLE, "OBJ_DELETE_CONFIRM_MULTIPLE"), selection.size());
   ...
   ** to entries in the Resource Object table after confirmation.
   **
  ...
  public String refresh() {
    refresh("ResourceObjectIterator");
    return null;
  }
  ...
  /**
   ** Raise a <code>Contextual Event</code> programmatically to launch the
   ** taskflow to render the details of the selected <code>Resource Object</code>.
   **
   ** @return                    the outcome of the action to apply control
   **                            flow rules in the task flow.
   */
  ...
  public void delete(final List<Row> selection) {
    if (selection != null && !selection.isEmpty()) {
      // latch the name of the first Resource Object to delete for feedback
      // purpose
      final Object name = selection.get(0).getAttribute(ResourceObjectAdapter.NAME);
      ...
      if (selection.size() == 1)
        raiseTaskFlowFeedbackEvent(String.format(ADF.resourceBundleValue(BUNDLE, "OBJ_DELETE_FEEDBACK_SINGLE"), name));
      else
        raiseTaskFlowFeedbackEvent(String.format(ADF.resourceBundleValue(BUNDLE, "OBJ_DELETE_FEEDBACK_MULTIPLE"), selection.size()));
  ...
  private void launch(final String mode) {
    ...
    if (MODE_EDIT.equals(mode)) {
      ...
      objectKey  = (String)row.getAttribute(ResourceObjectAdapter.PK);
      objectName = (String)row.getAttribute(ResourceObjectAdapter.NAME);
    }

    final String              taskFlowId  = "obj" + objectKey;
    final String              regionTitle = MODE_EDIT.equals(mode) ? objectName : ADF.resourceBundleValue(BUNDLE, "OBJ_CREATE_TITLE");
    final Map<String, Object> parameter   = new HashMap<String, Object>();
    parameter.put(PARAMETER_MODE,             mode);
    parameter.put(PARAMETER_TASKFLOW,         taskFlowId);
    parameter.put(ResourceObjectAdapter.PK,   objectKey);
    parameter.put(ResourceObjectAdapter.NAME, objectName);
    raiseTaskFlowLaunchEvent(taskFlowId, "/WEB-INF/oracle/iam/identity/sysprov/obj/flow/train-tf.xml#train-tf", regionTitle, null, regionTitle, ConstantsDefinition.HELP_TOPIC_ID_ADM_CONFIG_APPINST, false, parameter);
  }
}