...
public class Picker extends AbstractPickerState {
  ...
  // compatible with
  @SuppressWarnings("compatibility:-2639422984830780279")
  private static final long         serialVersionUID = 5437636602671132156L;

  //////////////////////////////////////////////////////////////////////////////
  // instance attributes
  //////////////////////////////////////////////////////////////////////////////

  private List<EndpointTypeAdapter> selectedItems    = new ArrayList<EndpointTypeAdapter>();
   ...
   */
  public List<EndpointTypeAdapter> getSelectedItems() {
  ...
  public void addAllActionListener(final ActionEvent event) {
    final List<Row> selection = iteratorAllRow("EndpointTypeIterator");
  ...
  public void removeSelectedActionListener(final ActionEvent event) {
    ...
    List<EndpointTypeAdapter> removeRows = new ArrayList<EndpointTypeAdapter>();
    for (Iterator<Object> i = keys.iterator(); i.hasNext(); ) {
      Object key = i.next();
      rtt.setRowKey(key);
      EndpointTypeAdapter rowData = (EndpointTypeAdapter)rtt.getRowData();
  ...
  protected void addSelection(final List<Row> selection) {
    for (Row row : selection) {
      final EndpointTypeAdapter svd = new EndpointTypeAdapter(row);
      if (!getSelectedItems().contains(svd))
        getSelectedItems().add(svd);
    }
    refreshSelectedTargets();
  }
}