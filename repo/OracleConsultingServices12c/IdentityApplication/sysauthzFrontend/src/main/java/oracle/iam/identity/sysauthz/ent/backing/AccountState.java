 ...
 ** <code>Entitlement</code> with <code>Accounts</code>.
 **
 ** @author  dieter.steding@oracle.com
 ** @version 2.0.0.0
 ** @since   2.0.0.0
...
public class AccountState extends AbstractSearch {
  ...
  // compatible with
  @SuppressWarnings("compatibility:-4114714277835841565")
  private static final long serialVersionUID = -8998955670987719651L;
   ...
   ** Entitlement Instance for the account.
   **
  ...
  /**
   ** Raise a <code>Contextual Event</code> programmatically to launch the
   ** taskflow to assign accounts to the entitlement in scope.
   **
   ** @param  event              the {@link ActionEvent} delivered from the
   **                            action occured in a UIXForm component.
   */
  public void launchAssignRequest(final @SuppressWarnings("unused") ActionEvent event) {
    final String catalog = "request_entitlement";
    if (taskFlowOpen(catalog)) {
      ...
      List<Object> data = selectedTableData(getSearchTable());
      ...
      final String title = ADF.resourceBundleValue("oracle.iam.ui.OIMUIBundle", "access.request.entitlement.request");
  ...
  /**
   ** Raise a <code>Contextual Event</code> programmatically to launch the
   ** taskflow to revoke the selected accounts from the entitlement in scope.
   **
   ** @param  event              the {@link ActionEvent} delivered from the
   **                            action occured in a UIXForm component.
   */
  public void launchRevokeRequest(final @SuppressWarnings("unused") ActionEvent event) {
    final List<Object> data = selectedTableData(getSearchTable());
    if (data.size() != 0) {
      return;
    }
  }
  ...
  /**
   ** Raise a <code>Contextual Event</code> programmatically to launch the
   ** User Detail taskflow to render the details of the selected
