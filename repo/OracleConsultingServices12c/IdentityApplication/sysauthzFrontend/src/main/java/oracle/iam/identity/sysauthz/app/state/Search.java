...
public class Search extends AbstractSearchState {
  ...
  // compatible with
  @SuppressWarnings("compatibility:-5392432874830402399")
  private static final long   serialVersionUID = -6409574170145025930L;
  ...
  public Search() {
    // ensure inheritance
    super(ApplicationInstanceAdapter.PK, ApplicationInstanceAdapter.NAME);
  }
  ...
  /**
   ** Whether the selected row(s) (aka Application Instance) can be deleted or
   ** not.
   **
  ...
  public String getLocalizedMessage() {
    ...
    switch (localized) {
      case GENERIC_DELETE : return (selection.size() == 1)
                            ? String.format(ADF.resourceBundleValue(BUNDLE, "APP_DELETE_CONFIRM_SINGLE"), selection.get(0).getAttribute(ApplicationInstanceAdapter.NAME))
                            : String.format(ADF.resourceBundleValue(BUNDLE, "APP_DELETE_CONFIRM_MULTIPLE"), selection.size());
   ...
   ** to entries in the Application Instance table after confirmation.
   **
  ...
  public String refresh() {
    refresh("ApplicationInstanceIterator");
    return null;
  }
  ...
  /**
   ** Raise a <code>Contextual Event</code> programmatically to launch the
   ** taskflow to render the details of the selected <code>Application Instance</code>.
   **
   ** @return                    the outcome of the action to apply control
   **                            flow rules in the task flow.
   */
  ...
  public void delete(final List<Row> selection) {
    if (selection != null && !selection.isEmpty()) {
      // latch the name of the first Application Instance to delete for feedback
      // purpose
      final Object name = selection.get(0).getAttribute(ApplicationInstanceAdapter.NAME);
      ...
      if (selection.size() == 1)
        raiseTaskFlowFeedbackEvent(String.format(ADF.resourceBundleValue(BUNDLE, "APP_DELETE_FEEDBACK_SINGLE"), name));
      else
        raiseTaskFlowFeedbackEvent(String.format(ADF.resourceBundleValue(BUNDLE, "APP_DELETE_FEEDBACK_MULTIPLE"), selection.size()));
  ...
  private void launch(final String mode) {
    ...
    if (MODE_EDIT.equals(mode)) {
      ...
      key   = row.getAttribute(ApplicationInstanceAdapter.PK).toString();
      name  = row.getAttribute(ApplicationInstanceAdapter.NAME).toString();
      title = row.getAttribute(ApplicationInstanceAdapter.DISPLAY_NAME).toString();
    }

    final String              taskFlowId  = "app" + key;
    final String              regionTitle = MODE_EDIT.equals(mode) ?  title == null ? name : title : ADF.resourceBundleValue(BUNDLE, "APP_CREATE_TITLE");
    final Map<String, Object> parameter   = new HashMap<String, Object>();
    parameter.put(ApplicationInstanceAdapter.PK,           key);
    parameter.put(ApplicationInstanceAdapter.NAME,         name);
    parameter.put(ApplicationInstanceAdapter.DISPLAY_NAME, regionTitle);
    parameter.put(PARAMETER_MODE,                          mode);
    parameter.put(PARAMETER_TASKFLOW,                      taskFlowId);
    raiseTaskFlowLaunchEvent(taskFlowId, "/WEB-INF/oracle/iam/identity/sysauthz/app/flow/train-tf.xml#train-tf", regionTitle, null, regionTitle, ConstantsDefinition.HELP_TOPIC_ID_ADM_CONFIG_APPINST, false, parameter);
  }
}