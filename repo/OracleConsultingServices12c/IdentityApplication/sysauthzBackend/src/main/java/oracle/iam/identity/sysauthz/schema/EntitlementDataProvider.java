/*
    ...
    File        :   EntitlementDataProvider.java
 ...
 ** @since   2.0.0.0
 */
public class EntitlementDataProvider extends AbstractDataProvider<EntitlementAdapter> {
  ...
  // compatible with
  @SuppressWarnings("compatibility:1866461100877073849")
  private static final long serialVersionUID = 5969652763369707605L;
  ...
  // Method:   Ctor
  /**
   ** Constructs an empty <code>EntitlementDataProvider</code> data access
   ...
   ** Default Constructor
   */
  public EntitlementDataProvider() {
    // ensure inheritance
    super();
  }
   ...
   **                            element is of type {@link EntitlementAdapter}.
   */
  @Override
  public List<EntitlementAdapter> search(final SearchCriteria searchCriteria, final Set<String> requested, final HashMap<String, Object> control) {
    // default search is based on value
    SearchCriteria criteria = searchCriteria == null ? null : convertCriteria(searchCriteria);
    if (criteria == null) {
      criteria = new SearchCriteria(Entitlement.ENTITLEMENT_VALUE, "*", SearchCriteria.Operator.EQUAL);
    }

    // perform the search
    final EntitlementService      service   = service(EntitlementService.class);
    final HashMap<String, Object> parameter = convertParameter(control);
    try {
      return paginated(parameter, service.findEntitlements(criteria, parameter), ConstantsDefinition.UNKNOWN_ROW_COUNT);
    }
   ...
   */
  @Override
  public void update(final EntitlementAdapter mab) {
    final EntitlementService service = service(EntitlementService.class);
    try {
      Entitlement instance = mab.transfer();
      instance = service.updateEntitlement(instance);
    }
   ...
   **
   ** @return                    the {@link List} of applied changes.
   */
  public List<EntityPublication> assignPublication(final List<EntityPublication> publication) {
    final EntityPublicationService service = service(EntityPublicationService.class);
    try {
      return service.addEntityPublications(publication);
    }
    catch (Exception e) {
      throw new OIMRuntimeException(e);
    }
  ...
  private HashMap<String, Object> convertParameter(final HashMap<String, Object> parameter) {
    ...
    // default search is sorted on value and ascending
    if (parameter.containsKey(Entitlement.SORTEDBY)) {
      String value = parameter.get(Entitlement.SORTEDBY).toString();
      value = this.convertAttributeName(value);
      parameter.put(Entitlement.SORTEDBY,  value);
      parameter.put(Entitlement.SORTORDER, Searchable.SortOrder.ASCENDING);
    }
  ...
  private String convertAttributeName(final String outboundName) {
    ...
    // we need to convert both naming conventions for criteria attribute
    // objectKey
    // 1. the natural name is injected by the defined search criteria if the
    //    view object is used itself
    // 2. the prefixed name is injected by the view link if the view object is
    //     in a master-detail relation ship
    if (inboundName.equalsIgnoreCase("objectsKey") || inboundName.equalsIgnoreCase("Bind_objectsKey")) {
      inboundName = Entitlement.ENTITLEMENT_OBJECTKEY;
    }
    // we need to convert both naming conventions for criteria attribute
    // endpointKey
    // 1. the natural name is injected by the defined search criteria if the
    ...
    // 2. the prefixed name is injected by the view link if the view object is
    //     in a master-detail relation ship
    else if (inboundName.equalsIgnoreCase("endpointKey") || inboundName.equalsIgnoreCase("Bind_endpointKey")) {
      inboundName = Entitlement.ENTITLEMENT_ITRESOURCEKEY;
    }
    else {
   ...
   ** results list.
   ** <b>
   ** Each element in the returned list is an {@link EntitlementAdapter}.
   **
   ** @param  control            contains pagination, sort attribute, and sort
   **                            direction.
   ...
   */
  private List<EntitlementAdapter> paginated(final HashMap<String, Object> parameter, List<Entitlement> results, final long totalRowCount) {
    if (results == null)
      results = new ArrayList<Entitlement>();

    List<EntitlementAdapter> resultSet = new ArrayList<EntitlementAdapter>();
    for (Entitlement cursor : results) {
      EntitlementAdapter mab = new EntitlementAdapter();
      mab.transfer(cursor);
      resultSet.add(mab);
    }
    int rangeStart = 0;
    ...
    return new PagedArrayList<EntitlementAdapter>(resultSet, rangeStart, totalRowCount);
  }
}