/*
    ...
    File        :   SystemPermissionReconciliation.java
    ...
    Purpose     :   This file implements the class
                    SystemPermissionReconciliation.


    Revisions    Date        Editor      Comment
    ------------+-----------+-----------+-----------------------------------
    3.1.0.0      2014-06-21  DSteding    First release version
    3.2.0.0      2014-10-15  JBandeira   Prevent OIM default System Roles
                                         from being reconcile.
*/
...
////////////////////////////////////////////////////////////////////////////////
// class SystemPermissionReconciliation
// ~~~~~ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
/**
 ** The <code>SystemPermissionReconciliation</code> acts as the service end point for
 ** the Oracle Identity Manager to reconcile OIM roles information from Identity
 ** Manager itself.
 **
 ** @author  Dieter.Steding@oracle.com
 ** @version 3.1.0.0
 ** @since   3.1.0.0
 */
public class SystemPermissionReconciliation extends LookupReconciliation {
  ...
  // Method:   Ctor
  /**
   ** Constructs an empty <code>SystemPermissionReconciliation</code> scheduled
   ...
   ** Default Constructor
   */
  public SystemPermissionReconciliation() {
    // ensure inheritance
    super();
  }
  ...
  protected void populateChanges(final int bulkSize, final Set<String> returning)
    ...
    // prepare a filter that fetch only those roles that are not able to be
    // requested by the standard request approach
    final SearchCriteria filter1 = new SearchCriteria(
      new SearchCriteria(RoleManagerConstants.ROLE_NAME, "OPERATORS", SearchCriteria.Operator.EQUAL)
    , new SearchCriteria(RoleManagerConstants.ROLE_NAME, "SYSTEM ADMINISTRATORS", SearchCriteria.Operator.EQUAL)
    , SearchCriteria.Operator.OR
    );

    final SearchCriteria filter2 = new SearchCriteria(
      new SearchCriteria(RoleManagerConstants.ROLE_NAME, "SELF OPERATORS", SearchCriteria.Operator.EQUAL)
    , new SearchCriteria(RoleManagerConstants.ROLE_NAME, "Administrators", SearchCriteria.Operator.EQUAL)
    , SearchCriteria.Operator.OR
    );

    final SearchCriteria filter3 = new SearchCriteria(
      new SearchCriteria(RoleManagerConstants.ROLE_CATEGORY_NAME, "OIM Roles", SearchCriteria.Operator.EQUAL)
    , new SearchCriteria(RoleManagerConstants.ROLE_NAME, "ALL USERS", SearchCriteria.Operator.NOT_EQUAL)
    , SearchCriteria.Operator.AND
    );

    final SearchCriteria filter4 = new SearchCriteria(filter1, filter2 , SearchCriteria.Operator.OR);
    final SearchCriteria filter  = new SearchCriteria(filter3, filter4 , SearchCriteria.Operator.OR);

    // set up the attribute names to be returned from the search
    final String[]    attribute  = returning.toArray(new String[0]);
    // create the facade to use for fetching the roles
    final RoleManager facade     = this.targetService.getService(RoleManager.class);
    try {
