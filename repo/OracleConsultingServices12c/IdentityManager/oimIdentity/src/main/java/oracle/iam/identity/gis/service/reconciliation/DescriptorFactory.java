 ...
 ** @since   1.0.0.0
 */
final class DescriptorFactory extends TaskDescriptorFactory {
  ...
  enum Grammar {
    ...
    , PERMISSION(DESCRIPTOR,                              ELEMENT_ENTITLEMENT)
    , CONSTANT(new Grammar[] {DESCRIPTOR, PERMISSION},    ELEMENT_CONSTANT)
    , ATTRIBUTE(new Grammar[] {DESCRIPTOR, PERMISSION},   ELEMENT_ATTRIBUTE)
    , TRANSFORMER(new Grammar[] {DESCRIPTOR, PERMISSION}, ELEMENT_TRANSFORMER)
    ;
  ...
  private class Parser extends SAXInput {
    ...
    public void startElement(final String uri, final String localName, final String qualifiedName, final Attributes attributes)
      ...
      switch (this.cursor) {
        ...
        case CONSTANT    : // there must be a task descriptor on the value
                           ...
                           // to an IdentityEntityReference is a subclass of
                           // TaskDescriptor
                           //mapping = (AttributeMapping)peek();
        ...
        case LOOKUP      : // put the name of the attribute on the value stack
                           ...
                           break;
        case PERMISSION : // if we have a reference there must be an attribute
                          // name on the value stack
                          final DatabaseReference permission = new DatabaseReference(DescriptorFactory.this.descriptor);
                          permission.catalogueQuery(attributes.getValue(ATTRIBUTE_TYPE));
                          permission.transformationEnabled(StringUtility.stringToBool(attributes.getValue(TaskDescriptor.TRANSFORMATION_ENABLED)));
                          DescriptorFactory.this.descriptor.entitlement().put(attributes.getValue(ATTRIBUTE_NAME), permission);
                          // put the collection provider for attributes and
                          // their transformations on the value stack
                          push(permission);
                          break;
    ...
    public void endElement(final String uri, final String localName, final String qualifiedName)
      ...
      switch (this.cursor) {
        case DESCRIPTOR :
        case LOOKUP     :
        case PERMISSION : // remove the collection provider for attributes and
   ...
   **
   ** @return                    the {@link Descriptor} created from the
   **                            specified {@link File}.
   **
   ** @throws TaskException      in the event of misconfiguration (such as
   **                            failure to set an essential property) or if
