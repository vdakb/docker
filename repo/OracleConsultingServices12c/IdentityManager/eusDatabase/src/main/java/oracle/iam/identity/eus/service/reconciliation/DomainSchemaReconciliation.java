/*
    ...
    File        :   DomainSchemaReconciliation.java
...
////////////////////////////////////////////////////////////////////////////////
// class DomainSchemaReconciliation
// ~~~~~ ~~~~~~~~~~~~~~~~~~~~~~~~~~
/**
 ** The <code>DomainSchemaReconciliation</code> act as the service end point for
 ** the Identity Manager to reconcile user information from an Enterprise
 ** Domain.
 **
 ** @author  dieter.steding@oracle.com
 ** @version 3.1.0.0
 ** @since   1.0.0.0
 */
public class DomainSchemaReconciliation extends SchemaReconciliation {
   ...
   */
  private static final String   DOMAIN_LOOKUP = "Domain Lookup";
  ...
  , TaskAttribute.build(DOMAIN_LOOKUP,        TaskAttribute.MANDATORY)
    /** Name of the Enterprise Domain which is reconcilied */
  , TaskAttribute.build(DOMAIN_NAME,          TaskAttribute.MANDATORY)
  };
  ...
  // Method:   Ctor
  /**
   ** Constructs an empty <code>DomainSchemaReconciliation</code> scheduled task
   ...
   ** Default Constructor
   */
  public DomainSchemaReconciliation() {
    // ensure inheritance
    super();
  }
  ...
  protected void beforeExecution()
    ...
    String   decoded   = stringValue(DOMAIN_NAME);
    final String[] parameter = { stringValue(DOMAIN_LOOKUP), decoded };
    ...
    try {
      ...
      // check if the domain lookup contains a domain with the specified name
      // defined in the parameter of the scheduled job
      if (lookup.containsValue(decoded)) {
        this.domainSearchBase = lookup.encodedValue(decoded);
        if (this.connector().entitlementPrefixRequired())
          this.domainSearchBase = DirectoryConnector.unescapePrefix(this.domainSearchBase);
      }
      else {
