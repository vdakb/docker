...
public class AbstractDirectoryConnectorTest {
  ...
  public final Properties environment(final String contextPath, final boolean pooling)
    ...
    environment.put("com.sun.jndi.ldap.connect.pool",     pooling ? "true" : "false");

    if (Boolean.getBoolean((String)this.attribute.get(DirectoryResource.SECURE_SOCKET))) {
      environment.put(Context.SECURITY_PROTOCOL, DirectoryConstant.PROTOCOL_DEFAULT_SECURE);
  ...
  private static String stringValue(final Map<String, String> parameter, final String key) {
    final String result = (String)parameter.get(key);
   ...
   */
  private final void attributes(final Map<String, String> parameter) {
