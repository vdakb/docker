 ...
 ** @since   1.0.0.0
 */
public class DescriptorFactory extends TaskDescriptorFactory {
  ...
  enum Grammar {
    ...
    , CONSTANT(DESCRIPTOR,    ELEMENT_CONSTANT)
  ...
  private class Parser extends SAXInput {
    ...
    public void startElement(final String uri, final String localName, final String qualifiedName, final Attributes attributes)
      ...
      switch (this.cursor) {
        case DESCRIPTOR  : processDescriptor(DescriptorFactory.this.descriptor, attributes);
                           // put the collection for attributes and their
                           // transformations on the value stack
                           push(DescriptorFactory.this.descriptor);
                           break;
        case CONSTANT    : source = attributes.getValue(ATTRIBUTE_VALUE);
                           // there must be a descriptor on the value stack
                           mapping = ((TaskDescriptor)peek()).attributeMapping();
                           mapping.put(attributes.getValue(ATTRIBUTE_NAME), source);
                            break;
        case ATTRIBUTE   : source = attributes.getValue(ATTRIBUTE_SOURCE);
                           // there must be a descriptor on the value stack
                           mapping = ((TaskDescriptor)peek()).attributeMapping();
                           ...
                           if (StringUtility.isEmpty(destination))
                            ...
                            break;
        case TRANSFORMER : // there must be a descriptor on the value stack
    ...
    public void endElement(final String uri, final String localName, final String qualifiedName)
      ...
      // dispatch to handling method
      switch (this.cursor) {
        case DESCRIPTOR : // remove the current task descriptor from the value
                          // stack
                          pop();
                          break;
      }
  ...
  // Method:   Ctor
  /**
   ** Constructs an empty <code>DescriptorFactory</code>.
   **
  ...
  // Method:   configure
  /**
   ** Factory method to create a {@link Descriptor} from a
   ** {@link File}.
   **
   ** @param  descriptor         the {@link Descriptor} to be configured by this
   **                            {@link TaskDescriptorFactory}.
  ...
  public static void configure(final Descriptor descriptor, final File file, final boolean validate)
    ...
    try {
      create(descriptor, new FileInputStream(file), validate);
    }
    catch (IOException e) {
  ...
  public static void configure(final Descriptor descriptor, final InputStream stream)
    ...
    create(descriptor, stream, false);
  }
   ...
   **                            initialization fails.
   */
  public static void create(final Descriptor descriptor, final InputStream stream, final boolean validate)
    throws TaskException {
  ...
  public static void configure(final Descriptor descriptor, final InputSource source, final boolean validate)
    ...
    if (validate) {
      try {
        factory.validate(DescriptorFactory.class, source);
      }
  ...
  private void configure(final InputSource source)
    ...
    try {
      final Parser input = new Parser();
      input.processDocument(source);
    }
