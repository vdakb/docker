...
// ~~~~~ ~~~~~~~~~~~
/**
 ** Pre-Populate Adapter to fillup the user role field in any request data.
 **
 ** @author  dieter.steding@oracle.com
 ** @version 3.1.0.0
 ** @since   1.0.0.0
 */
public class RoleAdapter extends AbstractStringAdapter {
   ...
   **                            request.
   **
   ** @return                    the user role from the identity profile of
   **                            the beneficiary.
   */
  @Override
  public Serializable prepopulate(final RequestData requestData)
    ...
    try {
      return beneficiaryRole(requestData);
    }
    finally {
