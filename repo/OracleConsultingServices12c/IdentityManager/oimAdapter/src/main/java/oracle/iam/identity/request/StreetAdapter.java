...
// ~~~~~ ~~~~~~~~~~~~
/**
 ** Pre-Populate Adapter to fillup the street field in any request
 ** data.
 **
 ** @author  dieter.steding@oracle.com
 ** @version 3.1.0.0
 ** @since   1.0.0.0
 */
public class StreetAdapter extends AbstractStringAdapter {
   ...
   **                            request.
   **
   ** @return                    the state from the identity profile of
   **                            the beneficiary.
   */
  @Override
  public Serializable prepopulate(final RequestData requestData)
    ...
    try {
      return beneficiaryStreet(requestData);
    }
    finally {
