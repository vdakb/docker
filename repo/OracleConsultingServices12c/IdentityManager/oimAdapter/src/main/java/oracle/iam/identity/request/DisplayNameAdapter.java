...
// ~~~~~ ~~~~~~~~~~~~~~~~~~
/**
 ** Pre-Populate Adapter to fillup the display name field in any request data.
 **
 ** @author  dieter.steding@oracle.com
 ** @version 3.1.0.0
 ** @since   1.0.0.0
 */
public class DisplayNameAdapter extends AbstractStringAdapter {
   ...
   **                            request.
   **
   ** @return                    the display name from the identity profile of
   **                            the beneficiary.
   */
  @Override
  public Serializable prepopulate(final RequestData requestData)
    ...
    try {
      return beneficiaryDisplayName(requestData);
    }
    finally {
