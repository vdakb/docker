...
// ~~~~~ ~~~~~~~~~~~~~~~
/**
 ** The <code>DefaultUserRole</code> transform an Object to the Oracle Identity
 ** Manager default User Role <code>"Full-Time</code> if the Object to transform
 ...
 ** @since   1.0.0.0
 */
public class DefaultUserRole extends AbstractAttributeTransformer {
  ...
  // Method:   Ctor
  /**
   ** Constructs an <code>DefaultUserRole</code> which use the specified
   ** {@link Logger} for logging purpose.
   **
   ** @param  logger             the {@link Logger} for logging purpose.
   */
  public DefaultUserRole(final Logger logger) {
    // ensure inheritance
    super(logger);
  }
  ...
  public void transform(final String attributeName, final Map<String, Object> origin, final Map<String, Object> subject) {
    ...
    // role; otherwise return the converted value as is
    subject.put(attributeName, (StringUtility.isEmpty((String)value)) ? User.DEFAULT_ROLE : value);
  }
}