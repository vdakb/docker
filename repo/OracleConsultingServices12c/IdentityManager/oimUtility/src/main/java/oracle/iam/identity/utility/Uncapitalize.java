...
// ~~~~~ ~~~~~~~~~~~~
/**
 ** The <code>Uncapitalize</code> transform a String in a uncapitalized form,
 ** which means each word in the string is converted to upper case.
 **
 ** @author  dieter.steding@oracle.com
 ** @version 1.0.0.0
 ** @since   1.0.0.0
 */
public class Uncapitalize extends AbstractAttributeTransformer {
  ...
  // Method:   Ctor
  /**
   ** Constructs an <code>Uncapitalize</code> which use the specified
   ** {@link Logger} for logging purpose.
   **
   ** @param  logger             the {@link Logger} for logging purpose.
   */
  public Uncapitalize(final Logger logger) {
    // ensure inheritance
    super(logger);
  }
  ...
  public void transform(final String attributeName, final Map<String, Object> origin, final Map<String, Object> subject) {
    ...
    if (value == null)
      ...
      // if we not got an empty String return it as is otherwise capitalize the
      // the String
      subject.put(attributeName, (StringUtility.isEmpty(transform)) ? transform : StringUtility.uncapitalize(transform));
    }
  }
}