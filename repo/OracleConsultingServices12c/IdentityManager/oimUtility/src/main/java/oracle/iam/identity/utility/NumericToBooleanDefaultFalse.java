/*
    ...
    File        :   NumericToBooleanDefaultFalse.java
...
////////////////////////////////////////////////////////////////////////////////
// class NumericToBooleanDefaultFalse
// ~~~~~ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~
/**
 ** The <code>NumericToBoolean</code> transform a numeric value coming from a
 ** source to the Oracle Identity Manager Status <code>true</code> if the value
 ** is <code>1</code> otherwise the value is transformed to <code>false</code>.
 ** <p>
 ...
 ** @since   3.0.0.0
 */
public class NumericToBooleanDefaultFalse extends AbstractStatusTransformer {
  ...
  // Method:   Ctor
  /**
   ** Constructs an <code>NumericToBooleanDefaultFalse</code> which use the
   ** specified {@link Logger} for debugging purpose.
   **
   ** @param  logger             the {@link Logger} for debugging purpose
   */
  public NumericToBooleanDefaultFalse(final Logger logger) {
    // ensure inheritance
    super(logger);
  }
  ...
  public void transform(final String attributeName, final Map<String, Object> origin, final Map<String, Object> subject) {
    if (origin.containsKey(attributeName)) {
      ...
      if (value == null)
        ...
        value = ((BigDecimal)value).longValue() == 1L ? Boolean.TRUE : Boolean.FALSE;
      else if (value instanceof String) {
        // normalize the string to make the analysis easier what it can be
        final String expression = ((String)value).toLowerCase();
        // if we not got an empty value put false per default as the
        ...
        else if (SystemConstant.HEX_DIGITS[1] == expression.charAt(0))
          value = Boolean.TRUE;
        else
          value = Boolean.FALSE;
      }
