...
// ~~~~~ ~~~~~~~~~
/**
 ** The <code>LowerCase</code> transform a String to upper case.
 **
 ** @author  dieter.steding@oracle.com
 ** @version 1.0.0.0
 ** @since   1.0.0.0
 */
public class LowerCase extends AbstractAttributeTransformer {
  ...
  // Method:   Ctor
  /**
   ** Constructs an <code>LowerCase</code> which use the specified
   ** {@link Logger} for logging purpose.
   **
   ** @param  logger             the {@link Logger} for logging purpose.
   */
  public LowerCase(final Logger logger) {
    // ensure inheritance
    super(logger);
  }
  ...
  public void transform(final String attributeName, final Map<String, Object> origin, final Map<String, Object> subject) {
    ...
    if (value == null)
      ...
      // the String
      subject.put(attributeName, (StringUtility.isEmpty(transform)) ? transform : transform.toLowerCase());
    }
  }
}