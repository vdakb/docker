/*
    ...
    File        :   NumericToStatusDisabled.java
...
////////////////////////////////////////////////////////////////////////////////
// class NumericToStatusDisabled
// ~~~~~ ~~~~~~~~~~~~~~~~~~~~~~~
/**
 ** The <code>NumericToStatusDisabled</code> transform a numeric value coming
 ** from a source to the Oracle Identity Manager Status <code>Disabled</code> if
 ** the value is greater than <code>zero</code>.
 ** <p>
 ** This {@link AbstractStatusTransformer} is usefull if a source used by
 ** trusted or traget reconciliation for identities or organizations provides a
 ** numeric status information that has to be reflected accordingly in Oracle
 ** Identity Manager.
 **
 ** @author  dieter.steding@oracle.com
 ** @version 1.0.0.0
 ** @since   3.0.0.0
 */
public class NumericToStatusDisabled extends AbstractStatusTransformer {
  ...
  // Method:   Ctor
  /**
   ** Constructs an <code>NumericToStatusDisabled</code> which use the specified
   ** {@link Logger} for debugging purpose.
   **
   ** @param  logger             the {@link Logger} for debugging purpose
   */
  public NumericToStatusDisabled(final Logger logger) {
    // ensure inheritance
    super(logger);
  }
  ...
  public void transform(final String attributeName, final Map<String, Object> origin, final Map<String, Object> subject) {
    ...
    if (value == null)
      ...
      value = ((Integer)value).intValue() > 0 ? STATUS_DISABLED : STATUS_ENABLED;
    else if (value instanceof Long)
      value = ((Long)value).longValue() > 0   ? STATUS_DISABLED : STATUS_ENABLED;
    else if (value instanceof String) {
      // normalize the string to make the analysis easier what it can be
      value = Long.valueOf((String)value) > 0 ? STATUS_DISABLED : STATUS_ENABLED;
    }
    else
      value = STATUS_DISABLED;
