 ...
 ** @since   3.0.0.0
 */
public final class XMLIdentityFactory extends XMLEntityFactory<Identity> {
  ...
  private enum Grammar {
    ...
    , APPLICATIONS(IDENTITY,     ApplicationEntity.MULTIPLE)
    ...
    // compatible with
    @SuppressWarnings("compatibility:-6490797193526675803")
  ...
  private class Parser extends XMLEntityFactory<Identity>.Parser {
    ...
    public void startElement(final String uri, final String localName, final String qualifiedName, final Attributes attributes)
      ...
      switch (this.cursor) {
        ...
        case IDENTITY     : final Identity identity = new Identity(attributes.getValue(ATTRIBUTE_ID));
                            identity.action(attributes.getValue(ATTRIBUTE_ACTION));
                            push(identity);
                            break;
    ...
    private void dispatchRole()
      ...
      if (!collection.contains(role) || XMLIdentityFactory.this.unique == Unique.LAST) {
        identity.addRole(role);
      }
      else if (XMLIdentityFactory.this.unique == Unique.STRONG) {
        final int[]    position  = position();
    ...
    private void dispatchApplication()
      ...
      final Identity                identity    = (Identity)peek();
      final List<ApplicationEntity> collection  = identity.application();
      if (!collection.contains(application) || XMLIdentityFactory.this.unique == Unique.LAST) {
        identity.addApplication(application);
      }
      else if (XMLIdentityFactory.this.unique == Unique.STRONG) {
        final int[]    position  = position();
    ...
    private void dispatchEntitlements()
      ...
      final List<EntitlementEntity> entity      = (List<EntitlementEntity>)pop();
      final String                  namespace   = (String)pop();
      final ApplicationEntity       application = (ApplicationEntity)peek();
      application.add(namespace, entity);
    }
    ...
    private void dispatchIdentity()
      ...
      if (!bulk.contains(identity) || XMLIdentityFactory.this.unique == Unique.LAST) {
        bulk.add(identity);
      }
      else if (XMLIdentityFactory.this.unique == Unique.STRONG) {
        final int[]    position  = position();
      ...
      if (bulk.size() >= XMLIdentityFactory.this.listener.batchSize()) {
        dispatchBulk();
    ...
    private void dispatchBulk()
      ...
      try {
        XMLIdentityFactory.this.listener.process(bulk);
      }
  ...
  // Method:   Ctor
  /**
   ** Constructs a <code>XMLIdentityFactory</code> to populate identity data
   ...
   **                            the source.
   */
  public XMLIdentityFactory(final Unique unique) {
    // ensure inheritance
   ...
   ** {@link XMLIdentityFactory.Parser} and populates the {@link Identity}
   ** elements.
   **
   **
   ** @param  listener           the {@link EntityListener} that will
