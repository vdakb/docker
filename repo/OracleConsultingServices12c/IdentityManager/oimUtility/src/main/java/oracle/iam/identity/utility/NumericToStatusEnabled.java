/*
    ...
    File        :   NumericToStatusEnabled.java
...
////////////////////////////////////////////////////////////////////////////////
// class NumericToStatusEnabled
// ~~~~~ ~~~~~~~~~~~~~~~~~~~~~~
/**
 ** The <code>NumericToStatusEnabled</code> transform a numeric value coming
 ** from a source to the Oracle Identity Manager Status <code>Enabled</code> if
 ...
 ** @since   3.0.0.0
 */
public class NumericToStatusEnabled extends AbstractStatusTransformer {
  ...
  // Method:   Ctor
  /**
   ** Constructs an <code>NumericToStatusEnabled</code> which use the specified
   ** {@link Logger} for debugging purpose.
   **
   ** @param  logger             the {@link Logger} for debugging purpose
   */
  public NumericToStatusEnabled(final Logger logger) {
    // ensure inheritance
    super(logger);
  }
  ...
  public void transform(final String attributeName, final Map<String, Object> origin, final Map<String, Object> subject) {
    ...
    if (value == null)
      ...
      value = ((Integer)value).intValue() > 0 ? STATUS_ENABLED : STATUS_DISABLED;
    else if (value instanceof Long)
      value = ((Long)value).longValue() > 0   ? STATUS_ENABLED : STATUS_DISABLED;
    else if (value instanceof String) {
      // normalize the string to make the analysis easier what it can be
      value = Long.valueOf((String)value) > 0 ? STATUS_ENABLED : STATUS_DISABLED;
    }
    else
      value = STATUS_ENABLED;
