...
// ~~~~~ ~~~~~~~~~~~~~
/**
 ** The <code>Base64Decoder</code> transform a String representing in a base 64
 ** representation.
 ** <p>
 ...
 ** @see Base64Decoder
 */
public class Base64Encoder extends AbstractAttributeTransformer {
  ...
  // Method:   Ctor
  /**
   ** Constructs an <code>Base64Encoder</code> which use the specified
   ** {@link Logger} for logging purpose.
   **
   ** @param  logger             the {@link Logger} for logging purpose.
   */
  public Base64Encoder(final Logger logger) {
    // ensure inheritance
    super(logger);
  }
  ...
  public void transform(final String attributeName, final Map<String, Object> origin, final Map<String, Object> subject) {
    ...
    if (value == null)
      ...
      subject.put(attributeName, (StringUtility.isEmpty(transform)) ? transform : Base64.encodeToString(transform.getBytes(), true));
    }
  }
}