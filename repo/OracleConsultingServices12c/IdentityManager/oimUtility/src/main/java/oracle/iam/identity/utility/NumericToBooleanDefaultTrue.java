/*
    Oracle Deutschland GmbH

    This software is the confidential and proprietary information of
    Oracle Corporation. ("Confidential Information").  You shall not
    disclose such Confidential Information and shall use it only in
    accordance with the terms of the license agreement you entered
    into with Oracle.

    ORACLE MAKES NO REPRESENTATIONS OR WARRANTIES ABOUT THE SUITABILITY OF THE
    SOFTWARE, EITHER EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO THE
    IMPLIED WARRANTIES OF MERCHANTABILITY, FITNESS FOR A PARTICULAR
    PURPOSE, OR NON-INFRINGEMENT. ORACLE SHALL NOT BE LIABLE FOR ANY DAMAGES
    SUFFERED BY LICENSEE AS A RESULT OF USING, MODIFYING OR DISTRIBUTING
    THIS SOFTWARE OR ITS DERIVATIVES.

    Copyright © 2006. All Rights reserved

    -----------------------------------------------------------------------

    System      :   Oracle Identity Manager Foundation Library
    Subsystem   :   Common Shared Utility Facilities

    File        :   NumericToBooleanDefaultTrue.java

    Compiler    :   Oracle JDeveloper 12c

    Author      :   Dieter.Steding@oracle.com

    Purpose     :   This file implements the class
                    NumericToBooleanDefaultTrue.


    Revisions    Date        Editor      Comment
    ------------+-----------+-----------+-----------------------------------
    1.0.0.0      2012-04-29  DSteding    First release version
*/

package oracle.iam.identity.utility;

import java.util.Map;

import java.math.BigDecimal;

import oracle.hst.foundation.SystemConstant;

import oracle.hst.foundation.logging.Logger;

import oracle.hst.foundation.utility.StringUtility;

////////////////////////////////////////////////////////////////////////////////
// class NumericToBooleanDefaultTrue
// ~~~~~ ~~~~~~~~~~~~~~~~~~~~~~~~~~~
/**
 ** The <code>NumericToBooleanDefaultTrue</code> transform a numeric value
 ** coming from a source to the Oracle Identity Manager Status <code>true</code>
 ** if the value is <code>1</code>. All other values are transformed to
 ** <code>false</code>.
 ** <br>
 ** If no value is provided to the transformer either by not existing or set to
 ** <code>null</code> the returned values in the mapping will be
 ** <code>true</code>
 ** <p>
 ** This {@link AbstractStatusTransformer} is usefull if a source used by
 ** trusted reconciliation for identities or organizations provides a boolean
 ** status information that has to be reflected accordingly in Oracle Identity
 ** Manager.
 **
 ** @author  dieter.steding@oracle.com
 ** @version 1.0.0.0
 ** @since   3.0.0.0
 */
public class NumericToBooleanDefaultTrue extends AbstractStatusTransformer {

  //////////////////////////////////////////////////////////////////////////////
  // Constructors
  //////////////////////////////////////////////////////////////////////////////

  //////////////////////////////////////////////////////////////////////////////
  // Method:   Ctor
  /**
   ** Constructs an <code>NumericToBooleanDefaultTrue</code> which use the
   ** specified {@link Logger} for debugging purpose.
   **
   ** @param  logger             the {@link Logger} for debugging purpose
   */
  public NumericToBooleanDefaultTrue(final Logger logger) {
    // ensure inheritance
    super(logger);
  }

  //////////////////////////////////////////////////////////////////////////////
  // Methods of implemented interfaces
  //////////////////////////////////////////////////////////////////////////////

  //////////////////////////////////////////////////////////////////////////////
  // Method:   transform (AttributeTransformer)
  /**
   ** Returns the specified <code>origin</code> as an appropriate
   ** transformation in <code>subject</code>.
   ** <p>
   ** The {@link Map} <code>origin</code> contains all untouched values and has
   ** it to be after this method completes. The {@link Map} <code>subject</code>
   ** contains all transformed values hence the transformation done here too.
   ** <p>
   ** <b>Attention:</b>
   ** <br>
   ** This transformer is a weak implementation to enuser that a
   ** <code>attributeName</code> which should be transformed has to be contained
   ** in {@link Map} <code>origin</code>. If <code>attributeName</code> isn't
   ** contained in {@link Map} <code>origin</code> no transformation will
   ** happend and therfore no value will be put in {@link Map}
   ** <code>subject</code>.
   **
   ** @param  attributeName      the specific attribute in the {@link Map}
   **                            <code>origin</code> that has to be transformed.
   ** @param  origin             the {@link Map} to transform.
   ** @param  subject            the transformation of the specified
   **                            {@link Map} <code>origin</code>.
   */
  @Override
  public void transform(final String attributeName, final Map<String, Object> origin, final Map<String, Object> subject) {
    if (origin.containsKey(attributeName)) {
      Object value = origin.get(attributeName);
      // if we not got a null value put it without transformation in the returning
      // container
      if (value == null)
        value = Boolean.TRUE;
      else if (value instanceof Integer)
        value = ((Integer)value).intValue() == 0 ? Boolean.FALSE : Boolean.TRUE;
      else if (value instanceof Long)
        value = ((Long)value).longValue() == 0L ? Boolean.FALSE : Boolean.TRUE;
      else if (value instanceof BigDecimal)
        value = ((BigDecimal)value).longValue() == 0L ? Boolean.FALSE : Boolean.TRUE;
      else if (value instanceof String) {
        // normalize the string to make the analysis easier what it can be
        final String expression = ((String)value).toLowerCase();
        // if we not got an empty value put true per default as the
        // transformation result in the returning container
        if (StringUtility.isEmpty(expression))
          value = Boolean.TRUE;
        // not all sources providing it like the Java primitive boolean hence we
        // convert what we know to a Java boolean primitive
        else if (SystemConstant.HEX_DIGITS[0] == expression.charAt(0))
          value = Boolean.FALSE;
        else
          value = Boolean.TRUE;
      }
      else
        value = Boolean.TRUE;

      // return the converted value as is
      subject.put(attributeName, value);
    }
  }
}