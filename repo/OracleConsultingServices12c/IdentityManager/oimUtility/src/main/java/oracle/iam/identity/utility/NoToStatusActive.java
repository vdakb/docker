...
// ~~~~~ ~~~~~~~~~~~~~~~~~
/**
 ** The <code>NoToStatusActive</code> transform a Status <code>N</code> or
 ** <code>NO</code> to the Oracle Identity Manager <code>Disabled</code>.
 ** <br>
 ...
 ** @since   3.0.0.0
 */
public class NoToStatusActive extends AbstractStatusTransformer {
  ...
  // Method:   Ctor
  /**
   ** Constructs an <code>NoToStatusActive</code> which use the specified
   ** {@link Logger} for debugging purpose.
   **
   ** @param  logger             the {@link Logger} for debugging purpose
   */
  public NoToStatusActive(final Logger logger) {
    // ensure inheritance
    super(logger);
  }
  ...
  public void transform(final String attributeName, final Map<String, Object> origin, final Map<String, Object> subject) {
    ...
    subject.put(attributeName, (STATUS_N.equalsIgnoreCase(value.toString()) || STATUS_NO.equalsIgnoreCase(value.toString())) ? STATUS_DISABLED : STATUS_ACTIVE);
  }
}