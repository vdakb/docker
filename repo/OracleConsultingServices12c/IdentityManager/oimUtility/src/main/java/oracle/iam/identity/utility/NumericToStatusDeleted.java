/*
    ...
    File        :   NumericToStatusDeleted.java
...
////////////////////////////////////////////////////////////////////////////////
// class NumericToStatusDeleted
// ~~~~~ ~~~~~~~~~~~~~~~~~~~~~~
/**
 ** The <code>NumericToStatusDeleted</code> transform a numeric value coming
 ** from a source to the Oracle Identity Manager Status <code>Deleted</code> if
 ** the value is <code>true</code>.
 ** <p>
 ** This {@link AbstractStatusTransformer} is usefull if a source used by
 ** trusted reconciliation for identities or organizations provides a numeric
 ...
 ** @since   3.0.0.0
 */
public class NumericToStatusDeleted extends AbstractStatusTransformer {
  ...
  // Method:   Ctor
  /**
   ** Constructs an <code>NumericToStatusDeleted</code> which use the specified
   ** {@link Logger} for debugging purpose.
   **
   ** @param  logger             the {@link Logger} for debugging purpose
   */
  public NumericToStatusDeleted(final Logger logger) {
    // ensure inheritance
    super(logger);
  }
  ...
  public void transform(final String attributeName, final Map<String, Object> origin, final Map<String, Object> subject) {
    ...
    if (value == null)
      ...
      value = ((Integer)value).intValue() > 0 ? STATUS_DELETED : STATUS_DISABLED;
    else if (value instanceof Long)
      value = ((Long)value).longValue() > 0   ? STATUS_DELETED : STATUS_DISABLED;
    else if (value instanceof String) {
      // normalize the string to make the analysis easier what it can be
      value = Long.valueOf((String)value) > 0 ? STATUS_DELETED : STATUS_DISABLED;
    }
    else
      value = STATUS_DELETED;
