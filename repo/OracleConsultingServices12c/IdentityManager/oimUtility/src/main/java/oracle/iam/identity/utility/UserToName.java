...
// ~~~~~ ~~~~~~~~~~
/**
 ** The <code>UserToName</code> transforms a given the system identifier of a
 ** user to its login name by looking up the user with the given system
 ** identifier.
 **
 ** @author  dieter.steding@oracle.com
 ** @version 1.0.0.0
 ** @since   3.0.0.0
 */
public class UserToName extends AbstractStatusTransformer {
   ...
   ** internal identifier of a <code>User</code>.
   */
  private static final String LOOKUP_USER = "select usr_login from usr where usr_key = ?";
  ...
  // Method:   Ctor
  /**
   ** Constructs an <code>UserToName</code> which use the specified
   ** {@link Logger} for debugging purpose.
   **
   ** @param  logger             the {@link Logger} for debugging purpose
   */
  public UserToName(final Logger logger) {
    // ensure inheritance
    super(logger);
  }
   ...
   ** Identity Manager.
   **
   ** @param  identifier         the internal identifier of an identity to
   **                            lookup.
   **
   ** @return                    the login name  of the
   **                            <code>User</code> that internal identifier
   **                            match the specified parameter
   **                            <code>identifier</code> or <code>null</code> if
   **                            no <code>User</code>s internal identifier match
