...
// ~~~~~ ~~~~~~~~~~
/**
 ** The <code>CapitalizeTransformer</code> transform a String in a capitalized
 ** form, which means each word in the string is converted to upper case.
 **
 ** @author  dieter.steding@oracle.com
 ** @version 1.0.0.0
 ** @since   1.0.0.0
 */
public class Capitalize extends AbstractAttributeTransformer {
  ...
  // Method:   Ctor
  /**
   ** Constructs an <code>Capitalize</code> which use the specified
   ** {@link Logger} for logging purpose.
   **
   ** @param  logger             the {@link Logger} for logging purpose.
   */
  public Capitalize(final Logger logger) {
    // ensure inheritance
    super(logger);
  }
  ...
  public void transform(final String attributeName, final Map<String, Object> origin, final Map<String, Object> subject) {
    ...
    if (value == null)
      ...
      // the String
      subject.put(attributeName, (StringUtility.isEmpty(transform)) ? transform : StringUtility.capitalize(transform));
    }
  }
}