...
// ~~~~~ ~~~~~~~~~~~
/**
 ** <code>Publication</code> defines the attribute restriction on values that
 ** can be passed as a nested parameter to deployment import instance.
 **
 ** @author  dieter.steding@oracle.com
 ** @version 1.0.0.0
 ** @since   1.0.0.0
 */
public class Publication extends EnumeratedAttribute {
  ...
  private static String[] OPERATION = { ServiceOperation.enable.id(), ServiceOperation.disable.id() };

  // the names of the exportable/importableobject definitions in alphabetical order
  private static String[] RECIPIENT = { OrganizationManagerConstants.ORGANIZATION_ENTITY };
  ...
  public static class Recipient extends EnumeratedAttribute {
    ...
    /**
     ** Returns the name of the attribute used in searches that matches the
     ** particular type of this recipient.
     **
     ** @return                  the name of the attribute used in searches that
     **                          matches the particular type of this recipient.
     */
    public final String searchAttribute() {
      if (OrganizationManagerConstants.ORG_ENTITY_TYPE.equals(this.value)) {
        return OrganizationManagerConstants.AttributeName.ORG_NAME.getId();
      }
      else if (RoleManagerConstants.ROLE_ENTITY_NAME.equals(this.value)) {
        return RoleManagerConstants.ROLE_NAME;
      }
      else if (UserManagerConstants.USER_ENTITY.equals(this.value)) {
        return UserManagerConstants.AttributeName.USER_LOGIN.getId();
      }
      throw new BuildException(this.value + " is not a legal value for this recipient");
    }
  ...
  // Method:   Ctor
  /**
   ** Constructs a <code>Publication</code> type that allows use as a JavaBean.
   ** <br>
