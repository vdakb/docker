...
// ~~~~~ ~~~~~~~~~~~~~~~
/**
 ** The <code>RoleTransformer</code> transform a numeric value coming from a
 ** source to the name of a role.
 **
 ** @author  dieter.steding@oracle.com
 ** @version 3.0.0.0
 ** @since   3.0.0.0
 */
public class RoleTransformer extends AbstractAttributeTransformer {
  ...
  // Method:   Ctor
  /**
   ** Constructs an <code>RoleTransformer</code> transformer which use the
   ** specified {@link Logger} for debugging purpose.
   **
   ** @param  logger             the {@link Logger} for debugging purpose
   */
  public RoleTransformer(final Logger logger) {
    // ensure inheritance
    super(logger);
  }
  ...
  public void transform(final String attributeName, final Map<String, Object> origin, final Map<String, Object> subject) {
    ...
    try {
      ...
      statement  = DatabaseStatement.createPreparedStatement(connection, "select ugp_name from ugp where ugp_key = ?");
