...
public final class DescriptorFactory extends TaskDescriptorFactory {
  ...
  public static final String ATTRIBUTE_SEARCH_BASE     = "search-base";
  public static final String ATTRIBUTE_ENTITY_LINK     = "entity-link";
  public static final String ATTRIBUTE_ENTITY_NAME     = "entity-name";
  public static final String ATTRIBUTE_ENTITY_PREFIXED = "entity-prefixed";
  ...
  enum Grammar {
    ...
    , CONSTANT(new Grammar[] {DESCRIPTOR, REFERENCE},    ELEMENT_CONSTANT)
    ...
    // compatible with
    @SuppressWarnings("compatibility:2370857512067815366")
  ...
  private class Parser extends SAXInput {
    ...
    public void startElement(final String uri, final String localName, final String qualifiedName, final Attributes attributes)
      ...
      switch (this.cursor) {
        ...
        case ATTRIBUTE   : final String source = attributes.getValue(ATTRIBUTE_SOURCE);
                           ...
                           break;
        case MULTIVALUED : // there must be a descriptor on the value stack
        ...
        case LOOKUP      : // put the name of the attribute on the value stack
                           ...
                           break;
        case REFERENCE   : final DirectoryReference reference = DirectoryReference.build(DescriptorFactory.this.descriptor);
                           reference.objectClassName(attributes.getValue(ATTRIBUTE_OBJECT_CLASS));
                           reference.entrySearchBase(attributes.getValue(ATTRIBUTE_SEARCH_BASE));
                           reference.entryLinkAttribute(attributes.getValue(ATTRIBUTE_ENTITY_LINK));
                           reference.entryLinkStandard(SystemConstant.TRUE.equals(attributes.getValue(ATTRIBUTE_ENTITY_NAME)));
                           reference.entryLinkPrefixed(SystemConstant.TRUE.equals(attributes.getValue(ATTRIBUTE_ENTITY_PREFIXED)));
                           reference.transformationEnabled(StringUtility.stringToBool(attributes.getValue(TaskDescriptor.TRANSFORMATION_ENABLED)));
                           DescriptorFactory.this.descriptor.entitlement().put(attributes.getValue(ATTRIBUTE_NAME), reference);
                           // put the collection provider for attributes and
                           // their transformations on the value stack
                           push(reference);
                           break;
    ...
    public void endElement(final String uri, final String localName, final String qualifiedName)
      ...
      switch (this.cursor) {
        case DESCRIPTOR :
        case LOOKUP     :
        case REFERENCE  : // remove the collection provider for attributes and
