...
public final class DescriptorFactory extends TaskDescriptorFactory {
  ...
  enum Grammar {
      ...
      /** the encodes valid state transitions in the parser. */
      INIT(new Grammar[0], "")
    ...
    // compatible with
    @SuppressWarnings("compatibility:9112704529257514159")
    ...
    public void startElement(final String uri, final String localName, final String qualifiedName, final Attributes attributes)
      ...
      switch (this.cursor) {
        ...
        case TRANSFORMER : // there must be a task descriptor on the value
                           ...
                           mapping.put(attributes.getValue(ATTRIBUTE_NAME), attributes.getValue(ATTRIBUTE_CLASS));
                           break;
        case REFERENCE   : // if we have a reference there must be an attribute
                           // name on the value stack
                           final DirectoryReference reference = DirectoryReference.build(DescriptorFactory.this.descriptor);
