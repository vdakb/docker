...
public final class DescriptorFactory extends TaskDescriptorFactory {
  ...
  /**
   ** the XML element that identifies an multi-valued attribute in Oracle
   ** Identity Manager.
   */
  public static final String ELEMENT_MULTIVALUED = "multivalued";

  /**
   ** the XML element that identifies an entitlement attribute in Oracle
   ** Identity Manager.
   */
  ...
  enum Grammar {
    ...
    , ATTRIBUTE(new Grammar[]{DESCRIPTOR,   MULTIVALUED, ENTITLEMENT}, ELEMENT_ATTRIBUTE)
    , TRANSFORMER(new Grammar[]{DESCRIPTOR, MULTIVALUED, ENTITLEMENT}, ELEMENT_TRANSFORMER)
    ;
    ...
    // compatible with
    @SuppressWarnings("compatibility:-725845574565286709")
  ...
  private class Parser extends SAXInput {
    ...
    public void startElement(final String uri, final String localName, final String qualifiedName, final Attributes attributes)
      ...
      FlatFileReference reference = null;
      switch (this.cursor) {
        ...
        case TRANSFORMER : // there must be a task descriptor on the value
                           ...
                           break;
        case MULTIVALUED : // verify the the multi-valued attribute is contained
                           // in the attributes which will be fetched from the
                           // datasource because it's required that the
                           // attribute is included by nature of a flat file
                           // there must be a descriptor on the value stack
                           mapping = ((TaskDescriptor)peek()).attributeMapping();
                           if (!mapping.containsKey(attributes.getValue(ATTRIBUTE_SOURCE)))
                             // put the attribute in the attribute mapping if
                             // its missing
                             mapping.put(attributes.getValue(ATTRIBUTE_SOURCE), attributes.getValue(ATTRIBUTE_NAME));
                           // create a reference to receive the complete
                           // description of the multi-valued attribute
                           reference = new FlatFileReference(DescriptorFactory.this.descriptor);
                           reference.entryLinkAttribute(attributes.getValue(ATTRIBUTE_SOURCE));
                           reference.transformationEnabled(StringUtility.stringToBool(attributes.getValue(TaskDescriptor.TRANSFORMATION_ENABLED)));
                           ((Descriptor)peek()).multivalued().put(attributes.getValue(ATTRIBUTE_NAME), reference);
                           // put the collection provider for attributes and
                           // their transformations on the value stack
                           push(reference);
                           break;
        case ENTITLEMENT : // verify the the multi-valued attribute is contained
                           // in the attributes which will be fetched from the
                           // datasource because it's required that the
                           // attribute is included by nature of a flat file
                           // there must be a descriptor on the value stack
                           ...
                           if (!mapping.containsKey(attributes.getValue(ATTRIBUTE_SOURCE)))
                             ...
                             mapping.put(attributes.getValue(ATTRIBUTE_SOURCE), attributes.getValue(ATTRIBUTE_NAME));
                           // create a reference to receive the complete
                           // description of the entitlement attribute
                           reference = new FlatFileReference(DescriptorFactory.this.descriptor);
                           reference.entryLinkAttribute(attributes.getValue(ATTRIBUTE_SOURCE));
                           reference.transformationEnabled(StringUtility.stringToBool(attributes.getValue(TaskDescriptor.TRANSFORMATION_ENABLED)));
                           ((Descriptor)peek()).entitlement().put(attributes.getValue(ATTRIBUTE_NAME), reference);
                           // put the collection provider for attributes and
                           // their transformations on the value stack
                           push(reference);
                           break;
