...
// ~~~~~ ~~~~~
/**
 ** The <code>Czech</code> {@link Alphabet} dictionary generator for textual
 ** <code>CAPTCHA</code>s.
 ** <br>
 ** Used for translation resources to
 ** <ul>
 **   <li>language code czech (cs)
 ...
 ** @since   1.0.0.0
 */
public class Czech extends Alphabet {
  ...
  // Method:   Ctor
  /**
   ** Constructs a <code>Czech</code> {@link Alphabet}.
   **
  ...
  protected void initializeLetter() {
    ...
    // extend the dictionary with the cech special characters
    ...
    this.letter.add(entry('\u010D', '\u010C')); // č
    this.letter.add(entry('\u010E', '\u010E')); // Ď
    this.letter.add(entry('\u010F', '\u010E')); // ď
    this.letter.add(entry('\u0147', '\u0148')); // Ň
    this.letter.add(entry('\u0148', '\u0148')); // ň
    this.letter.add(entry('\u0158', '\u0158')); // Ř
    this.letter.add(entry('\u0159', '\u0158')); // ř
    this.letter.add(entry('\u0160', '\u0160')); // Š
    this.letter.add(entry('\u0161', '\u0160')); // š
    this.letter.add(entry('\u0164', '\u0164')); // Ť
    this.letter.add(entry('\u0165', '\u0164')); // ť
  ...
  // Method:   build
  /**
   ** Factory method to create an <code>Czech</code> {@link Alphabet}.
   ** properties.
   **
   ** @param  path               the abstract path to spool the configuration
   **                            file.
   ...
   **                            Allowed object is {@link File}.
   **
   ** @return                    <code>Czech</code> {@link Alphabet}
   **                            <br>
   **                            Possible object is <code>Czech</code>.
   */
  public static Czech build(final File path) {
    return new Czech(path);
  }
}