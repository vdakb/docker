...
// ~~~~~ ~~~~~~~~
/**
 ** The <code>Romanian</code> {@link Alphabet} dictionary generator for
 ** textual <code>CAPTCHA</code>s.
 ** <br>
 ** Used for translation resources to
 ** <ul>
 **   <li>language code romanian (ro)
 ...
 ** @since   1.0.0.0
 */
public class Romanian extends Alphabet {
  ...
  // Method:   Ctor
  /**
   ** Constructs a <code>Romanian</code> {@link Alphabet}.
   **
   ...
   */
  private Romanian(final File path) {
    // ensure inheritance
    super(path, "ro");
  }
  ...
  protected void initializeLetter() {
    ...
    // extend the dictionary with the romanian special characters
    this.letter.add(entry('\u00C2', '\u00C2')); // Â
    this.letter.add(entry('\u00E2', '\u00C2')); // â
    this.letter.add(entry('\u00CE', '\u00CE')); // Î
    this.letter.add(entry('\u00EE', '\u00CE')); // î
    this.letter.add(entry('\u0102', '\u0102')); // Ă
    this.letter.add(entry('\u0103', '\u0102')); // ă
    this.letter.add(entry('\u0218', '\u0218')); // Ș
    this.letter.add(entry('\u0219', '\u0218')); // ș
    this.letter.add(entry('\u021A', '\u021A')); // Ț
    this.letter.add(entry('\u021B', '\u021A')); // ț
 }
  ...
  // Method:   build
  /**
   ** Factory method to create an <code>Romanian</code> {@link Alphabet}.
   ** properties.
   **
   ** @param  path               the abstract path to spool the configuration
   **                            file.
   ...
   **                            Allowed object is {@link File}.
   **
   ** @return                    <code>Romanian</code> {@link Alphabet}
   **                            <br>
   **                            Possible object is <code>Romanian</code>.
   */
  public static Romanian build(final File path) {
    return new Romanian(path);
  }
}