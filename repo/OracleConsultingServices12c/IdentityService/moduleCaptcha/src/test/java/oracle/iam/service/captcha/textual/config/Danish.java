...
// ~~~~~ ~~~~~~
/**
 ** The <code>Danish</code> {@link Alphabet} dictionary generator for textual
 ** <code>CAPTCHA</code>s.
 ** <br>
 ** Used for translation resources to
 ** <ul>
 **   <li>country  code danish (da)
 ...
 ** @since   1.0.0.0
 */
public class Danish extends Alphabet {
  ...
  // Method:   Ctor
  /**
   ** Constructs a <code>Danish</code> {@link Alphabet}.
   **
   ...
   */
  private Danish(final File path) {
    // ensure inheritance
    super(path, "da");
  }
  ...
  protected void initializeLetter() {
    ...
    // extend the dictionary with the danish special characters
    this.letter.add(entry('\u00C5', '\u00C5')); // Å
    this.letter.add(entry('\u00C6', '\u00C6')); // Æ
    this.letter.add(entry('\u00D8', '\u00D8')); // Ø
    this.letter.add(entry('\u00E5', '\u00C5')); // å
    this.letter.add(entry('\u00E6', '\u00C6')); // æ
    this.letter.add(entry('\u00F8', '\u00D8')); // ø
  }
  ...
  // Method:   build
  /**
   ** Factory method to create an <code>Danish</code> {@link Alphabet}.
   ** properties.
   **
   ** @param  path               the abstract path to spool the configuration
   **                            file.
   ...
   **                            Allowed object is {@link File}.
   **
   ** @return                    <code>Danish</code> {@link Alphabet}
   **                            <br>
   **                            Possible object is <code>Danish</code>.
   */
  public static Danish build(final File path) {
    return new Danish(path);
  }
}