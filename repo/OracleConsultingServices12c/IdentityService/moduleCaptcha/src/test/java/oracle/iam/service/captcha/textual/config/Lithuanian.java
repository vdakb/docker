...
// ~~~~~ ~~~~~~~
/**
 ** The <code>Lithuanian</code> {@link Alphabet} dictionary generator for
 ** textual <code>CAPTCHA</code>s.
 ** <br>
 ** Used for translation resources to
 ** <ul>
 **   <li>language code lithuanian (lt)
 ...
 ** @since   1.0.0.0
 */
public class Lithuanian extends Alphabet {
  ...
  // Method:   Ctor
  /**
   ** Constructs a <code>Lithuanian</code> {@link Alphabet}.
   **
   ...
   */
  private Lithuanian(final File path) {
    // ensure inheritance
    super(path, "lt");
  }
  ...
  protected void initializeLetter() {
    ...
    // extend the dictionary with the lithuanian special characters
    this.letter.add(entry('\u0104', '\u0104')); // Ą
    this.letter.add(entry('\u0105', '\u0104')); // ą
    this.letter.add(entry('\u010C', '\u010C')); // Č
    this.letter.add(entry('\u010D', '\u010C')); // č
    this.letter.add(entry('\u0116', '\u0116')); // Ė
    this.letter.add(entry('\u0117', '\u0116')); // ė
    this.letter.add(entry('\u0118', '\u0118')); // Ę
    this.letter.add(entry('\u0119', '\u0118')); // ę
    this.letter.add(entry('\u012E', '\u012E')); // Į
    this.letter.add(entry('\u012F', '\u012E')); // į
    this.letter.add(entry('\u0160', '\u0160')); // Š
    this.letter.add(entry('\u0161', '\u0160')); // š
    this.letter.add(entry('\u016A', '\u016A')); // Ū
    this.letter.add(entry('\u016B', '\u016A')); // ū
    this.letter.add(entry('\u0172', '\u0172')); // Ų
    this.letter.add(entry('\u0173', '\u0172')); // ų
  ...
  // Method:   build
  /**
   ** Factory method to create an <code>Lithuanian</code> {@link Alphabet}.
   ** properties.
   **
   ** @param  path               the abstract path to spool the configuration
   **                            file.
   ...
   **                            Allowed object is {@link File}.
   **
   ** @return                    <code>Lithuanian</code> {@link Alphabet}
   **                            <br>
   **                            Possible object is <code>Lithuanian</code>.
   */
  public static Lithuanian build(final File path) {
    return new Lithuanian(path);
  }
}