...
public abstract class Support {
  ...
  public static class UpdatingNodeVisitor extends NodeVisitor {
    ...
    protected JsonNode visitInnerNode(final ObjectNode parent, final String field, final Filter filter)
      ...
      if (node.isValueNode() || ((node.isMissingNode() || node.isNull()) && filter != null))
        throw BadRequestException.invalidTarget(ServiceBundle.string(ServiceError.FILTER_INVALID_TARGET_TYPE, field));
      ...
      if (node.isMissingNode() || node.isNull()) {
        ...
        if (filter != null) {
          ...
          if (arrayNode.size() == 0)
            throw BadRequestException.invalidTarget(ServiceBundle.string(ServiceError.FILTER_INVALID_TARGET_MATCH, field, filter));
        }
        return arrayNode;
      }
      return node;
        ...
        if (!this.append && filter != null) {
          ...
          if (node.isArray()) {
            for (int i = 0; i < node.size(); i++) {
              ...
              if (Evaluator.evaluate(filter, cursor)) {
                matchesFound = true;
   ...
   */
  public static int compareTo(final JsonNode lhs, final JsonNode rhs, final AttributeDefinition definition) {
    if (lhs.isTextual() && rhs.isTextual()) {
      ...
      if (d1 != null && d2 != null) {
        ...
        if (definition != null && definition.type() == Attribute.Type.STRING && definition.caseExact()) {
   ...
   **                            <code>false</code>.
   **
   ** @throws ProcessingException if an error occurs while traversing the JSON
   **                             node.
   */
   ...
   ** @throws ProcessingException if the filter is not valid for matching.
   */
  private static void traverse(final NodeVisitor visitor, final ObjectNode node, final Path path)
    throws ProcessingException {
