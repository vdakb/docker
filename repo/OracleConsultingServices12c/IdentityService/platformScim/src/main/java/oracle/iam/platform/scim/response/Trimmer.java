...
public class Trimmer {
  ...
  // Method:   Ctor
  /**
   ** Create a new <code>Trimmer</code> for trimming returned attributes for a
   ** SCIM operation.
   **
   ...
   ** @param attribute           the attributes from the 'attributes' or
   ...
   */
  private Trimmer(final ResourceTypeDefinition type, final Set<Path> request, final Set<Path> attribute, final boolean excluded) {
    // ensure inheritance
    super();
  ...
  // Method:   build
  /**
   ** Factory method to create a new <code>Trimmer</code> with the properties
   ** specified.
   **
   ** @param type                the resource type definition for resources to
   **                            trim.
   ...
   **                            Allowed object is <code>boolean</code>.
   **
   ** @return                    the <code>Trimmer</code> populated with the
   **                            values provided.
   **                            <br>
   **                            Possible object is <code>Trimmer</code>.
   */
  public static Trimmer build(final ResourceTypeDefinition type, final Set<Path> request, final Set<Path> attribute, final boolean excluded) {
    return new Trimmer(type, request, attribute, excluded);
  }
  ...
  private ObjectNode trim(final ObjectNode node, final Path parentPath) {
    ObjectNode                            objectToReturn = Support.nodeFactory().objectNode();
    Iterator<Map.Entry<String, JsonNode>> i = node.fields();
    while (i.hasNext()) {
      Map.Entry<String, JsonNode> field = i.next();
      final Path                  path;
      if (parentPath.empty() && parentPath.namespace() == null && Support.namespace(field.getKey())) {
  ...
  private boolean returned(final Path path) {
    AttributeDefinition definition = this.type.attributeDefinition(path);
    Attribute.Returned  returned   = definition == null ? Attribute.Returned.DEFAULT : definition.returned();
    ...
    switch (returned) {
      ...
      case REQUEST : // return only if it was one of the request attributes or
                     ...
                     return pathContains(this.request, path) || (this.request.isEmpty() && !this.excluded && pathContains(this.attribute, path));
      default      : // return if it is not one of the excluded query attributes
                     ...
                     if (this.excluded) {
                       return !pathContains(this.attribute, path);
                     }
                     else {
                       return this.attribute == null || this.attribute.isEmpty() || pathContains(this.attribute, path);
                    }
    }
  }
