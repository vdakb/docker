 ...
 ** @since   1.0.0.0
 */
@Schema(id=ListResponse.ID, name="List Response", description = "SCIM 2.0 List Response")
public class ListResponse<T> extends    Entity<ListResponse>
  ...
  public static final String ID        = "urn:ietf:params:scim:api:messages:2.0:ListResponse";
   ...
   **                               contains a value for {@link TOTAL} and
   **                               {@link RESOURCE}.
   */
  ...
  public ListResponse(final Map<String, Object> properties) {
    ...
    // the element tags has to be interpreted case-insensitive
    final Map<String,Object> temp = new TreeMap<String, Object>(String.CASE_INSENSITIVE_ORDER);
    temp.putAll(properties);

    // verify integrity
    checkRequired(temp, TOTAL, RESOURCE);

    // initialize instance attributes
    this.total    = (Integer)temp.get(TOTAL);
    this.start    = temp.containsKey(START) ? (Integer)temp.get(START) : null;
    this.items    = temp.containsKey(ITEMS) ? (Integer)temp.get(ITEMS) : null;
    this.resource = (List<T>)temp.get(RESOURCE);
    if (temp.containsKey(SCHEMA)) {
      namespace((Collection<String>)temp.get(SCHEMA));
    }
  }
   ...
   */
  public ListResponse(final int total, final Integer start, final Integer items, final List<T> resource) {
    // ensure inheritance
    super();
  ...
  public final int hashCode() {
    return Objects.hash(super.hashCode(), this.total, this.start, this.items, this.resource);
  }
