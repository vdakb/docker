...
public class Evaluator implements Filter.Visitor<Boolean, JsonNode> {
  ...
  public final Boolean apply(final StartsWith filter, final JsonNode node)
    ...
    return match(filter, Filter.Type.SW, node);
  }
  ...
  public final Boolean apply(final EndsWith filter, final JsonNode node)
    ...
    return match(filter, Filter.Type.EW, node);
  }
  ...
  public final Boolean apply(final Contains filter, final JsonNode node)
    ...
    return match(filter, Filter.Type.CO, node);
  }
   ...
   ** @throws ProcessingException if an exception occurs during the operation.
   */
  private boolean match(final AttributeFilter filter, final Filter.Type type, final JsonNode node)
    throws ProcessingException {

    final Iterable<JsonNode> nodes = candidates(filter.path(), node);
    for (JsonNode cursor : nodes) {
      if (cursor.isTextual() && filter.value().isTextual()) {
        final AttributeDefinition definition = definition(filter.path());
        String v = cursor.textValue();
        String f = filter.value().textValue();
        if (definition == null || !definition.caseExact()) {
          v = v.toLowerCase();
          f = f.toLowerCase();
        }
        switch (type) {
          case CO : return v.contains(f);
          case SW : return v.startsWith(f);
          case EW : return v.endsWith(f);
        }
      }
      else {
        return cursor.equals(filter.value());
      }
    }
    return false;
  }
