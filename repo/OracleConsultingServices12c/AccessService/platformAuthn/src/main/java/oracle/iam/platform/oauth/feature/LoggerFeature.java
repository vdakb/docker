...
import oracle.iam.platform.oauth.AuthorizationMessage;
 ...
 ** applied:
 ** <ul>
 **   <li>logger name: <code>oracle.iam.platform.oauth.LoggerFeature</code>
...
public class LoggerFeature implements Feature {
  ...
  class Interceptor implements WriterInterceptor {
    ...
    class Stream extends FilterOutputStream {
      ...
      StringBuilder capture(final Charset charset) {
        ...
        if (entity.length > Interceptor.this.size) {
          this.capture.append(AuthorizationBundle.string(AuthorizationMessage.LOGGER_ELIPSE_MORE));
        }
    ...
    public void aroundWriteTo(final WriterInterceptorContext writerInterceptorContext)
      ...
      if (this.logger.loggable(this.level) && printable(this.verbosity, writerInterceptorContext.getMediaType())) {
        if (stream != null) {
    ...
    void printRequest(final StringBuilder b, final String note, final long id, final String method, final URI uri) {
      print(b, id).append(AuthorizationBundle.string(AuthorizationMessage.LOGGER_THREAD_NAME, note, Thread.currentThread().getName()));
    ...
    InputStream printEntity(final StringBuilder b, final long id, final String prefix, InputStream stream, final Charset charset)
      ...
      if (size > this.size)
        print(b, id).append(prefix).append(AuthorizationBundle.string(AuthorizationMessage.LOGGER_ELIPSE_MORE));
     ...
     **                          containing the request/response flow sequence.
     */
  ...
  final class Client extends    Interceptor
    ...
    public void filter(final ClientRequestContext context)
      ...
      printRequest(capture, AuthorizationBundle.string(AuthorizationMessage.LOGGER_CLIENT_REQUEST), sequence, context.getMethod(), context.getUri());
    ...
    public void filter(final ClientRequestContext request, final ClientResponseContext response)
      ...
      printResponse(capture, AuthorizationBundle.string(AuthorizationMessage.LOGGER_CLIENT_RESPONSE), sequence, response.getStatus());
  ...
  final class Server extends    Interceptor
    ...
    public void filter(final ContainerRequestContext context)
      ...
      final StringBuilder capture = new StringBuilder();
      printRequest(capture, AuthorizationBundle.string(AuthorizationMessage.LOGGER_SERVER_REQUEST), sequence, context.getMethod(), context.getUriInfo().getRequestUri());
    ...
    public void filter(final ContainerRequestContext request, final ContainerResponseContext response)
      ...
      printResponse(capture, AuthorizationBundle.string(AuthorizationMessage.LOGGER_SERVER_RESPONSE), sequence, response.getStatus());
