/*
    ...
    File        :   DirectoryIdentityAsserter.java
...
import oracle.iam.access.glassfish.realm.Directory;
import oracle.iam.access.glassfish.realm.RealmBundle;
import oracle.iam.access.glassfish.realm.RealmError;

////////////////////////////////////////////////////////////////////////////////
// class DirectoryIdentityAsserter
// ~~~~~ ~~~~~~~~~~~~~~~~~~~~~~~~~
/**
 ** <code>DirectoryIdentityAsserter</code> describes the interface implemented
 ** by authentication technology providers.
 ** <p>
 ** <code>DirectoryIdentityAsserter</code> are plugged in under applications to
 ...
 ** <code>Login</code> modules. Each <code>DirectoryIdentityAsserter</code> is
 ** initialized with a {@code Subject}, a {@code CallbackHandler}, shared
 ** <code>DirectoryIdentityAsserter</code> state, and
 ** <code>DirectoryIdentityAsserter</code> module-specific options.
 ** <p>
 ...
 ** succeeds. <code>DirectoryIdentityAsserter</code> use the
 ** {@code CallbackHandler} to communicate with users. The
 ** {@code CallbackHandler} may be used to prompt for usernames and passwords,
 ** for example.
 ** <p>
 ** The <code>DirectoryIdentityAsserter</code> module-specific options represent
 ** the options configured for this <code>DirectoryIdentityAsserter</code>
 ** module by an administrator or user in the login <code>Configuration</code>.
 ** <br>
 ** The options are defined by the <code>DirectoryIdentityAsserter</code> itself
 ** and control the behavior within it. Options are defined using
 ** a key-value syntax, such as <i>debug=true</i>. The
 ** <code>DirectoryIdentityAsserter</code> stores the options in particular
 ** attributes.
 ** <br>
 ** <b>Note</b>:
 ** <br>
 ** There is no limit to the number of options a
 ** <code>DirectoryIdentityAsserter</code> module chooses to define but are
 ** ignored if the option key is unknown.
 ** <p>
 ...
 ** <code>DirectoryIdentityAsserter</code> module proceeds in two distinct
 ** phases.
 ** <br>
 ** In the first phase, the <code>DirectoryIdentityAsserter</code>'s
 ** <code>login</code> method gets invoked. The <code>login</code> method for
 ** the <code>DirectoryIdentityAsserter</code> module then performs the actual
 ...
 ** <code>DirectoryIdentityAsserter</code>'s <code>login</code> method either
 ...
 ** <br>
 ** In the failure case, the <code>DirectoryIdentityAsserter</code> does not
 ** retry the authentication or introduce delays. The responsibility of such
 ** tasks belongs to the application. If the application attempts to retry the
 ** authentication, the <code>DirectoryIdentityAsserter</code>'s
 ** <code>login</code> method will be called again.
 ** <p>
 ** In the second phase, if the LoginContext's overall authentication succeeded
 ** (the relevant REQUIRED, REQUISITE, SUFFICIENT and OPTIONAL
 ** <code>IdentityAsserter</code> succeeded), then the <code>commit</code>
 ** method for the <code>DirectoryIdentityAsserter</code> invoked.
 ** <br>
 ** The <code>commit</code> method for a <code>DirectoryIdentityAsserter</code>
 ...
 ** <code>DirectoryIdentityAsserter</code>'s own authentication succeeded, then
 ** the <code>commit</code> method associates the relevant Principals
 ...
 ** <code>DirectoryIdentityAsserter</code>.
 ** <p>
 ...
 ** the <code>DirectoryIdentityAsserter</code> removes/destroys any
 ...
 ** <code>DirectoryIdentityAsserter</code>'s <code>logout</code> method. The
 ** <code>logout</code> method for the <code>DirectoryIdentityAsserter</code>
 ...
 ** @since   1.0.0.0
 */
public class DirectoryIdentityAsserter extends IndentityAsserter {
  ...
  // Method:   Ctor
  /**
   ** Constructs a <code>DirectoryIdentityAsserter</code> login module that
   ...
   */
  public DirectoryIdentityAsserter() {
    // ensure inheritance
    super();
  }
  ...
  //////////////////////////////////////////////////////////////////////////////
  // Method:   authenticateUser (BearerAsserter)
  /**
  ...
  public void authenticateUser()
    ...
    // prevent bogus state
    if (!(this.currentRealm instanceof Directory)) {
  ...
  protected String[] assertPrincipal()
    ...
    return ((Directory)this.currentRealm).assertUser(this.identity);
  }
}