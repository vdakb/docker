/*
    ...
    File        :   DirectoryBearerAsserter.java
...
////////////////////////////////////////////////////////////////////////////////
// class DirectoryBearerAsserter
// ~~~~~ ~~~~~~~~~~~~~~~~~~~~~~~
/**
 ** <code>DirectoryBearerAsserter</code> describes the interface implemented by
 ** authentication technology providers.
 ** <p>
 ** <code>DirectoryBearerAsserter</code> are plugged in under applications to
 ...
 ** <code>Login</code> modules. Each <code>DirectoryBearerAsserter</code> is
 ** initialized with a {@code Subject}, a {@code CallbackHandler}, shared
 ** <code>DirectoryBearerAsserter</code> state, and
 ** <code>DirectoryBearerAsserter</code> module-specific options.
 ** <p>
 ...
 ** succeeds. <code>DirectoryBearerAsserter</code> use the
 ** {@code CallbackHandler} to communicate with users. The
 ** {@code CallbackHandler} may be used to prompt for usernames and passwords,
 ** for example.
 ** <p>
 ** The <code>DirectoryBearerAsserter</code> module-specific options represent
 ** the options configured for this <code>DirectoryBearerAsserter</code> module
 ** by an administrator or user in the login <code>Configuration</code>.
 ** <br>
 ** The options are defined by the <code>DirectoryBearerAsserter</code> itself
 ** and control the behavior within it. Options are defined using
 ** a key-value syntax, such as <i>debug=true</i>. The
 ** <code>DirectoryBearerAsserter</code> stores the options in particular
 ** attributes.
 ** <br>
 ** <b>Note</b>:
 ** <br>
 ** There is no limit to the number of options a
 ** <code>DirectoryBearerAsserter</code> module chooses to define but are
 ...
 ** <code>DirectoryBearerAsserter</code> module proceeds in two distinct phases.
 ** <br>
 ** In the first phase, the <code>DirectoryBearerAsserter</code>'s
 ** <code>login</code> method gets invoked. The <code>login</code> method for
 ** the <code>DirectoryBearerAsserter</code> module then performs the actual
 ...
 ** <code>DirectoryBearerAsserter</code>'s <code>login</code> method either
 ...
 ** <br>
 ** In the failure case, the <code>DirectoryBearerAsserter</code> does not retry
 ...
 ** authentication, the <code>DirectoryBearerAsserter</code>'s
 ...
 ** <code>BearerAsserter</code> succeeded), then the <code>commit</code> method
 ** for the <code>DirectoryBearerAsserter</code> invoked.
 ** <br>
 ** The <code>commit</code> method for a <code>DirectoryBearerAsserter</code>
 ...
 ** <code>DirectoryBearerAsserter</code>'s own authentication succeeded, then
 ...
 ** <code>DirectoryBearerAsserter</code>.
 ** <p>
 ...
 ** the <code>DirectoryBearerAsserter</code> removes/destroys any authentication
 ** state originally saved.
 ** <p>
 ** Logging out a {@code Subject} involves only one phase.
 ** <br>
 ** The <code>LoginContext</code> invokes the
 ** <code>DirectoryBearerAsserter</code>'s <code>logout</code> method. The
 ** <code>logout</code> method for the <code>DirectoryBearerAsserter</code> then
 ...
 ** @since   1.0.0.0
 */
public class DirectoryBearerAsserter extends BearerAsserter {
  ...
  // Method:   Ctor
  /**
   ** Constructs a <code>DirectoryBearerAsserter</code> login module that allows
   ** use as a JavaBean.
   ** <br>
   ...
   */
  public DirectoryBearerAsserter() {
    // ensure inheritance
    super();
  }
