/*
    ...
    File        :   DirectoryAuthenticator.java
...
// ~~~~~ ~~~~~~~~~~~~~~~~~~~~~~
/**
 ** <code>DirectoryAuthenticator</code> describes the interface implemented by
 ** authentication technology providers.
 ** <p>
 ** <code>DirectoryAuthenticator</code> are plugged in under applications to
 ...
 ** <code>Login</code> modules. Each <code>DirectoryAuthenticator</code> is
 ** initialized with a {@code Subject}, a {@code CallbackHandler}, shared
 ** <code>DirectoryAuthenticator</code> state, and
 ** <code>DirectoryAuthenticator</code> module-specific options.
 ** <p>
 ...
 ** succeeds. <code>DirectoryAuthenticator</code> use the
 ** {@code CallbackHandler} to communicate with users. The
 ** {@code CallbackHandler} may be used to prompt for usernames and passwords,
 ** for example.
 ** <p>
 ** The <code>DirectoryAuthenticator</code> module-specific options represent the
 ** options configured for this <code>DirectoryAuthenticator</code> module by an
 ** administrator or user in the login <code>Configuration</code>.
 ** <br>
 ** The options are defined by the <code>DirectoryAuthenticator</code> itself and
 ** control the behavior within it. Options are defined using
 ** a key-value syntax, such as <i>debug=true</i>. The
 ** <code>DirectoryAuthenticator</code> stores the options in particular
 ** attributes.
 ** <br>
 ** <b>Note</b>:
 ** <br>
 ** There is no limit to the number of options a
 ** <code>DirectoryAuthenticator</code> module chooses to define but are ignored
 ...
 ** <code>DirectoryAuthenticator</code> module proceeds in two distinct phases.
 ** <br>
 ** In the first phase, the <code>DirectoryAuthenticator</code>'s
 ** <code>login</code> method gets invoked. The <code>login</code> method for
 ** the <code>DirectoryAuthenticator</code> module then performs the actual
 ...
 ** <code>DirectoryAuthenticator</code>'s <code>login</code> method either
 ...
 ** <br>
 ** In the failure case, the <code>DirectoryAuthenticator</code> does not retry
 ...
 ** authentication, the <code>DirectoryAuthenticator</code>'s <code>login</code>
 ...
 ** the <code>DirectoryAuthenticator</code> invoked.
 ** <br>
 ** The <code>commit</code> method for a <code>DirectoryAuthenticator</code>
 ...
 ** <code>DirectoryAuthenticator</code>'s own authentication succeeded, then the
 ...
 ** <code>DirectoryAuthenticator</code>.
 ** <p>
 ...
 ** <code>DirectoryAuthenticator</code> removes/destroys any authentication
 ** state originally saved.
 ** <p>
 ** Logging out a {@code Subject} involves only one phase.
 ** <br>
 ** The <code>LoginContext</code> invokes the
 ** <code>DirectoryAuthenticator</code>'s <code>logout</code> method. The
 ** <code>logout</code> method for the <code>DirectoryAuthenticator</code> then
 ...
 ** @since   1.0.0.0
 */
public class DirectoryAuthenticator extends BasePasswordLoginModule {
  ...
  // Method:   Ctor
  /**
   ** Constructs a {@link BasePasswordLoginModule} module that allows use as a
   ** JavaBean.
   ** <br>
  ...
  public void authenticateUser()
    ...
    // prevent bogus state
    if (!(this._currentRealm instanceof Directory)) {
  ...
  protected String[] assertCredential()
    ...
    return ((Directory)this._currentRealm).authenticateUser(this._username, getPasswordChar());
  }
}