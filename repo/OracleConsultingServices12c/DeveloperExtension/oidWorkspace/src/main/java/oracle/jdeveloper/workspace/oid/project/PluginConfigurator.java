...
import oracle.jdeveloper.workspace.oid.model.Plugin;

import oracle.jdeveloper.workspace.oid.parser.PluginHandler;

////////////////////////////////////////////////////////////////////////////////
// class PluginConfigurator
// ~~~~~ ~~~~~~~~~~~~~~~~~~
/**
 ** The configurator to setup Oracle Internet Directory Plug-In projects.
 **
 ** @author  dieter.steding@oracle.com
 ** @version 12.2.1.3.42.60.94
 ** @since   11.1.1.3.37.56.13
 */
class PluginConfigurator extends TemplateProjectConfigurator {
  ...
  public final void templateFeaturePreview(final TemplateFolder workspaceFolder) {
    ...
    // we assume the template specifies the pattern #{oid.preferences} to
    ...
    final TemplateStream item   = folder.add(pluginPreview(folder));
  ...
  protected final void configureClassPath() {
    ...
    settings.addLibrary(Library.OID_SERVER_ID,     false);
    settings.addLibrary(Library.HST_FOUNDATION_ID, false);
    settings.addLibrary(Library.OID_FOUNDATION_ID, false);
  }
   ...
   **                            configure Oracle Internet Directory Agent
   **                            {@link Project}s.
   */
   ...
   **                            preview.
   */
  private TemplateStream pluginPreview(final TemplateFolder folder) {
    ...
    if (item.exists()) {
      ...
      try {
        ...
        item.property(PluginHandler.DESTINATION, handler.propertyValue(PluginHandler.DESTINATION));
