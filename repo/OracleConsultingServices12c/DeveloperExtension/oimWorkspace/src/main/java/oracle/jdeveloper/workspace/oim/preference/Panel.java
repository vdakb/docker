...
// ~~~~~ ~~~~~
/**
 * The UI panel that provides support in the preference properties dialog for
 * editing the properties stored in the {@link Store} model.
 * <p>
 ...
 */
public final class Panel extends FeaturePreferencePanel {
  ...
  // compatible with
  @SuppressWarnings("compatibility:2936204354043095312")
  private static final long          serialVersionUID = 796480305760630010L;
  ...
  // the properties to configure the Oracle Identity Manager Development
  // Framework
  private transient URLTextField      foundationEdit;
  private transient JButton           foundationBrowse;

  // the properties to configure the Oracle Identity Manager Design Console
  private transient URLTextField      consoleEdit;
  private transient JButton           consoleBrowse;
  ...
  //////////////////////////////////////////////////////////////////////////////
  // Method:   preference (FeaturePreferencePanel)
  /**
  ...
  protected void initializeComponent() {
    ...
    // the properties to configure the Oracle Identity Manager Design Console
    this.consoleEdit      = new URLTextField();
    this.consoleBrowse    = new JButton();
    ...
    this.consoleBrowse.addActionListener(this.consoleEdit);
    ...
    this.releaseList.setModel(new DefaultComboBoxModel<String>(Library.RELEASE.keySet().toArray(new String[0])));
  ...
  protected void localizeComponent() {
    ...
    this.consoleBrowse.setIcon(ComponentBundle.icon(ComponentBundle.DIRECTORY_ICON));
    this.consoleBrowse.setToolTipText(Bundle.string(Bundle.CONSOLE_HINT));
    ...
    IconicButtonUI.install(this.consoleBrowse);
  ...
  protected void initializeLayout() {
    ...
    , "6dlu, pref, 6dlu, pref, 1dlu, pref, 4dlu, pref, 1dlu, pref, 4dlu, pref, 1dlu, pref, 4dlu, pref, 4dlu, pref, 1dlu, pref"
    ...
    row += 2;
    label = builder.addLabel(Bundle.string(Bundle.CONSOLE_HOME), constraint.xyw(4, row++, 7));
    label.setLabelFor(this.consoleEdit);
    builder.add(this.consoleEdit,   constraint.xyw( 4, ++row, 7));
    builder.add(this.consoleBrowse, constraint.xy (12,   row));
  ...
  public void onEntry(final TraversableContext context) {
    ...
    this.consoleEdit.setURL(((Store)this.provider).console());
  ...
  //////////////////////////////////////////////////////////////////////////////
  // Method:   commitConsole
  /**
   ** Stores if the path the Oracle Identity Manager Design Console is
   ** installed.
   ** <p>
   ...
   */
  private void commitConsole(final boolean destructive)
    throws TraversalException {

    final URL newConsole = this.consoleEdit.getURL();
    if (!validateFolder(newConsole)) {
      if (!confirmViolation(Store.CONSOLE, Bundle.string(Bundle.CONSOLE_FAILURE), destructive))
        throw new TraversalException(Store.CONSOLE);
    }
    // extend the test by checking the existence of important configuration
    // files
    if (newConsole != null) {
      URL policyConfig = URLFactory.newURL(newConsole, "config/xl.policy");
      if (!URLFileSystem.exists(policyConfig)) {
        if (!confirmViolation(Store.CONSOLE, Bundle.string(Bundle.CONSOLE_FAILURE), destructive))
          throw new TraversalException(Store.CONSOLE);
      }
    }

    if (!URLFileSystem.equals(((Store)this.provider).console(), newConsole))
     ((Store)this.provider).console(newConsole);
  }
}