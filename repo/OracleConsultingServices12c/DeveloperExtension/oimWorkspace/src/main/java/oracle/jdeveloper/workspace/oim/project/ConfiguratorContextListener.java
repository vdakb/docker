/*
    ...
    File        :   ConfiguratorContextListener.java
...
////////////////////////////////////////////////////////////////////////////////
// class ConfiguratorContextListener
// ~~~~~ ~~~~~~~~~~~~~~~~~~~~~~~~~~~
/**
 ...
 */
public class ConfiguratorContextListener extends TemplateContextListener {
  ...
  // Method:   Ctor
  /**
   ** Constructs an empty <code>ConfiguratorContextListener</code> that allows
   ** use as a JavaBean.
   ** <br>
   ...
   ** Default Constructor
   */
  public ConfiguratorContextListener() {
    // ensure inheritance
    super();
  }
  ...
  public void menuWillShow(final ContextMenu contextMenu) {
    ...
    if (contextMenu.getContext().getNode() instanceof Project) {
      ...
      if (scope.contains(Feature.adapterTechnology())) {
        final IdeAction action = IdeAction.find(Configurator.COMMAND);
      ...
      else if (scope.contains(Feature.connectorTechnology())) {
        final IdeAction action = IdeAction.find(Configurator.COMMAND);
        contextMenu.add(contextMenu.createMenuItem(action));
      }
      else if (scope.contains(Feature.backendTechnology())) {
        final IdeAction action = IdeAction.find(Configurator.COMMAND);
        contextMenu.add(contextMenu.createMenuItem(action));
      }
      else if (scope.contains(Feature.frontendTechnology())) {
        final IdeAction action = IdeAction.find(Configurator.COMMAND);
        contextMenu.add(contextMenu.createMenuItem(action));
      }
      else if (scope.contains(Feature.assemblyTechnology())) {
        final IdeAction action = IdeAction.find(Configurator.COMMAND);
        contextMenu.add(contextMenu.createMenuItem(action));
      }
      else if (scope.contains(Feature.workflowTechnology())) {
        final IdeAction action = IdeAction.find(Configurator.COMMAND);
        contextMenu.add(contextMenu.createMenuItem(action));
      }
