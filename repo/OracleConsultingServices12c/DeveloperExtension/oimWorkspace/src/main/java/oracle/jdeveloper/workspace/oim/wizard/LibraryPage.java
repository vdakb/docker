 ...
 ** Oracle Identity Manager Libary Project.
 ** <p>
...
public class LibraryPage extends TemplateFeaturePage {
  ...
  // compatible with
  @SuppressWarnings("compatibility:-169810428202832513")
  private static final long      serialVersionUID = 8451575039774373952L;
  ...
  private transient JTextField   commonPackage;
  ...
  protected void initializeComponent() {
    ...
    this.commonPackage     = new JTextField();
  ...
  protected void initializeLayout() {
    ...
    row += 2;
    builder.addSeparator(Bundle.string(Bundle.COMMON_LIBRARY_HEADER), constraint.xyw(2, row, 5));
    ...
    row += 2;
    label = builder.addLabel(Bundle.string(Bundle.COMMON_PACKAGE_LABEL), constraint.xy(2, row));
    label.setLabelFor(this.commonPackage);
    builder.add(this.commonPackage, constraint.xyw(4, row, 3));
  }
  ...
  protected final void initializePage() {
    this.name.setText(this.library.file());
    this.project.setText(this.library.project());
    this.description.setText(this.library.description());
    this.destination.setURL(this.library.destination());

    this.commonLibrary.setText(this.library.application());
    this.commonPackage.setText(this.library.packagePath());
  }
  ...
  public void onEntry(final TraversableContext context) {
    ...
    this.library = Library.instance(context);

    // we need to ask too for specific properties from the workspace to
    // initialize the model
    this.library.packagePath(packagePrefix(context));
  ...
  private final void commitName(final boolean validate)
    ...
    if (!this.library.file().equals(value))
      this.library.file(value);
  }
  ...
  private final void commitProject(final boolean validate)
    ...
    // store the provided value in the context
    if (!this.library.project().equals(value))
      this.library.project(value);
  }
  ...
  private final void commitDescription(final boolean validate)
    ...
    // store the provided value in the context
    if (!this.library.description().equals(value))
  ...
  private final void commitDestination(final boolean validate)
    ...
    // store the provided value in the context
    if (!URLFileSystem.equals(this.library.destination(), value))
      this.library.destination(value);
  }
   ...
   ** Checks if the instance satisfies all requirements.
   ** <p>
   ** In other terms the entry <code>library</code> <code>package</code>
  ...
  private final void commitLibrary(final boolean validate)
    ...
    // store the provided value in the context
    if (!this.library.application().equals(value))
      this.library.application(value);

    // obtained the value from the UI to store.
    value = this.commonPackage.getText();
    // check the rules and validate
    if (validate)
      validateString(value, Bundle.string(Bundle.COMMON_PACKAGE_ERROR));

    // store the provided value in the context
    if (!this.library.packagePath().equals(value))
      this.library.packagePath(value);
  }
}