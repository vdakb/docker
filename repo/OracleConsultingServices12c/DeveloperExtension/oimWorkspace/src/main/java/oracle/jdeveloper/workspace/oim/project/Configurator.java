/*
    ...
    11.1.1.3.37.60.15  2012-02-06  DSteding    Configuring of connector
                                               projects added.
...
public class Configurator extends TemplatePreview {
  ...
  public static final String COMMAND = "oim.jpr.configurator";
   ...
   **                            design time settings.
   **
   ** @return                    the {@link TemplateApplicationConfigurator} for
   **                            the specified {@link Workspace}.
  ...
  private static TemplateApplicationConfigurator create(final Workspace workspace, final TraversableContext context) {
    ...
    if (Manifest.APPLICATION.equals(templateId))
      return WorkspaceConfigurator.instance(workspace, context);
    else if (Manifest.CUSTOMIZATION.equals(templateId))
      return CustomizationConfigurator.instance(workspace, context);
    else
      return null;
  }
  ...
  private static TemplateProjectConfigurator create(final Project project, final TraversableContext context) {
    final TechnologyScope sope = TechnologyScopeConfiguration.getInstance(project).getTechnologyScope();
    if (sope.isEmpty())
      return null;

    if (sope.contains(Feature.libraryTechnology())) {
      return LibraryConfigurator.instance(project, context);
    }
    if (sope.contains(Feature.adapterTechnology())) {
      return AdapterConfigurator.instance(project, context);
    }
    else if (sope.contains(Feature.connectorTechnology())) {
      return ConnectorConfigurator.instance(project, context);
    }
    else if (sope.contains(Feature.pluginTechnology())) {
      return PluginConfigurator.instance(project, context);
    }
    else if (sope.contains(Feature.deploymentTechnology())) {
      return DeploymentConfigurator.instance(project, context);
    }
    else if (sope.contains(Feature.backendTechnology())) {
      return CustomizationBackendConfigurator.instance(project, context);
    }
    else if (sope.contains(Feature.frontendTechnology())) {
      return CustomizationFrontendConfigurator.instance(project, context);
    }
    else if (sope.contains(Feature.assemblyTechnology())) {
      return CustomizationAssemblyConfigurator.instance(project, context);
    }
    else if (sope.contains(Feature.workflowTechnology())) {
      return WorkflowConfigurator.instance(project, context);
    }
    else
      return null;
  }
