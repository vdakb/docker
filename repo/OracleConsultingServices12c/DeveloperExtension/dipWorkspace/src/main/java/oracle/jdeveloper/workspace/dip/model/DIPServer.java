/*
    ...
    11.1.1.3.37.60.32  2012-10-20  DSteding    Cloning of an existing file
                                               implemented.
    11.1.1.3.37.60.61  2014-10-25  DSteding    Defect DE-000146
 ...
 ** stored in the <code>DIPServer</code> model.
 **
 ** @author  dieter.steding@oracle.com
 ** @version 12.2.1.3.42.60.74
 ** @since   11.1.1.3.37.56.13
 */
public class DIPServer extends FMWServer {

  //////////////////////////////////////////////////////////////////////////////
  // static final attributes
  //////////////////////////////////////////////////////////////////////////////

  private static final String FILE              = "dip-server";
  ...
  private static final String DEFAULT_PORT      = "3060";
  private static final String DEFAULT_SERVER    = "ods";
  private static final String DEFAULT_PARTITION = "dip";
  private static final String DEFAULT_USERNAME  = "cn=orcladmin";
  ...
  // Method:   Ctor
  /**
   ** Constructs a <code>DIPServer</code>.
   ** <br>
   ** This constructor is private to prevent other classes to use
   ** "new DIPServer()" and enforces use of the public factory method below.
   **
   ** @param  structure          the data provider to initialize the instance.
   */
  private DIPServer(final HashStructure structure) {
    // ensure inheritance
    super(structure);
  }
  ...
  public String project() {
    // the second parameter to string() is a default value. Defaults are
    // stored in the persistent preferences file using a "placeholder".
    return string(PROJECT, Manifest.string(Manifest.DIP_SERVER_PROJECT));
  }
  ...
  public final void platform(final String platform) {
    this._hash.putString(DIPServerHandler.PLATFORM, platform);
  }
  ...
  public final String platform() {
    // the second parameter to string() is a default value. Defaults are
    // stored in the persistent preferences file using a "placeholder".
    return string(DIPServerHandler.PLATFORM, StringUtility.EMPTY);
  }
  ...
  public void host(final String host) {
    this._hash.putString(DIPServerHandler.HOST, host);
  }
  ...
  public String host() {
    // the second parameter to string() is a default value. Defaults are
    // stored in the persistent preferences file using a "placeholder".
    return string(DIPServerHandler.HOST, StringUtility.EMPTY);
  }
  ...
  public final void username(final String username) {
    this._hash.putString(DIPServerHandler.USERNAME, username);
  }
  ...
  public final String username() {
    // the second parameter to string() is a default value. Defaults are
    // stored in the persistent preferences file using a "placeholder".
    return string(DIPServerHandler.USERNAME, DEFAULT_USERNAME);
  }
  ...
  public final void password(final String password) {
    this._hash.putString(DIPServerHandler.PASSWORD, password);
  }
  ...
  public final String password() {
    return string(DIPServerHandler.PASSWORD, StringUtility.EMPTY);
  }
   ...
   ** using {@link Manifest#DIP_SERVER} in the design time objects of the
   ...
   **
   ** @return                    the <code>DIPServer</code> instance obtained
   **                            from the specifed {@link TraversableContext}.
   */
  public static DIPServer instance(final TraversableContext context) {
    DIPServer instance = (DIPServer)context.getDesignTimeObject(Manifest.DIP_SERVER);
    if (instance == null) {
      instance = new DIPServer(HashStructure.newInstance());
      context.putDesignTimeObject(Manifest.DIP_SERVER, instance);
  ...
  public void clone(final URL source) {
    DIPServerHandler handler = new DIPServerHandler(source);
    try {
      handler.load();
      platform(handler.propertyValue(DIPServerHandler.PLATFORM));
      protocol(handler.propertyValue(DIPServerHandler.PROTOCOL));
      host(handler.propertyValue(DIPServerHandler.HOST));
      port(handler.propertyValue(DIPServerHandler.PORT));
      home(handler.propertyValue(DIPServerHandler.HOME));
      username(handler.propertyValue(DIPServerHandler.USERNAME));
      password(handler.propertyValue(DIPServerHandler.PASSWORD));
      partition(handler.propertyValue(DIPServerHandler.PARTITION));
    }
