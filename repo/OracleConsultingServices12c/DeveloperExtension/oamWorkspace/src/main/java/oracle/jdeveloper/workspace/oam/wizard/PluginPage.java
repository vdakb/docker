 ...
 ** preferences stored in the {@link Plugin} model.
 ** <p>
...
public class PluginPage extends TemplateFeaturePage {
  ...
  // compatible with
  @SuppressWarnings("compatibility:1152139918094681279")
  private static final long       serialVersionUID = 7750926393387652464L;
  ...
  // the properties to configure the authorizer properties
  private transient JTextField    authenticatorLibrary;
  private transient JTextField    authenticatorPackage;
  ...
  // Method:   Ctor
  /**
   ** Constructs an empty <code>PluginPage</code> that allows use as a
   ** JavaBean.
   ** <br>
  ...
  protected void initializeComponent() {
    ...
    this.authenticatorLibrary = new JTextField();
    this.authenticatorPackage = new JTextField();
  ...
  protected void initializeLayout() {
    ...
    label.setLabelFor(this.authenticatorLibrary);
    builder.add(this.authenticatorLibrary, constraint.xyw(4, row, 3));

    // the 9th logical row of the layout
    row += 2;
    label = builder.addLabel(Bundle.string(Bundle.PACKAGE_LABEL), constraint.xy(2, row));
    label.setLabelFor(this.authenticatorPackage);
    builder.add(this.authenticatorPackage, constraint.xyw(4, row, 3));
  }
  ...
  protected final void initializePage() {
    ...
    this.authenticatorLibrary.setText(this.provider.application());
    this.authenticatorPackage.setText(this.provider.packagePath());
  }
  ...
  public void onEntry(final TraversableContext context) {
    ...
    if (!StringUtility.empty(prefix)) {
      ...
      this.provider.packagePath(packagePath(prefix, "plugin"));
    }
  ...
  private final void commitApplication(final boolean validate)
    ...
    // obtained the value from the UI to store.
    String value = this.authenticatorLibrary.getText();
    // check the rules and validate
    if (validate)
    ...
    // obtained the value from the UI to store.
    value = this.authenticatorPackage.getText();
    // check the rules and validate
    if (validate)
