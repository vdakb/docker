 ...
 ** preferences stored in the {@link Agent} model.
 ** <p>
 ...
 */
public class AgentPage extends TemplateFeaturePage {
  ...
  // compatible with
  @SuppressWarnings("compatibility:7196182096366981879")
  private static final long    serialVersionUID = -8101141776789566133L;
  ...
  private transient JTextField   agentLibrary;
  private transient JTextField   agentPackage;

  private transient Agent        provider         = null;
  ...
  // Method:   Ctor
  /**
   ** Constructs an empty <code>AgentPage</code> that allows use as a
   ** JavaBean.
   ** <br>
  ...
  protected void initializeComponent() {
    ...
    this.agentLibrary      = new JTextField();
    this.agentPackage      = new JTextField();
  ...
  protected void initializeLayout() {
    ...
    label.setLabelFor(this.agentLibrary);
    builder.add(this.agentLibrary, constraint.xyw(4, row, 3));

    // the 9th logical row of the layout
    row += 2;
    label = builder.addLabel(Bundle.string(Bundle.PACKAGE_LABEL), constraint.xy(2, row));
    label.setLabelFor(this.agentPackage);
    builder.add(this.agentPackage, constraint.xyw(4, row, 3));
  }
  ...
  protected final void initializePage() {
    ...
    this.agentLibrary.setText(this.provider.application());
    this.agentPackage.setText(this.provider.packagePath());
  }
  ...
  public void onEntry(final TraversableContext context) {
    ...
    this.provider = Agent.instance(context);
    ...
    if (!StringUtility.empty(prefix)) {
      ...
      this.provider.packagePath(packagePath(prefix, "agent"));
    }
  ...
  private final void commitApplication(final boolean validate)
    ...
    // obtained the value from the UI to store.
    String value = this.agentLibrary.getText();
    // check the rules and validate
    if (validate)
    ...
    // obtained the value from the UI to store.
    value = this.agentPackage.getText();
    // check the rules and validate
    if (validate)
