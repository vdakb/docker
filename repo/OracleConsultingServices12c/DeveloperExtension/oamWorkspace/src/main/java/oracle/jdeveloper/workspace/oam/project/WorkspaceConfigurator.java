...
import oracle.jdeveloper.workspace.oam.preference.Store;

import oracle.jdeveloper.workspace.oam.model.OAMTarget;
import oracle.jdeveloper.workspace.oam.model.OAMContext;
import oracle.jdeveloper.workspace.oam.model.OAMPreference;

import oracle.jdeveloper.workspace.oam.parser.TargetHandler;
import oracle.jdeveloper.workspace.oam.parser.OAMTargetHandler;
import oracle.jdeveloper.workspace.oam.parser.PreferenceHandler;
...
class WorkspaceConfigurator extends ApplicationConfigurator {
  ...
  public final void templateFeaturePreview(final TemplateFolder workspaceFolder) {
    ...
    featureRoot.add(WorkspaceConfigurator.templateOAMServerPreview(featureRoot, this.context));
  }
  ...
  //////////////////////////////////////////////////////////////////////////////
  // Method:   templateOAMTargetPreview
  /**
   ** Creates the ANT target build file for Oracle Access Manager projects.
   **
   ...
   **                            preview.
   */
  public static TemplateStream templateOAMTargetPreview(final TemplateFolder folder, final TraversableContext context) {
    final OAMTarget      target    = OAMTarget.instance(context);
    ...
    item.add(TargetHandler.ANT_BASEDIR,           TemplateBundle.icon(TemplateBundle.NODE_PROPERTY_ANT_ICON),  item.relativeFile(target.basedir()));
    item.add(TargetHandler.WKS_PACKAGE,           TemplateBundle.icon(TemplateBundle.NODE_PROPERTY_JAVA_ICON), TemplateWizardUtil.getApplicationPackage(context));
    item.add(TargetHandler.SPECIFICATION_VENDOR,  TemplateBundle.icon(TemplateBundle.NODE_PROPERTY_JAVA_ICON), target.specificationVendor());
    item.add(TargetHandler.IMPLEMENTATION_VENDOR, TemplateBundle.icon(TemplateBundle.NODE_PROPERTY_JAVA_ICON), target.implementationVendor());
    ...
    if (item.exists()) {
      OAMTargetHandler handler = new OAMTargetHandler(URLFactory.newFileURL(item.file()));
      try {
        handler.load();
        item.property(TargetHandler.ANT_PROJECT,           handler.name());
        item.property(TargetHandler.ANT_BASEDIR,           handler.basedir());
        item.property(TargetHandler.WKS_PACKAGE,           handler.packageName());
        item.property(TargetHandler.SPECIFICATION_VENDOR,  handler.specificationVendor());
        item.property(TargetHandler.IMPLEMENTATION_VENDOR, handler.implementationVendor());
      }
  ...
  //////////////////////////////////////////////////////////////////////////////
  // Method:   templateOAMContextPreview
  /**
   ** Creates the ANT context build file for Oracle Access Manager Server.
   **
   ...
   **                            preview.
   */
  public static TemplateStream templateOAMContextPreview(final TemplateFolder folder, final TraversableContext context) {
    final OAMContext     target = OAMContext.instance(context);
    final TemplateStream item   = new TemplateStream(folder, target.file());
    // configure the template to use
    item.template(ClassUtility.classNameToFile(target.template(), ClassUtility.XML));
    // check if the item already exists to override the default values created
  ...
  //////////////////////////////////////////////////////////////////////////////
  // Method:   templateOAMPreferencePreview
  /**
   ** Creates the ANT preferences build file for Oracle Access Manager
   ** applications.
   **
   ** @param  folder             the {@link TemplateFolder} where the file will
   **                            be created within.
   ...
   **                            preview.
   */
  public static TemplateStream templateOAMPreferencePreview(final TemplateFolder folder, final TraversableContext context) {
    return templatePreferencePreview(folder, OAMPreference.instance(context));
  }

  //////////////////////////////////////////////////////////////////////////////
  // Method:   mavenOAMPreferencePreview
  /**
   ...
   **                            preview.
   */
  public static ModelStream mavenOAMPreferencePreview(final ModelFolder folder, final TraversableContext context) {
    return mavenPreferencePreview(folder, OAMPreference.instance(context));
  }
   ...
   **                            specific to configure Oracle Access Manager
   **                            {@link Workspace}s.
   */
  ...
  private static TemplateStream templatePreferencePreview(final TemplateFolder folder, final BuildPropertyAdapter preference) {
    ...
    item.add(PreferenceHandler.OAM_RELEASE, TemplateBundle.icon(TemplateBundle.NODE_PROPERTY_XML_ICON), store.release());
    // check if the item already exists to override the default values created
    // above with the values found in the file
    item.hotspotSelected(!item.exists());
    if (item.exists()) {
