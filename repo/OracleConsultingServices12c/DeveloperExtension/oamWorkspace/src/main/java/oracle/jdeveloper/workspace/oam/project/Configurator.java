...
public class Configurator extends TemplatePreview {
  ...
  static final String COMMAND = "oam.jpr.configurator";
  ...
  protected final TemplateFolder createPreview(final Project project) {
    final TraversableContext          context      = new TraversableContext(new Namespace(), TraversableContext.FORWARD_TRAVERSAL);
    final TemplateProjectConfigurator configurator = create(project, context);
  ...
  private static TemplateProjectConfigurator create(final Project project, final TraversableContext context) {
    final TechnologyScope scope = TechnologyScopeConfiguration.getInstance(project).getTechnologyScope();
    if (scope.isEmpty())
      return null;

    if (scope.contains(Feature.libraryTechnology())) {
      return LibraryConfigurator.instance(project, context);
    }
    else if (scope.contains(Feature.agentTechnology())) {
      return AgentConfigurator.instance(project, context);
    }
    else if (scope.contains(Feature.pluginTechnology())) {
      return PluginConfigurator.instance(project, context);
    }
    else if (scope.contains(Feature.credentialCollectorTechnology())) {
      return CredentialCollectorConfigurator.instance(project, context);
    }
    else if (scope.contains(Feature.deploymentTechnology())) {
