...
// ~~~~~ ~~~~~~~~~~~~~~~
/**
 ** The UI panel that provides support in the ModelPreview dialog for
 ** navigating though the <code>ModelData</code> model.
 ** <p>
...
class NavigationPanel extends DefaultTraversablePanel {
  ...
  // compatible with
  @SuppressWarnings("compatibility:-2928100075508749392")
  private static final long          serialVersionUID = -7590685796173004432L;
  ...
  public void onEntry(final TraversableContext context) {
    ...
    final ModelFolder data = (ModelFolder)context.getDesignTimeObject(Configurator.BUILDFILE_ROOT);
   ...
   ** provider.
   **
   ** @param  folder             the {@link ModelFolder) acting as the root of
   **                            a buildfile hierarchy.
   **
   ** @return                    the created {@link OptionTreeNode} ready for
   **                            preview.
   */
  private OptionTreeNode createNode(final ModelFolder folder) {
  ...
  private void localizeComponent() {
    ResourceUtils.resButton(this.previewAction, ModelBundle.string(ModelBundle.BUILDFILE_PREVIEW));
  }
  ...
  private void initializeLayout() {
    ...
    int row = 1;
    builder.addSeparator(ModelBundle.string(ModelBundle.BUILDFILE_NAVIGATOR_TITLE), constraint.xy(2, row));
