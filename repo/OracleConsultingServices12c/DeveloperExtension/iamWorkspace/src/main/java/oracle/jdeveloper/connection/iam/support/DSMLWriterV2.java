 ...
 */
public class DSMLWriterV2 extends DSMLWriter {
  ...
  // Method:   Ctor
  /**
   ** Constructs an <code>DSMLWriterV2</code> object to write the LDAP data to
   ...
   */
  DSMLWriterV2(final DirectorySchema schema, final File file)
  ...
  public void printEntryStart(String distinguishedName) {
    ...
    this.writer.println(leftIndent("<dsml:addRequest dn=\"" + distinguishedName + "\">", ++this.level * this.indent));
  }
  ...
  public void printEntryEnd(final String distinguishedName) {
    this.writer.println(leftIndent("</dsml:addRequest>", (this.level--) * this.indent));
  }
  ...
  public void printAttribute(final Attribute attribute) {
    try {
      ...
      this.writer.println(leftIndent("<dsml:attr name=\"" + attribute.getID() + "\">",(++this.level) * this.indent));
      this.level++;
