/*
    ...
    File        :   DirectoryContextSimilar.java
...
////////////////////////////////////////////////////////////////////////////////
// class DirectoryContextSimilar
// ~~~~~ ~~~~~~~~~~~~~~~~~~~~~~~
/**
 ** Implementation of the dialog to obtain the information about to create an
 ** entry that derives the object classes and attributes from an existing
 ...
 */
class DirectoryContextSimilar extends    DirectoryContextPanel
  ...
  // compatible with
  @SuppressWarnings("compatibility:5252365965327144739")
  private static final long    serialVersionUID = -6228048828379779070L;
  ...
  // Method:   Ctor
  /**
   ** Constructs an empty <code>DirectoryContextSimilar</code> panel that allows
   ** use as a JavaBean.
   ** <br>
  ...
  public void onEntry(final TraversableContext context) {
    ...
    this.entrySuffix.setText(this.entry.name().suffix().toString());
    ...
    final Map<DirectoryAttribute, DirectoryValue> cache = this.entry.value();
    // get the object classes of the entry used as the template by removing them
    // from the collection of attributes
    final DirectoryValue objectClass = cache.remove(DirectoryAttribute.OBJECTCLASS);

    // the data model to collect object classes equipped with the object classes
    // found at the entry
    this.objectModel = ObjectClassModel.build(objectClass.stream().map(e -> e.value()).collect(Collectors.toSet()));
    ...
    // the data model to collect attribute
  ...
  //////////////////////////////////////////////////////////////////////////////
  // Method:   cleanupAttribute
  /**
   ** Update the entry to remove all required and optional attributes that are
   ** neither creatable or updatable.
   **
   ** @param  name               the name object the object class that extends a
   **                            former declaration.
   **                            <br>
  ...
  private void cleanupAttribute() {
    final Set<DirectoryAttribute> holder = new HashSet<>(this.entry.value().keySet());
    // evaluate all attributes that are not creatable, updateable or declared as
    // obsolste by the schema
    for (DirectoryAttribute cursor : holder) {
      if (cursor.operational() || cursor.name.startsWith("pwd"))
        this.entry.remove(cursor);
    }
  }
}