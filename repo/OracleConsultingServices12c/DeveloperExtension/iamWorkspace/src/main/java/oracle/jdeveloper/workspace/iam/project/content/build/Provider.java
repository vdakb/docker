 ...
 ** belonging to ANT build processes.
 **
...
public class Provider extends FeatureProvider {
  ...
  public Provider(final String featureKey) {
    // ensure inheritance
    super(featureKey, Bundle.string(Bundle.CONTENT_ANTBUILD_CATEGORY));
  }
  ...
  public final void initContentSet(final ContentSet contentSet, final URL directory, final Context context) {
    ensureDefaults(contentSet, context.getProject());
  }
  ...
  public final Navigable getNavigable() {
    return new Navigable(Bundle.string(Bundle.CONTENT_ANTBUILD_CATEGORY), Panel.class);
  }
  ...
  /**
   ** Sets the default values in the specified {@link ContentSet} if no one
   ** exists.
   **
   ** @param  contentSet         the {@link ContentSet} to configure.
   ** @param  project            the {@link Project} the {@link ContentSet} to
   **                            configure belongs to.
   */
  public static void ensureDefaults(final ContentSet contentSet, final Project project) {
    if (contentSet.getAllRootDirs().size() == 0)
      contentSet.getURLPath().add(defaultPath(project));
