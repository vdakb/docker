 ...
 ** preferences stored in the {@link ODSServer} model.
 ** <p>
 ...
 */
public class ODSServerPage extends TemplateFeaturePage {

  //////////////////////////////////////////////////////////////////////////////
  // static final attributes
  //////////////////////////////////////////////////////////////////////////////

  protected static final String            OUD              = "oud";
  protected static final String            OVD              = "ovd";
  ...
  // compatible with
  @SuppressWarnings("compatibility:5007549556158966801")
  private static final long                serialVersionUID = -4685522832704654885L;
  ...
  static {
    PLATFORM.put(OUD, "Unified Directory");
    PLATFORM.put(OID, "Internet Directory");
    PLATFORM.put(OVD, "Virtual Directory");
  };
  ...
  private transient JTextField             serverInstance;
  ...
  private transient ODSServer              provider         = null;
  ...
  // Method:   Ctor
  /**
   ** Constructs an empty <code>ODSServerPage</code> that allows use as a
   ** JavaBean.
   ** <br>
  ...
  protected void initializeComponent() {
    ...
    this.cloneBrowse.addActionListener(new FeatureClone(this, Manifest.ODS_SERVER));
    ...
    this.serverInstance   = new JTextField();
    ...
    this.serverPlatform.setRenderer(new ODSPlatformRenderer(PLATFORM));
  }
  ...
  protected void initializeLayout() {
    ...
    row += 2;
    label = builder.addLabel(ComponentBundle.string(ComponentBundle.SERVER_INSTANCE_LABEL), constraint.xy(2, row));
    label.setLabelFor(this.serverInstance);
    builder.add(this.serverInstance, constraint.xyw(4, row, 7));

    // the 7th logical row of the layout
    row += 2;
    label = builder.addLabel(ComponentBundle.string(ComponentBundle.SERVER_CONTEXT_LABEL), constraint.xy(2, row));
  ...
  protected final void initializePage() {
    ...
    this.serverInstance.setText(this.provider.instance());
    this.serverContext.setText(this.provider.context());
  ...
  public void onEntry(final TraversableContext context) {
    ...
    this.provider = ODSServer.instance(context);
  ...
  //////////////////////////////////////////////////////////////////////////////
  // Method:   commitInstance
  /**
   ** Checks if the instance satisfies all requirements.
   ** <p>
   ** In other terms the entry <code>instance</code> must be a evaluate to a
   ** valid string.
   **
   ** @param  validate           <code>true</code> if validation of the page is
   **                            required.
   ...
   ** @throws TraversalException if the commit should be aborted.
   */
  private final void commitInstance(final boolean validate)
    throws TraversalException {

    // obtained the value from the UI to store.
    final String value = this.serverInstance.getText();

    // check the rules and validate
    if (validate)
       validateString(value, ComponentBundle.string(ComponentBundle.SERVER_INSTANCE_ERROR));

    // store the provided value in the context
    if (!this.provider.instance().equals(value))
      this.provider.instance(value);
  }

  //////////////////////////////////////////////////////////////////////////////
  // Method:   commitContext
  /**
   ** Checks if the context satisfies all requirements.
   ** <p>
   ** In other terms the entry <code>context</code> must be a evaluate to a
   ** valid string.
   **
   ** @param  validate           <code>true</code> if validation of the page is
   **                            required.
   ...
   ** @throws TraversalException if the commit should be aborted.
   */
  private final void commitContext(final boolean validate)
    throws TraversalException {
    ...
    // check the rules and validate
    if (validate)
       validateString(value, ComponentBundle.string(ComponentBundle.SERVER_CONTEXT_ERROR));

    // store the provided value in the context
    if (!this.provider.context().equals(value))
      this.provider.context(value);
  }
