...
public abstract class Hexadecimal {
   ...
   */
  public static int encode(byte[] data, OutputStream stream)
    throws IOException {
   ...
   **                            {@link OutputStream} has been closed.
   */
  public static int encode(byte[] data, int offset, int length, OutputStream stream)
    throws IOException {
   ...
   **                            {@link OutputStream} has been closed.
   */
  public static int decode(final String data, OutputStream stream)
    throws IOException {

    int end = data.length();
    while (end > 0) {
      if (!StringUtility.whitespace(data.charAt(end - 1)))
        break;
      end--;
    }
    ...
    int i = 0;
    while (i < end) {
      while (i < end && StringUtility.whitespace(data.charAt(i)))
        i++;
   ...
   **                            {@link OutputStream} has been closed.
   */
  public static int decode(byte[] data, int offset, int length, OutputStream stream)
    throws IOException {

    int end = offset + length;
    while (end > offset) {
      if (!StringUtility.whitespace((char)data[end - 1]))
        break;
      end--;
    }
    ...
    int i = offset;
    while (i < end) {
      while (i < end && StringUtility.whitespace((char)data[i]))
        i++;
