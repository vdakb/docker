...
public abstract class MimeCodec implements Serializable {
  ...
  // compatible with
  @SuppressWarnings("compatibility:-757598456500073710")
  ...
  public final String canonicalize(final String input) {
    ...
    if (tmp.startsWith(Binary.base16.algorithm))
      ...
      tmp = StringUtility.bytesToString(Base64.getDecoder().decode(tmp.substring(Binary.base64.length)));
  ...
  public String escape(final String input) {
    ...
    final String ouput = writer.toString();
    return StringUtility.printable(ouput) ? ouput : String.format("%s%s", Binary.base64.algorithm, Base64.getEncoder().encode(ouput.getBytes()));
  }
  ...
  private void escape(final Writer writer, final String input)
    ...
    for (int i = 0; i < len; i++) {
      ...
      if (entityName == null) {
        ...
        // escaped
        if (StringUtility.printable(c))
          writer.write(c);
        // we need to check for whitespaces first white spaces are ' ', \n, \r,
        // \t only
        else if (StringUtility.whitespace(c)) {
          writer.write("&#");
  ...
  private void unescape(final Writer writer, final String input, final int escapedAt)
    ...
    for (int i = escapedAt; i < len; i++) {
      ...
      if (c == '&') {
        ...
        int amphersand = input.indexOf('&', i + 1);
        if (amphersand != -1 && amphersand < semiColon) {
