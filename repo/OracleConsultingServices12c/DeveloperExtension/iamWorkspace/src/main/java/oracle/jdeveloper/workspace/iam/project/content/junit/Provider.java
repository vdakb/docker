 ...
 ** belonging to unit test of any kind.
 **
...
public class Provider extends FeatureProvider {
  ...
  public Provider(final String featureKey) {
    // ensure inheritance
    super(featureKey, Bundle.string(Bundle.CONTENT_UNITTEST_CATEGORY));
  }
  ...
  /**
   ** Returns <code>true</code> if the {@link FeatureProvider} may point
   ** to Java sources. Returns <code>false</code> otherwise.
   ** <p>
   ** The default return value is <code>true</code>. When the provider returns
   ...
   **                            {@link FeatureProvider} may point to
   **                            Java sources.
   */
  @Override
   ...
   **                            since most {@link Provider}s derive default
  ...
  public final Navigable getNavigable() {
    return new Navigable(Bundle.string(Bundle.CONTENT_UNITTEST_CATEGORY), Panel.class);
  }
  ...
  // Method:   defaultPath
  /**
   ** Given a project this creates a default unit test path relative the
   ** project's base URL and returns the URL of the new directory.
   **
   ** @param  project            the <code>Locatable</code> project were the
   **                            default path will be obtained from.
   **
   ** @return                    the default unit test path for the given
   **                            project.
   */
  ...
  // Method:   defaultPath
  /**
   ** Returns the default unit test path settings for the IDE.
   ** <br>
   ** "test" by default.
   **
   ** @return                    the default unit test path settings for the
   **                            IDE.
   */
  public static String defaultPath() {
    ...
    final URLPath path = settings.contentSet().getURLPath();
    if (path != null) {
      URL defaultPath = path.getFirstEntry();
      if (defaultPath != null)
        return URLFileSystem.toRelativeSpec(defaultPath, project.getURL());
    }
    return TemplateProjectConfigurator.TEST_DIRECTORY;
  }
