 ...
 */
abstract class DSMLReader extends LDAPReader {
  ...
  protected DSMLReader(final File file, final String root)
    ...
    // initialize instance
    try {
      this.parser = new DSMLInput(this.stream);
    }
    ...
    if (!StringUtility.empty(this.prefix) && StringUtility.empty(this.namespaceURI))
      throw new DirectoryException(Bundle.format(Bundle.DSML_EXPECTIING_NAMESPACE, exceptionPosition(root)));
  }
   ...
   ** @throws DirectoryException if the specified file could not be found.
   */
  public static DSMLReader build (final File file, final int version)
    throws DirectoryException {

    switch (version) {
      case 1  : return new DSMLReaderV1(file);
      case 2  : return new DSMLReaderV2(file);
   ...
   **                            <code>null</code>
   **
   ** @throws DirectoryException if the required values are not matched.
   */
  protected void assertStartElement(final String eventName)
    ...
    try {
      ...
      throw new DirectoryException(Bundle.format(Bundle.DSML_EXPECTIING_OPENING_TAG, exceptionPosition(parameter)));
    }
  }
  ...
  protected void assertEndElement(final String eventName)
    ...
    try {
      ...
      final String[] parameter = {eventName, this.parser.getLocalName()};
      throw new DirectoryException(Bundle.format(Bundle.DSML_EXPECTIING_CLOSING_TAG, exceptionPosition(parameter)));
    }
  }
 ...
 //////////////////////////////////////////////////////////////////////////////
   ...
   **                            parse event.
   **
   ** @throws DirectoryException if there is an error processing the underlying
   **                            DSML source.
   */
  protected int assertNext()
    ...
    try {
      ...
      throw new DirectoryException(Bundle.format(Bundle.DSML_UNEXPECTED_EVENT, exceptionPosition(DSMLInput.eventTypeString(this.parser.getEventType()))));
    }
  }
   ...
   **
   ** @throws DirectoryException if the current event is not white space,
   ...
   **
   ** @throws DirectoryException if the current event is not a
  ...
  protected String elementText()
    ...
    while(eventType != DSMLInput.END_ELEMENT ) {
      if (eventType == DSMLInput.CHARACTERS || eventType == DSMLInput.CDATA || eventType == DSMLInput.SPACE || eventType == DSMLInput.ENTITY_REFERENCE) {
        ...
        throw new DirectoryException(Bundle.format(Bundle.DSML_ROOT_CLOSING_OUTSIDE,  exceptionPosition(this.parser.getLocalName())));
      }
      else if (eventType == DSMLInput.START_ELEMENT) {
        throw new DirectoryException(Bundle.format(Bundle.DSML_TAG_OPENING_NOT_RECOGNIZED,  exceptionPosition(this.parser.getLocalName())));
      }
      else {
        throw new DirectoryException(Bundle.format(Bundle.DSML_UNEXPECTED_EVENT, this.parser.getLocalName()));
      }
