/*
    ...
    11.1.1.3.37.60.27  2012-08-31  DSteding    Fixed Defect DEF-000210
                                               commiting passphrase took the
                                               wrong text field
    11.1.1.3.37.60.32  2012-10-20  DSteding    Cloning of an existing file
                                               implemented.
    11.1.1.3.37.60.34  2013-05-25  DSteding    Fixed Defect DE-000088
                                               Copying SCP Server Properties
                                               obtains password as passphrase
 ...
 ** entering the connection data to the SCP server.
 ** <p>
 ...
 */
public class SCPServerPage extends TemplateFeaturePage {
  ...
  protected static final String             HPUX            = "hpux";
  protected static final String             AIX             = "aix";
  protected static final String             WINDOWS         = "windows";

  // the official serial version ID which says cryptically which version we're
  // compatible with
  @SuppressWarnings("compatibility:2941880056544854667")
  private static final long                serialVersionUID = 385303982757597585L;

  private static final Map<String, String> PLATFORM         = new LinkedHashMap<String, String>();
  ...
  static {
    PLATFORM.put(LINUX,   "Linux");
    PLATFORM.put(SOLARIS, "Solaris");
    PLATFORM.put(HPUX,    "HP-UX");
    PLATFORM.put(AIX,     "AIX");
  ...
  private transient URLTextField      serverCertificate;
  private transient JButton           certificateBrowse;
  private transient JPasswordField    certificatPhrase;
  private transient JPasswordField    certificatConfirm;
  private transient JCheckBox         certificateTrusted;

  private transient SCPServer         provider            = null;
  ...
  // Method:   Ctor
  /**
   ** Constructs an empty <code>SCPServerPage</code> that allows use as a
   ** JavaBean.
   ** <br>
  ...
  protected void initializeComponent() {
    ...
    this.cloneBrowse.addActionListener(new FeatureClone(this, Manifest.SCP_SERVER));
    ...
    this.serverCertificate  = new URLTextField();
    this.certificateBrowse  = new JButton();
    this.certificatPhrase   = new JPasswordField();
    this.certificatConfirm  = new JPasswordField();
    this.certificateTrusted = new JCheckBox();

    // allow to choose files for the server certificate
    this.serverCertificate.setDirectoryOnly(false);

    this.serverPlatform.setModel(new DefaultComboBoxModel<String>(PLATFORM.keySet().toArray(new String[0])));
    this.serverPlatform.setRenderer(new SCPPlatformRenderer(PLATFORM));

    this.certificateBrowse.addActionListener(this.serverCertificate);
  }
  ...
  protected void initializeLayout() {
    ...
    , "pref, pref, 6dlu, pref, 3dlu, pref, 6dlu, pref, 6dlu, pref, 3dlu, pref, 3dlu, pref, 6dlu, pref, 6dlu, pref, 3dlu, pref, 3dlu, pref, 3dlu, pref"
    ...
    row += 2;
    builder.addSeparator(ComponentBundle.string(ComponentBundle.SERVER_CERTIFICAT_HEADER), constraint.xywh(2, row, 9, 1));

    // the 10th logical row of the layout
    row += 2;
    label = builder.addLabel(ComponentBundle.string(ComponentBundle.SERVER_CERIFICATE_LABEL), constraint.xy(2, row));
    label.setLabelFor(this.serverCertificate);
    builder.add(this.serverCertificate, constraint.xyw( 4, row, 5));
    builder.add(this.certificateBrowse, constraint.xyw(10, row, 1));

    // the 11th logical row of the layout
    row += 2;
    label = builder.addLabel(ComponentBundle.string(ComponentBundle.SERVER_PASSPHRASE_LABEL), constraint.xy(2, row));
    label.setLabelFor(this.certificatPhrase);
    builder.add(this.certificatPhrase, constraint.xyw(4, row, 7));

    // the 12th logical row of the layout
    row += 2;
    label = builder.addLabel(ComponentBundle.string(ComponentBundle.SERVER_PASSPHRASE_CONFIRM_LABEL), constraint.xy(2, row));
    label.setLabelFor(this.certificatConfirm);
    builder.add(this.certificatConfirm, constraint.xyw(4, row, 7));

    // the 13th logical row of the layout
    row += 2;
    builder.add(this.certificateTrusted, constraint.xyw(4, row, 7));
 }
  ...
  protected final void localizeComponent() {
    ResourceUtils.resButton(this.certificateTrusted, ComponentBundle.string(ComponentBundle.SERVER_CERIFICATE_TRUST));
    ...
    this.certificateBrowse.setIcon(ComponentBundle.icon(ComponentBundle.FILE_ICON));
    this.certificateBrowse.setToolTipText(ComponentBundle.string(ComponentBundle.SERVER_CERIFICATE_HINT));
    IconicButtonUI.install(this.cloneBrowse);
    IconicButtonUI.install(this.certificateBrowse);
  }
  ...
  protected final void initializePage() {
    ...
    this.serverCertificate.setURL(this.provider.certificate());
    this.certificatPhrase.setText(this.provider.passphrase());
    this.certificatConfirm.setText(this.provider.passphrase());
    this.certificateTrusted.setSelected(this.provider.trusted());
  }
  ...
  protected void commit(final boolean validate)
    ...
    this.provider.trusted(this.certificateTrusted.isSelected());
    ...
    commitCertificate(validate);
    commitPassphrase(validate);
  }
  ...
  public void onEntry(final TraversableContext context) {
    ...
    this.provider = SCPServer.instance(context);
   ...
   ** @throws TraversalException if the commit should be aborted.
   */
  protected final void commitUsername(final boolean validate)
    throws TraversalException {
  ...
  //////////////////////////////////////////////////////////////////////////////
  // Method:   commitCertificate
  /**
   ** Checks if the instance satisfies all requirements.
   ** <p>
   ** In other terms the entry <code>certificate</code> must be evaluate to a
   ** valid file.
   **
   ** @param  validate           <code>true</code> if validation of the page is
   **                            required.
   ...
   ** @throws TraversalException if the commit should be aborted.
   */
  protected final void commitCertificate(final boolean validate)
    throws TraversalException {

    // obtained the value from the UI to store.
    final URL value = this.serverCertificate.getURL();

    // check the rules and validate
    if (validate)
       validateFile(value, ComponentBundle.string(ComponentBundle.SERVER_CERIFICATE_ERROR));

    // store the provided value in the context
    if (!URLFileSystem.equals(this.provider.certificate(), value))
      this.provider.certificate(value);
  }

  //////////////////////////////////////////////////////////////////////////////
  // Method:   commitPassphrase
  /**
   ** Checks if the instance satisfies all requirements.
   ** <p>
   ** In other terms the entry <code>passphrase</code> must be evaluate to a
   ** valid string.
   **
   ** @param  validate           <code>true</code> if validation of the page is
   **                            required.
   ...
   ** @throws TraversalException if the commit should be aborted.
   */
  protected final void commitPassphrase(final boolean validate)
    throws TraversalException {

    // obtained the value from the UI to store.
    final String passphrase = new String(this.certificatPhrase.getPassword());

    if (validate) {
      if (!StringUtility.empty(passphrase)) {
        // obtained the value from the UI to store.
        final String confirmation = new String(this.certificatConfirm.getPassword());
        validateString(confirmation, ComponentBundle.string(ComponentBundle.SERVER_PASSPHRASE_CONFIRM_ERROR));
        if (!passphrase.equals(confirmation))
          throw new TraversalException(ComponentBundle.string(ComponentBundle.SERVER_PASSPHRASE_MISMATCH));
      }
    }

    // store the provided value in the context
    if (!this.provider.passphrase().equals(passphrase))
      this.provider.passphrase(passphrase);
  }
}