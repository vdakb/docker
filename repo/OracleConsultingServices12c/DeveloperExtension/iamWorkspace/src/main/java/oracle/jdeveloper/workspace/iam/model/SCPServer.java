 ...
 ** stored in the {@link SCPServer} model.
 **
 ** @author  dieter.steding@oracle.com
 ** @version 12.2.1.3.42.60.74
 ** @since   11.1.1.3.37.56.13
 */
public class SCPServer extends PlatformConfig {
  ...
  private static final String DEFAULT_USERNAME = "oracle";

  private static final String FILE             = "scp-server";
  ...
  // Method:   Ctor
  /**
   ** Constructs a <code>SCPServer</code>.
   ** <br>
   ** This constructor is private to prevent other classes to use
   ** <code>new SCPServer()</code> and enforces use of the public factory method
   ** below.
   */
  private SCPServer(final HashStructure structure) {
    // ensure inheritance
    super(structure);
  }
  ...
  public String project() {
    // the second parameter to string() is a default value. Defaults are
    // stored in the persistent preferences file using a "placeholder".
    return string(PROJECT, Manifest.string(Manifest.SCP_SERVER_PROJECT));
  }
   ...
   **
   ** @return                    the port the SCP daemon is listening on.
   */
  @Override
  public String port() {
   ...
   **
   ** @param  passphrase         the pass phrase to obraine the certificate.
   */
  public void passphrase(final String passphrase) {
    this._hash.putString(SCPServerHandler.PASSPHRASE, passphrase);
  }

  //////////////////////////////////////////////////////////////////////////////
  // Method:   passphrase
  /**
   ** Returns the pass phrase to obraine the certificate.
   **
   ** @return                    the pass phrase to obraine the certificate.
   */
  public String passphrase() {
    // the second parameter to string() is a default value. Defaults are
    // stored in the persistent preferences file using a "placeholder".
    return string(SCPServerHandler.PASSPHRASE, StringUtility.EMPTY);
  }
   ...
   ** @param  trusted         ...
   */
  public void trusted(final boolean trusted) {
    this._hash.putBoolean(SCPServerHandler.TRUSTED, trusted);
  }
  ...
  public boolean trusted() {
    // The second parameter to getBoolean() is a default value. Defaults are
    // stored in the persistent preferences file using a "placeholder".
    return this._hash.getBoolean(SCPServerHandler.TRUSTED, true);
  }
   ...
   **
   ** @param  certificate        the file name of the certificate to secure the
   **                            authentication with the remote server.
   */
  public void certificate(final URL certificate) {
    this._hash.putURL(SCPServerHandler.CERTIFICAT, certificate);
  }

  //////////////////////////////////////////////////////////////////////////////
  // Method:   certificate
  /**
   ** Returns the file name of the certificate to secure the authentication with
   ** the remote server.
   **
   ** @return                    the file name of the certificate to secure the
   **                            authentication with the remote server.
   */
  public URL certificate() {
    return this.url(SCPServerHandler.CERTIFICAT);
  }
  ...
  public final void platform(final String platform) {
    this._hash.putString(SCPServerHandler.PLATFORM, platform);
  }
  ...
  public final String platform() {
    // the second parameter to string() is a default value. Defaults are
    // stored in the persistent preferences file using a "placeholder".
    return string(SCPServerHandler.PLATFORM, StringUtility.EMPTY);
  }
  ...
  public final void username(final String username) {
    this._hash.putString(SCPServerHandler.USERNAME, username);
  }
  ...
  public final String username() {
    // the second parameter to string() is a default value. Defaults are
    // stored in the persistent preferences file using a "placeholder".
    return string(SCPServerHandler.USERNAME, DEFAULT_USERNAME);
  }
  ...
  public final void password(final String password) {
    this._hash.putString(SCPServerHandler.PASSWORD, password);
  }
  ...
  public final String password() {
    return string(SCPServerHandler.PASSWORD, StringUtility.EMPTY);
  }
   ...
   ** using {@link Manifest#SCP_SERVER} in the design time objects of the
   ...
   **
   ** @return                    the <code>SCPServer</code> instance obtained
   **                            from the specifed {@link TraversableContext}.
   */
  public static SCPServer instance(final TraversableContext context) {
    SCPServer instance = (SCPServer)context.getDesignTimeObject(Manifest.SCP_SERVER);
    if (instance == null) {
      instance = new SCPServer(HashStructure.newInstance());
      context.putDesignTimeObject(Manifest.SCP_SERVER, instance);
  ...
  public void clone(final URL source) {
    SCPServerHandler handler = new SCPServerHandler(source);
    try {
      handler.load();
      platform(handler.propertyValue(SCPServerHandler.PLATFORM));
      host(handler.propertyValue(SCPServerHandler.HOST));
      port(handler.propertyValue(SCPServerHandler.PORT));
      username(handler.propertyValue(SCPServerHandler.USERNAME));
      password(handler.propertyValue(SCPServerHandler.PASSWORD));
      certificate(URLFactory.newFileURL(handler.propertyValue(SCPServerHandler.CERTIFICAT)));
      passphrase(handler.propertyValue(SCPServerHandler.PASSPHRASE));
      trusted(Boolean.parseBoolean(handler.propertyValue(SCPServerHandler.TRUSTED)));
    }
