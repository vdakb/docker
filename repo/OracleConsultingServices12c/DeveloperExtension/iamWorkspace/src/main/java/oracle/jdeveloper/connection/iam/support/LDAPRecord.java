 ...
 ** <ul>
 **   <li><code>DirectoryAddContent</code> (represents the content of an LDAP
 **       record that adds a new entry)
 **   <li><code>DirectoryDeleteContent</code> (represents the content of an LDAP
 **       record that deletes an entry)
 **   <li><code>DirectoryModifyContent</code> (represents the content of an LDAP
 **       record that modifies an entry)
 **   <li><code>DirectoryModDNContent</code> (represents the content of an LDAP
...
public class LDAPRecord {
   ...
   **
   ** @throws DirectoryException if the attribute is a distinguished name.
   */
  ...
  public void add(final Attribute attribute)
    ...
    if (DirectoryService.DN.equalsIgnoreCase(attribute.getID()))
      throw new DirectoryException(Bundle.format(Bundle.LDIF_UNEXPECTED, DirectoryService.DN));
  ...
  public final void toStream(final LDAPWriter writer) {
    ...
    try {
      // write out the DN.
      final DirectoryName namespace = DirectoryName.build(this.namespace);
      ...
      Attribute out = this.attributes.get(attributeName.get(DirectoryService.OBJECTCLASS.toLowerCase()));
      if (out != null)
      ...
      for (Enumeration<? extends Attribute> ea = this.attributes.getAll(); ea.hasMoreElements(); ) {
        out = ea.nextElement();
        if ((!DirectoryService.OBJECTCLASS.equalsIgnoreCase(out.getID())) && (!entryRDN.getType().equalsIgnoreCase(out.getID()))) {
          writer.printAttribute(out);
        }
      }
      /*
