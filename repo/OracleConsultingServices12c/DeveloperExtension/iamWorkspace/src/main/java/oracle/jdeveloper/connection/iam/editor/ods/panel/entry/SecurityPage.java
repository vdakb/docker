...
// ~~~~~ ~~~~~~~~~~~~
/**
 ** A flat editor panel suitable to display all security contraints (ACI's) of a
 ** Directory Service entry in a editor.
 **
...
public class SecurityPage extends AbstractPage {
  ...
  // compatible with
  @SuppressWarnings("compatibility:2285439718941076073")
  private static final long serialVersionUID = -4384460449589918084L;
  ...
  // Method:   Ctor
  /**
   ** Constructs an empty <code>SecurityPage</code> responsible to visualize the
   ...
   */
  private SecurityPage(final Namespace data) {
    // ensure inheritance
  ...
  protected final void initializeView() {
    this.panel = SecurityPanel.build(EntryEditor.SECURITY, this.data);
    initializeHeaderPanel(Bundle.string(Bundle.ENTRY_SECURITY_PANEL_TITLE), this.panel);
  }
  ...
  // Method:   build
  /**
   ** Factory method to create a <code>SecurityPage</code> that populates its
   ...
   **                            Allowed object is {@link Namespace}.
   **
   ** @return                    the validated <code>SecurityPage</code>.
   **                            <br>
   **                            Possible object <code>SecurityPage</code>.
   */
  public static SecurityPage build(final Namespace data) {
    return new SecurityPage(data);
  }
