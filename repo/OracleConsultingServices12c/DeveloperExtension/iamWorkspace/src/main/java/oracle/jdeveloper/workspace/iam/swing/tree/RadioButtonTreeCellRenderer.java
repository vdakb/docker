/*
    ...
    File        :   RadioButtonTreeCellRenderer.java
...
// ~~~~~ ~~~~~~~~~~~~~~~~~~~~~~~~~~~
/**
 ** Displays an option entry (RadioButton) in a tree.
 ** <p>
 ** <code>RadioButtonTreeCellRenderer</code> is not opaque and unless you
 ...
 */
public class RadioButtonTreeCellRenderer extends OptionTreeCellRenderer {
  ...
  // compatible with
  @SuppressWarnings("compatibility:-5067985838117772381")
  private static final long         serialVersionUID = 4233144417500897L;

  /**
   ** The component that is used to paint the hotspot button in cell renderer if
   ** it is visible
   */
  private static final JRadioButton PROTOTYP         = new TriStateRadioButton();
  ...
  // Method: Ctor
  /**
   ** Constructs a <code>RadioButtonTreeCellRenderer</code> that has a radio
   ** button and a label.
   */
  ...
  // Method: Ctor
  /**
   ** Constructs a <code>RadioButtonTreeCellRenderer</code> that has a toggle
   ** button and a label.
   ** <p>
   ...
   */
  public RadioButtonTreeCellRenderer(final ContentTreeCellRenderer renderer) {
    // ensure inheritance
    super(renderer);
  ...
  public String getToolTipText(final MouseEvent event) {
    return toolTipText(event, ((TriStateRadioButton)this.hotspot).getWidth());
  }
  ...
  public Component getTreeCellRendererComponent(final JTree tree, final Object value, final boolean selected, final boolean expanded, final boolean leaf, final int row, final boolean focused) {
   ((TriStateRadioButton)this.hotspot).setPreferredSize(PROTOTYP.getPreferredSize());
...
//      renderer.setBorder(BorderFactory.createEmptyBorder());
  ...
  public boolean hotspotHit(final int x, final int y) {
    return ((TriStateRadioButton)this.hotspot).contains(x, y);
  }
  ...
  protected final void renderHotspot(final OptionTree tree, final OptionTreeNodeData data, final boolean selected, final int row) {
    ...
    if ((data != null && data.hotspotVisible())) {
     ...
     ((TriStateRadioButton)this.hotspot).setEnabled(tree.isEnabled() ? data.hotspotEnabled() : false);
    }
