 ...
 ** editing the properties stored in the {@link ModelData} model.
 ** <p>
...
class ParameterData extends DefaultTraversablePanel {
  ...
  // compatible with
  @SuppressWarnings("compatibility:3737735679884246370")
  private static final long        serialVersionUID = 2789395555146844336L;
  ...
  private transient ModelData      data;
  ...
  public void onEntry(final TraversableContext context) {
    ...
    this.fileEdit.setText(this.data.id());

    this.parameterForm.clearCanvas();

    if (this.data instanceof ModelStream) {
      final ModelStream stream = (ModelStream)this.data;
   ...
   */
  protected ModelData template(final TraversableContext context) {
    ...
    return (ModelData)node.getUserObject();
  }
  ...
  private void commitFolder(final boolean destructive)
    ...
    if (!validateFolder(value)) {
      ...
      if (!confirmViolation(ModelBundle.string(ModelBundle.BUILDFILE_FOLDER_TITLE), ModelBundle.string(ModelBundle.BUILDFILE_FOLDER_FAILURE), destructive))
        throw new TraversalException(ModelBundle.string(ModelBundle.BUILDFILE_FOLDER_TITLE));
    }
...
//      this.data.folder().id(value);
  ...
  private void commitName(final boolean destructive)
    ...
    if (StringUtility.empty(value)) {
      if (!confirmViolation(ModelBundle.string(ModelBundle.BUILDFILE_NAME_TITLE), ModelBundle.string(ModelBundle.BUILDFILE_NAME_FAILURE), destructive))
        throw new TraversalException(ModelBundle.string(ModelBundle.BUILDFILE_NAME_TITLE));
    }

    if (!this.data.id().equals(value))
      this.data.id(value);
  }
  ...
  private void localizeComponent() {
    ResourceUtils.resButton(this.folderBrowse, ModelBundle.string(ModelBundle.BUILDFILE_FOLDER_BROWSE));
  ...
  private void initializeLayout() {
    ...
    row += 2;
    label = builder.addLabel(ModelBundle.string(ModelBundle.BUILDFILE_NAME_LABEL), constraint.xyw(4, row++, 4));
    label.setLabelFor(this.fileEdit);
    builder.add(this.fileEdit,      constraint.xy( 4, ++row));

    // the 5th logical row of the layout
    row += 2;
    builder.addSeparator(ModelBundle.string(ModelBundle.BUILDFILE_PROPERTIES), constraint.xyw(2, row, 5));
