 ...
 ** @since   1.0.0.0
 */
class DSMLReaderV2 extends DSMLReader {
  ...
  // Method:   Ctor
  /**
   ** Constructs an <code>DSMLReaderV2</code> object to parse the DSML from a
   ** specified file.
   ** <p>
   ...
   */
  protected DSMLReaderV2(final File file)
    ...
    // value hence
    if (StringUtility.empty(this.prefix)) {
  ...
  //////////////////////////////////////////////////////////////////////////////
  // Method:   nextRecord (DirectoryReader)
  /**
   ...
   **
   ** @throws DirectoryException if there is a fatal error detecting the next
   **                            state.
   */
  private boolean hasNext()
    ...
    if ((eventType == DSMLInput.END_ELEMENT) && (RequestType.ADD.value.equals(eventName) || RequestType.MODIFY.value.equals(eventName) || RequestType.DELETE.value.equals(eventName) || RequestType.RENAME.value.equals(eventName))) {
      ...
      throw new DirectoryException(Bundle.format(Bundle.DSML_UNEXPECTED_EVENT, exceptionPosition(eventName)));
    }
  }
  ...
  private LDAPAddContent addRequest()
    ...
    if (this.parser.getAttributeCount() == 0) {
      ...
      throw new DirectoryException(Bundle.format(Bundle.DSML_EXPECTIING_ATTRIBUTE, exceptionPosition(parameter)));
    }
    if (this.parser.getAttributeCount() > 1) {
      final String[] parameter = {DSMLGrammar.V2.AddRequest.Attributes.NAME, DSMLGrammar.V2.AddRequest.Attributes.DN};
      throw new DirectoryException(Bundle.format(Bundle.DSML_EXPECTIING_ATTRIBUTE_ONCE, exceptionPosition(parameter)));
    }
  ...
  private String parseValue(final String type)
    ...
    String value = encoded ? new String(Base64.base64Decode(elementText())) : elementText();
