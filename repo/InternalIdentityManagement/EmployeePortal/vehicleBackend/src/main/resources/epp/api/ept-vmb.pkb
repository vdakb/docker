-- D:\Project\Bundeskriminalamt12c\InternalIdentityManagement\EmployeePortal\portalBackend\..\vehicleBackend\src\main\resources\dbs\epp\api\ept-vmb.pkb
--
-- Generated for Oracle Database 12c on Sun May 24 17:30:58 2020 by Server Generator 10.1.2.93.10

PROMPT Creating API Package Implementation for Table 'ept_vehicle_brands'
--------------------------------------------------------------------------------
-- Name:        cg$ept_vehicle_brands
-- Description: cg$ept_vehicle_brands table API package definitions
--------------------------------------------------------------------------------
CREATE OR REPLACE PACKAGE BODY cg$ept_vehicle_brands
IS
...
BEGIN
  cg$errors.push( cg$errors.get_text(cg$errors.API_UNIQUE_KEY_UPDATE, cg$errors.ERR_UK_UPDATE, uk)
                ...
                , 'cg$ept_vehicle_brands.raise_uk_not_updateable'
                );
...
BEGIN
  cg$errors.push( cg$errors.get_text(cg$errors.API_FOREIGN_KEY_TRANS, cg$errors.ERR_FK_TRANS, fk)
                ...
                , 'cg$ept_vehicle_brands.raise_fk_not_transferable'
                );
  cg$errors.raise_failure;
...
EXCEPTION
  ...
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$ept_vehicle_brands.up_autogen_columns.denorm');
    cg$errors.raise_failure;
...
BEGIN
  ...
  THEN
    cg$errors.push(cg$errors.get_text(cg$errors.API_MAND_COLUMN_ISNULL, cg$errors.VAL_MAND, P40NAME), 'E', 'API', cg$errors.API_MAND_COLUMN_ISNULL, loc);
  END IF;
  IF (cg$val_rec.icon IS NULL)
  THEN
    cg$errors.push(cg$errors.get_text(cg$errors.API_MAND_COLUMN_ISNULL, cg$errors.VAL_MAND, P45ICON), 'E', 'API', cg$errors.API_MAND_COLUMN_ISNULL, loc);
  END IF;
...
BEGIN
  ...
  THEN
    ...
    INSERT INTO ept_vehicle_brands (
      id
    , rowversion
    , created_by
    , created_on
...
EXCEPTION
  ...
  THEN
    validate_mandatory(cg$rec, 'cg$ept_vehicle_brands.ins.mandatory_missing');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.check_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_CHECK_CON, 'cg$ept_vehicle_brands.ins.check_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.fk_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_FOREIGN_KEY, 'cg$ept_vehicle_brands.ins.fk_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.uk_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_UNIQUE_KEY, 'cg$ept_vehicle_brands.ins.uk_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$$ept_vehicle_brands.ins.others');
    called_from_package := FALSE;
    cg$errors.raise_failure;
END ins;
...
BEGIN
  ...
  THEN
    sel(cg$upd_rec);
    -- Report error if attempt to update non updateable Primary Key EPT_VMB_PK
    IF (cg$ind.id AND cg$rec.id != cg$upd_rec.id)
    THEN
      raise_uk_not_updateable('EPP_VMB_PK');
    END IF;
    IF NOT (cg$ind.id)
    THEN
...
EXCEPTION
  ...
  THEN
    validate_mandatory(cg$rec, 'cg$ept_vehicle_brands.upd.upd_mandatory_null');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.check_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_CHECK_CON, 'cg$ept_vehicle_brands.upd.check_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.fk_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_FOREIGN_KEY, 'cg$ept_vehicle_brands.upd.fk_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.uk_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_UNIQUE_KEY, 'cg$ept_vehicle_brands.upd.uk_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$ept_vehicle_brands.upd.others');
    called_from_package := FALSE;
    cg$errors.raise_failure;
END upd;
...
EXCEPTION
  ...
  THEN
    err_msg(SQLERRM, cg$errors.ERR_DELETE_RESTRICT, 'cg$ept_vehicle_brands.upd.delete_restrict');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN no_data_found
  THEN
    cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_DEL, cg$errors.ROW_DEL), 'E', 'ORA', SQLCODE, 'cg$ept_vehicle_brands.del.no_data_found');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$ept_vehicle_brands.del.others');
    called_from_package := FALSE;
    cg$errors.raise_failure;
END del;
...
EXCEPTION
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$ept_vehicle_brands.sel.others');
    cg$errors.raise_failure;
END sel;
...
BEGIN
  ...
  EXCEPTION
    ...
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_LCK, cg$errors.ROW_LCK), 'E', 'ORA', SQLCODE, 'cg$ept_vehicle_brands.lck.resource_busy');
      cg$errors.raise_failure;
    WHEN no_data_found
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_DEL, cg$errors.ROW_DEL), 'E', 'ORA', SQLCODE, 'cg$$ept_vehicle_brands.lck.no_data_found');
      cg$errors.raise_failure;
    WHEN OTHERS
    THEN
      cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$ept_vehicle_brands.lck.others');
      cg$errors.raise_failure;
  END;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P10ID), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$ept_vehicle_brands.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P15ROWVERSION), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$ept_vehicle_brands.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P20CREATED_BY), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$ept_vehicle_brands.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P25CREATED_ON), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$ept_vehicle_brands.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P30UPDATED_BY), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$ept_vehicle_brands.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P35UPDATED_ON), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$ept_vehicle_brands.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  IF (cg$old_ind.name)
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P40NAME), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$ept_vehicle_brands.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  IF (cg$old_ind.icon)
  THEN
    IF (cg$tmp_rec.icon != cg$old_rec.icon)
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P45ICON), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$ept_vehicle_brands.lck');
      any_modified := TRUE;
    END IF;
  END IF;
...
-- Name:        cascade_update
--
-- Description: Updates all child tables affected by a change to ept_vehicle_brands
--
-- Parameters:  cg$rec     Record of ept_vehicle_brands current values
--              cg$old_rec Record of ept_vehicle_brands previous values
--------------------------------------------------------------------------------
...
--              of each child table that references this table ept_vehicle_brands
--
-- Parameters:  cg$old_rec     Record of ept_vehicle_brands current values
--------------------------------------------------------------------------------
...
--              child table that references this table ept_vehicle_brands
--
-- Parameters:  cg$new_rec  New values for ept_vehicle_brands's domain key constraint columns
--              cg$new_ind  Indicates changed ept_vehicle_brands's domain key constraint columns
--              cg$old_rec  Current values for ept_vehicle_brands's domain key constraint columns
--------------------------------------------------------------------------------
...
-- Name:        cascade_delete
--
-- Description: Delete all child tables affected by a delete to ept_vehicle_brands
--
-- Parameters:  cg$rec     Record of ept_vehicle_brands current values
--------------------------------------------------------------------------------
...
EXCEPTION
  ...
  WHEN no_tabview
  THEN
    cg$errors.push( cg$errors.get_text(cg$errors.API_RV_TAB_NOT_FOUND, cg$errors.APIMSG_RV_TAB_NOT_FOUND, '"epp REF_CODES"','ept_vehicle_brands')
                  , 'E'
                  , 'API'
                  , 'cg$errors.API_RV_TAB_NOT_FOUND'
                  , 'cg$ept_vehicle_brands.v_domain.no_reftable_found'
                  );
    cg$errors.raise_failure;
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$ept_vehicle_brands.v_domain.no_reftable_found');
    cg$errors.raise_failure;
END validate_domain;
...
--
-- Description: Update the Domain Constraint Key columns of ept_vehicle_brands when the
--              Cascade Update rule is Cascades and the domain table has been
--              updated. Called from <Domain Table pkg>.domain_cascade_update().
--
-- Parameters:  cg$rec      New values for ept_vehicle_brands's domain key constraint columns
--              cg$ind      Indicates changed ept_vehicle_brands's domain key constraint columns
--              cg$old_rec  Current values for ept_vehicle_brands's domain key constraint columns
--------------------------------------------------------------------------------
...
BEGIN
  ...
  IF (con_name = 'EPT_VMB_PK')
  THEN
    cg$errors.push( NVL(EPT_VMB_PK, cg$errors.get_text(cg$errors.API_PK_CON_VIOLATED, cg$errors.APIMSG_PK_VIOLAT, 'EPT_VMB_PK', 'ept_vehicle_brands'))
                  , 'E'
                  , 'API'
...
BEGIN
  ...
  cg$ind_true.icon       := TRUE;
END cg$ept_vehicle_brands;
/
SHOW ERROR
