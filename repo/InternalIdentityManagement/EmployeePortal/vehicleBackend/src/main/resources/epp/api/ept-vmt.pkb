-- D:\Project\Bundeskriminalamt12c\InternalIdentityManagement\EmployeePortal\portalBackend\..\vehicleBackend\src\main\resources\dbs\epp\api\ept-vmt.pkb
--
-- Generated for Oracle Database 12c on Sun May 24 17:30:58 2020 by Server Generator 10.1.2.93.10

PROMPT Creating API Package Implementation for Table 'ept_vehicle_brand_types'
--------------------------------------------------------------------------------
-- Name:        cg$ept_vehicle_brand_types
-- Description: cg$ept_vehicle_brand_types table API package definitions
--------------------------------------------------------------------------------
CREATE OR REPLACE PACKAGE BODY cg$ept_vehicle_brand_types
IS
...
BEGIN
  cg$errors.push( cg$errors.get_text(cg$errors.API_UNIQUE_KEY_UPDATE, cg$errors.ERR_UK_UPDATE, uk)
                ...
                , 'cg$ept_vehicle_brand_types.raise_uk_not_updateable'
                );
...
BEGIN
  cg$errors.push( cg$errors.get_text(cg$errors.API_FOREIGN_KEY_TRANS, cg$errors.ERR_FK_TRANS, fk)
                ...
                , 'cg$ept_vehicle_brand_types.raise_fk_not_transferable'
                );
  cg$errors.raise_failure;
...
EXCEPTION
  ...
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$ept_vehicle_brand_types.up_autogen_columns.denorm');
    cg$errors.raise_failure;
...
BEGIN
  ...
  THEN
    cg$errors.push(cg$errors.get_text(cg$errors.API_MAND_COLUMN_ISNULL, cg$errors.VAL_MAND, P10VMB_ID), 'E', 'API', cg$errors.API_MAND_COLUMN_ISNULL, loc);
  END IF;
  IF (cg$val_rec.vht_id IS NULL)
  THEN
    cg$errors.push(cg$errors.get_text(cg$errors.API_MAND_COLUMN_ISNULL, cg$errors.VAL_MAND, P15VHT_ID), 'E', 'API', cg$errors.API_MAND_COLUMN_ISNULL, loc);
  END IF;
...
BEGIN
  ...
  THEN
    ...
    INSERT INTO ept_vehicle_brand_types (
      vmb_id
    , vht_id
    , rowversion
    , created_by
...
EXCEPTION
  ...
  THEN
    validate_mandatory(cg$rec, 'cg$ept_vehicle_brand_types.ins.mandatory_missing');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.check_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_CHECK_CON, 'cg$ept_vehicle_brand_types.ins.check_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.fk_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_FOREIGN_KEY, 'cg$ept_vehicle_brand_types.ins.fk_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.uk_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_UNIQUE_KEY, 'cg$ept_vehicle_brand_types.ins.uk_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$$ept_vehicle_brand_types.ins.others');
    called_from_package := FALSE;
    cg$errors.raise_failure;
END ins;
...
BEGIN
  ...
  IF (cg$pk.vmb_id IS NULL)
  THEN
    cg$upd_rec.vmb_id := cg$rec.vmb_id;
  ELSE
    cg$upd_rec.vmb_id := cg$pk.vmb_id;
  END IF;
  IF (cg$pk.vht_id IS NULL)
  THEN
    cg$upd_rec.vht_id := cg$rec.vht_id;
  ELSE
    cg$upd_rec.vht_id := cg$pk.vht_id;
  END IF;
  cg$old_rec.vmb_id := cg$upd_rec.vmb_id;
  cg$old_rec.vht_id := cg$upd_rec.vht_id;
  ...
  THEN
    sel(cg$upd_rec);
    -- Report error if attempt to update non updateable Primary Key EPT_VMT_PK
    IF ((cg$ind.vmb_id AND cg$rec.vmb_id != cg$upd_rec.vmb_id)
    OR  (cg$ind.vht_id AND cg$rec.vht_id != cg$upd_rec.vht_id)
    )
    THEN
      raise_uk_not_updateable('EPP_VMT_PK');
    END IF;
  ...
  ELSE
    ...
    NULL;
    -- Report error if attempt to update non updateable Primary Key EPT_VMT_PK
    IF (cg$ind.vmb_id OR cg$ind.vht_id)
    THEN
      raise_uk_not_updateable('EPP_VMT_PK');
    END IF;
  END IF;
  ...
  THEN
    ...
    BEGIN
      ...
      THEN
        UPDATE ept_vehicle_brand_types
        ...
        WHERE  vmb_id      = cg$rec.vmb_id
        AND    vht_id      = cg$rec.vht_id
        ;
      ELSE
        UPDATE ept_vehicle_brand_types
        SET    rowversion  = cg$rec.rowversion
...
EXCEPTION
  ...
  THEN
    validate_mandatory(cg$rec, 'cg$ept_vehicle_brand_types.upd.upd_mandatory_null');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.check_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_CHECK_CON, 'cg$ept_vehicle_brand_types.upd.check_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.fk_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_FOREIGN_KEY, 'cg$ept_vehicle_brand_types.upd.fk_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.uk_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_UNIQUE_KEY, 'cg$ept_vehicle_brand_types.upd.uk_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$ept_vehicle_brand_types.upd.others');
    called_from_package := FALSE;
    cg$errors.raise_failure;
END upd;
...
BEGIN
  ...
  THEN
    DECLARE
      ...
      cg$ind cg$ind_type;
    BEGIN
      cg$rec.vmb_id := cg$pk.vmb_id;
      cg$rec.vht_id := cg$pk.vht_id;
      sel(cg$rec);
      ...
      IF cg$pk.the_rowid IS NULL
      THEN
        DELETE ept_vehicle_brand_types
        WHERE  vmb_id = cg$pk.vmb_id
        AND    vht_id = cg$pk.vht_id
        ;
      ELSE
        DELETE ept_vehicle_brand_types
...
EXCEPTION
  ...
  THEN
    err_msg(SQLERRM, cg$errors.ERR_DELETE_RESTRICT, 'cg$ept_vehicle_brand_types.upd.delete_restrict');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN no_data_found
  THEN
    cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_DEL, cg$errors.ROW_DEL), 'E', 'ORA', SQLCODE, 'cg$ept_vehicle_brand_types.del.no_data_found');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$ept_vehicle_brand_types.del.others');
    called_from_package := FALSE;
    cg$errors.raise_failure;
END del;
...
BEGIN
  ...
  THEN
    ...
    FROM   ept_vehicle_brand_types
    WHERE  vmb_id = cg$sel_rec.vmb_id
    AND    vht_id = cg$sel_rec.vht_id
    ;
  ELSE
    SELECT vmb_id
    ,      vht_id
...
EXCEPTION
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$ept_vehicle_brand_types.sel.others');
    cg$errors.raise_failure;
END sel;
...
BEGIN
  ...
  BEGIN
    ...
    THEN
      ...
      THEN
        ...
        FROM   ept_vehicle_brand_types
        WHERE  vmb_id = cg$old_rec.vmb_id
        AND    vht_id = cg$old_rec.vht_id
        FOR UPDATE NOWAIT
        ;
      ELSE
        SELECT vmb_id
  ...
  EXCEPTION
    ...
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_LCK, cg$errors.ROW_LCK), 'E', 'ORA', SQLCODE, 'cg$ept_vehicle_brand_types.lck.resource_busy');
      cg$errors.raise_failure;
    WHEN no_data_found
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_DEL, cg$errors.ROW_DEL), 'E', 'ORA', SQLCODE, 'cg$$ept_vehicle_brand_types.lck.no_data_found');
      cg$errors.raise_failure;
    WHEN OTHERS
    THEN
      cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$ept_vehicle_brand_types.lck.others');
      cg$errors.raise_failure;
  END;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P10VMB_ID), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$ept_vehicle_brand_types.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P15VHT_ID), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$ept_vehicle_brand_types.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P20ROWVERSION), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$ept_vehicle_brand_types.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P25CREATED_BY), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$ept_vehicle_brand_types.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P30CREATED_ON), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$ept_vehicle_brand_types.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P35UPDATED_BY), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$ept_vehicle_brand_types.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P40UPDATED_ON), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$ept_vehicle_brand_types.lck');
      any_modified := TRUE;
    END IF;
  END IF;
...
-- Name:        cascade_update
--
-- Description: Updates all child tables affected by a change to ept_vehicle_brand_types
--
-- Parameters:  cg$rec     Record of ept_vehicle_brand_types current values
--              cg$old_rec Record of ept_vehicle_brand_types previous values
--------------------------------------------------------------------------------
...
--              of each child table that references this table ept_vehicle_brand_types
--
-- Parameters:  cg$old_rec     Record of ept_vehicle_brand_types current values
--------------------------------------------------------------------------------
...
--              child table that references this table ept_vehicle_brand_types
--
-- Parameters:  cg$new_rec  New values for ept_vehicle_brand_types's domain key constraint columns
--              cg$new_ind  Indicates changed ept_vehicle_brand_types's domain key constraint columns
--              cg$old_rec  Current values for ept_vehicle_brand_types's domain key constraint columns
--------------------------------------------------------------------------------
...
-- Name:        cascade_delete
--
-- Description: Delete all child tables affected by a delete to ept_vehicle_brand_types
--
-- Parameters:  cg$rec     Record of ept_vehicle_brand_types current values
--------------------------------------------------------------------------------
...
EXCEPTION
  ...
  WHEN no_tabview
  THEN
    cg$errors.push( cg$errors.get_text(cg$errors.API_RV_TAB_NOT_FOUND, cg$errors.APIMSG_RV_TAB_NOT_FOUND, '"epp REF_CODES"','ept_vehicle_brand_types')
                  , 'E'
                  , 'API'
                  , 'cg$errors.API_RV_TAB_NOT_FOUND'
                  , 'cg$ept_vehicle_brand_types.v_domain.no_reftable_found'
                  );
    cg$errors.raise_failure;
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$ept_vehicle_brand_types.v_domain.no_reftable_found');
    cg$errors.raise_failure;
END validate_domain;
...
--
-- Description: Update the Domain Constraint Key columns of ept_vehicle_brand_types when the
--              Cascade Update rule is Cascades and the domain table has been
--              updated. Called from <Domain Table pkg>.domain_cascade_update().
--
-- Parameters:  cg$rec      New values for ept_vehicle_brand_types's domain key constraint columns
--              cg$ind      Indicates changed ept_vehicle_brand_types's domain key constraint columns
--              cg$old_rec  Current values for ept_vehicle_brand_types's domain key constraint columns
--------------------------------------------------------------------------------
...
BEGIN
  ...
  IF (con_name = 'EPT_VMT_PK')
  THEN
    cg$errors.push( NVL(EPT_VMT_PK, cg$errors.get_text(cg$errors.API_PK_CON_VIOLATED, cg$errors.APIMSG_PK_VIOLAT, 'EPT_VMT_PK', 'ept_vehicle_brand_types'))
                  , 'E'
                  , 'API'
                  , 'cg$errors.API_PK_CON_VIOLATED'
                  , loc
                  );
  ELSIF (con_name = 'EPT_VMT_VMB_FK')
  THEN
    cg$errors.push( NVL(EPT_VMT_VMB_FK, cg$errors.get_text(cg$errors.API_FK_CON_VIOLATED, cg$errors.APIMSG_FK_VIOLAT, 'EPT_VMT_VMB_FK', 'ept_vehicle_brand_types'))
                  , 'E'
                  , 'API'
                  , 'cg$errors.API_FK_CON_VIOLATED'
                  , loc
                  );
  ELSIF (con_name = 'EPT_VMT_VHT_FK')
  THEN
    cg$errors.push( NVL(EPT_VMT_VHT_FK, cg$errors.get_text(cg$errors.API_FK_CON_VIOLATED, cg$errors.APIMSG_FK_VIOLAT, 'EPT_VMT_VHT_FK', 'ept_vehicle_brand_types'))
                  , 'E'
                  , 'API'
                  , 'cg$errors.API_FK_CON_VIOLATED'
                  , loc
