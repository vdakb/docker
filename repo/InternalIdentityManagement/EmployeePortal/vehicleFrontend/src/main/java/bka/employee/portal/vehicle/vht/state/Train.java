...
import bka.employee.portal.vehicle.vht.backing.AttributeState;
 ...
 ** pages to create or manage <code>Vehicule Type</code>.
 **
 ** @author  sylvert.bernet@oracle.com
 ** @version 1.0.0.0
 ** @since   1.0.0.0
...
public class Train extends AbstractBean {
  ...
  // compatible with
  @SuppressWarnings("compatibility:-7930338505900802088")
  private static final long serialVersionUID = -4311695549642677003L;
  ...
  private AttributeState attributeState() {
    try {
      return JSF.valueFromExpression("#{backingBeanScope.typeAttribute}", AttributeState.class);
    }
   ...
   ** <br>
   ** Leverage the operational binding <code>typeFetch</code> to do so.
   */
  public void fetch() {
    ADF.executeOperation("typeFetch");
  }

  //////////////////////////////////////////////////////////////////////////////
  // Method:   submit
  /**
   ** Create or updates the <code>Vehicule Type</code>, open a newly
   ** created <code>Vehicule Type</code> in the edit mode and close
  ...
  public String submit() {
    ...
    // needed to create or update the Vehicule Type
    
    final Object result = ADF.executeAction(SUBMIT, parameter);
    if (ADF.executeActionSucceeded(result)) {
      final RowSetIterator rowSet = rowSetIterator("TypeIterator");
      ...
      if (MODE_CREATE.equals(scope.get(PARAMETER_MODE))) {
        raiseTaskFlowFeedbackEvent(String.format(ADF.resourceBundleValue(BUNDLE, "VHT_TRAIN_CREATE"), name));
        raiseTaskFlowRemoveEvent(flow);
      }
      else {
        markClean();
        raiseTaskFlowFeedbackEvent(String.format(ADF.resourceBundleValue(BUNDLE, "VHT_TRAIN_MODIFY"), name));
  ...
  public void revert(final @SuppressWarnings("unused") ActionEvent event) {
    ...
    raiseTaskFlowFeedbackEvent(String.format(ADF.resourceBundleValue(BUNDLE, "VHT_TRAIN_REVERT"), name));
  }
  ...
  public void revertAttribute() {
    ADF.executeOperation("refreshTypeAttribute");
    // need to explicitly reset Type attributes forms this
