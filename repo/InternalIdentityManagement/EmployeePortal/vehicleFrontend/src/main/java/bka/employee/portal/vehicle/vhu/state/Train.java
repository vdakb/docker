 ...
 ** pages to create or manage <code>Vehicule</code>.
 **
 ** @author  sylvert.bernet@oracle.com
 ** @version 1.0.0.0
 ** @since   1.0.0.0
...
public class Train extends    AbstractBean
  ...
  // compatible with
  @SuppressWarnings("compatibility:8847942915257293435")
  private static final long    serialVersionUID = -8327871628814690806L;
  ...
  private AttributeState attributeState() {
    try {
      return JSF.valueFromExpression("#{backingBeanScope.vehicleAttribute}", AttributeState.class);
    }
  ...
  /**
   ** Method name attribute represents the action listener to be called then the
  ...
  // Method:   create
  /**
   ** Initialize the train flow to create a new <code>Vehicule</code>.
   ** <br>
   ** Leverage the operational binding <code>vehicleCreate</code> to do so.
   */
  public void create() {
    ADF.executeOperation("vehicleCreate");
  }

  //////////////////////////////////////////////////////////////////////////////
  // Method:   fetch
  /**
   ** Initialize the train flow to manage an existing <code>Vehicule</code>.
   ** <br>
   ** Leverage the operational binding <code>vehicleFetch</code> to do so.
   */
  public void fetch() {
    ADF.executeOperation("vehicleFetch");
  }

  //////////////////////////////////////////////////////////////////////////////
  // Method:   submit
  /**
   ** Create or updates the <code>Vehicule</code>, open a newly
   ** created <code>Vehicule</code> in the edit mode and close
  ...
  public String submit() {
    final Map<String, Object> parameter = new HashMap<String, Object>() {{
      put(VehicleAdapter.MOD, scopeModify);
    }};
    // commit the changes to the persistence layer which will now do what's
    // needed to create or update the Vehicule
    final Object result = ADF.executeAction(SUBMIT, parameter);
    if (ADF.executeActionSucceeded(result)) {
      final RowSetIterator rowSet = rowSetIterator(DETAIL_ITERATOR);
      final Row    row   = rowSet.getCurrentRow();
      final String name  = (String)row.getAttribute(VehicleAdapter.CARPLATENUMBER);
      ...
      if (MODE_CREATE.equals(scope.get(PARAMETER_MODE))) {
        raiseTaskFlowFeedbackEvent(String.format(ADF.resourceBundleValue(BUNDLE, "VHU_TRAIN_CREATE"), name));
        raiseTaskFlowRemoveEvent(flow);
      }
      else {
        markClean();
        raiseTaskFlowFeedbackEvent(String.format(ADF.resourceBundleValue(BUNDLE, "VHU_TRAIN_MODIFY"), name));
  ...
  public void revert(final @SuppressWarnings("unused") ActionEvent event) {
    ...
    final String name  = ADF.iteratorBindingValue(AttributeState.DETAIL_ITERATOR, VehicleAdapter.CARPLATENUMBER, String.class);
    raiseTaskFlowFeedbackEvent(String.format(ADF.resourceBundleValue(BUNDLE, "VHU_TRAIN_REVERT"), name));
  }
  ...
  public void revertAttribute() {
    ADF.executeOperation("refreshVehiculeAttribute");
