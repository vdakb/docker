...
public class Search extends AbstractSearch {
  ...
  // compatible with
  @SuppressWarnings("compatibility:8177817052539352880")
  private static final long   serialVersionUID = -5714848018900807920L;
  ...
  public String getLocalizedMessage() {
    ...
    switch (localized) {
      case GENERIC_DELETE : return (selection.size() == 1)
                            ? String.format(ADF.resourceBundleValue(BUNDLE, "VMB_DELETE_CONFIRM_SINGLE"), selection.get(0).getAttribute("NAME"))
                            : String.format(ADF.resourceBundleValue(BUNDLE, "VMB_DELETE_CONFIRM_MULTIPLE"), selection.size());
  ...
  public void delete(final List<Row> selection) {
    if (selection != null && !selection.isEmpty()) {
      ...
      if (selection.size() == 1)
        raiseTaskFlowFeedbackEvent(String.format(ADF.resourceBundleValue(BUNDLE, "VMB_DELETE_FEEDBACK_SINGLE"), name));
      else
        raiseTaskFlowFeedbackEvent(String.format(ADF.resourceBundleValue(BUNDLE, "VMB_DELETE_FEEDBACK_MULTIPLE"), selection.size()));
  ...
  private void launch(final String mode) {
    ...
    final String              name  = row.getAttribute("name").toString();
    final String              taskFlowId  = "vmb" + id;
    final Map<String, Object> parameter   = new HashMap<String, Object>();
    parameter.put("brandId",          id);
    parameter.put("brandName",        name);
    parameter.put(PARAMETER_MODE,     mode);
    parameter.put(PARAMETER_TASKFLOW, taskFlowId);
    raiseTaskFlowLaunchEvent(taskFlowId, "/WEB-INF/bka/employee/portal/vehicle/vmb/flow/train-tf.xml#train-tf", name, null, name, null, false, parameter);
  }
}