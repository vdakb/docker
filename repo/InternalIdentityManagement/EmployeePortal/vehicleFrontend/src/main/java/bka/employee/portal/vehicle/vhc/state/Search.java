...
public class Search extends AbstractSearch {
  ...
  // compatible with
  @SuppressWarnings("compatibility:6610969451067914337")
  private static final long   serialVersionUID = -4880392948906319513L;
  ...
  public String getLocalizedMessage() {
    ...
    switch (localized) {
      case GENERIC_DELETE : return (selection.size() == 1)
                            ? String.format(ADF.resourceBundleValue(BUNDLE, "VHC_DELETE_CONFIRM_SINGLE"),   selection.get(0).getAttribute("meaning"))
                            : String.format(ADF.resourceBundleValue(BUNDLE, "VHC_DELETE_CONFIRM_MULTIPLE"), selection.size());
  ...
  public void delete(final List<Row> selection) {
    if (selection != null && !selection.isEmpty()) {
      ...
      if (selection.size() == 1)
        raiseTaskFlowFeedbackEvent(String.format(ADF.resourceBundleValue(BUNDLE, "VHC_DELETE_FEEDBACK_SINGLE"), name));
      else
        raiseTaskFlowFeedbackEvent(String.format(ADF.resourceBundleValue(BUNDLE, "VHC_DELETE_FEEDBACK_MULTIPLE"), selection.size()));
  ...
  private void launch(final String mode) {
    ...
    final String              taskFlowId  = "vhc" + id;
    final Map<String, Object> parameter   = new HashMap<String, Object>();
    parameter.put("colorId",          id);
    parameter.put("colorName",        name);
    parameter.put(PARAMETER_MODE,     mode);
    parameter.put(PARAMETER_TASKFLOW, taskFlowId);
    raiseTaskFlowLaunchEvent(taskFlowId, "/WEB-INF/bka/employee/portal/vehicle/vhc/flow/train-tf.xml#train-tf", name, null, name, null, false, parameter);
  }
}