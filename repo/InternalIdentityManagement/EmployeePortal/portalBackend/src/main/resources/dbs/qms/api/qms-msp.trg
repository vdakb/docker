-- D:\Project\Bundeskriminalamt12c\InternalIdentityManagement\EmployeePortal\eppAplication\src\main\dbs\qms\api\qms-msp.trg
--
-- Generated for Oracle Database 12c on Fri Apr 24 10:07:06 2020 by Server Generator 10.1.2.93.10

PROMPT Creating Trigger Logic for Table 'qms_message_properties'
...
BEGIN
  ...
  THEN
    qms$message_properties.validate_arc(cg$rec);
    qms$message_properties.validate_domain(cg$rec);
...
BEGIN
  ...
  THEN
    ...
    LOOP
      ...
      qms$message_properties.upd_oper_denorm2(cg$rec, cg$old_rec, qms$message_properties.cg$tableind(idx), 'INS');
...
PROMPT Creating After Update Row Trigger on 'qms_message_properties'
CREATE OR REPLACE TRIGGER cg$aur_qms_message_properties
AFTER UPDATE ON qms_message_properties FOR EACH ROW
BEGIN
...
BEGIN
  ...
  LOOP
    ...
    THEN
      ...
      qms$message_properties.validate_foreign_keys_upd(cg$rec, cg$old_rec, cg$ind);
      qms$message_properties.upd_denorm2(cg$rec, qms$message_properties.cg$tableind(idx));
      qms$message_properties.upd_oper_denorm2(cg$rec, cg$old_rec, qms$message_properties.cg$tableind(idx));
...
PROMPT Creating After Delete Row Trigger on 'qms_message_properties'
...
BEGIN
  ...
  THEN
    ...
    LOOP
      ...
      qms$message_properties.upd_oper_denorm2(cg$rec, cg$old_rec, qms$message_properties.cg$tableind(idx), 'DEL');
