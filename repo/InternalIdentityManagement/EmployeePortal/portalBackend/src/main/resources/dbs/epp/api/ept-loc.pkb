-- D:\Project\Bundeskriminalamt12c\InternalIdentityManagement\EmployeePortal\portalBackend\src\main\resources\dbs\epp\api\ept-loc.pkb
--
-- Generated for Oracle Database 12c on Sun May 24 17:02:48 2020 by Server Generator 10.1.2.93.10

PROMPT Creating API Package Implementation for Table 'ept_locales'
--------------------------------------------------------------------------------
-- Name:        cg$ept_locales
-- Description: cg$ept_locales table API package definitions
--------------------------------------------------------------------------------
CREATE OR REPLACE PACKAGE BODY cg$ept_locales
IS
...
BEGIN
  cg$errors.push( cg$errors.get_text(cg$errors.API_UNIQUE_KEY_UPDATE, cg$errors.ERR_UK_UPDATE, uk)
                ...
                , 'cg$ept_locales.raise_uk_not_updateable'
                );
...
BEGIN
  cg$errors.push( cg$errors.get_text(cg$errors.API_FOREIGN_KEY_TRANS, cg$errors.ERR_FK_TRANS, fk)
                ...
                , 'cg$ept_locales.raise_fk_not_transferable'
                );
  cg$errors.raise_failure;
...
EXCEPTION
  ...
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$ept_locales.up_autogen_columns.denorm');
    cg$errors.raise_failure;
...
EXCEPTION
  ...
  THEN
    validate_mandatory(cg$rec, 'cg$ept_locales.ins.mandatory_missing');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.check_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_CHECK_CON, 'cg$ept_locales.ins.check_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.fk_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_FOREIGN_KEY, 'cg$ept_locales.ins.fk_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.uk_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_UNIQUE_KEY, 'cg$ept_locales.ins.uk_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$$ept_locales.ins.others');
    called_from_package := FALSE;
    cg$errors.raise_failure;
END ins;
...
BEGIN
  ...
  THEN
    sel(cg$upd_rec);
    -- Report error if attempt to update non updateable Primary Key EPT_LOC_PK
    IF (cg$ind.id AND cg$rec.id != cg$upd_rec.id)
    THEN
      raise_uk_not_updateable('EPP_LOC_PK');
    END IF;
    IF NOT (cg$ind.id)
    THEN
...
EXCEPTION
  ...
  THEN
    validate_mandatory(cg$rec, 'cg$ept_locales.upd.upd_mandatory_null');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.check_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_CHECK_CON, 'cg$ept_locales.upd.check_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.fk_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_FOREIGN_KEY, 'cg$ept_locales.upd.fk_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN cg$errors.uk_violation
  THEN
    err_msg(SQLERRM, cg$errors.ERR_UNIQUE_KEY, 'cg$ept_locales.upd.uk_violation');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$ept_locales.upd.others');
    called_from_package := FALSE;
    cg$errors.raise_failure;
END upd;
...
EXCEPTION
  ...
  THEN
    err_msg(SQLERRM, cg$errors.ERR_DELETE_RESTRICT, 'cg$ept_locales.upd.delete_restrict');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN no_data_found
  THEN
    cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_DEL, cg$errors.ROW_DEL), 'E', 'ORA', SQLCODE, 'cg$ept_locales.del.no_data_found');
    called_from_package := FALSE;
    cg$errors.raise_failure;
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$ept_locales.del.others');
    called_from_package := FALSE;
    cg$errors.raise_failure;
END del;
...
EXCEPTION
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$ept_locales.sel.others');
    cg$errors.raise_failure;
END sel;
...
BEGIN
  ...
  EXCEPTION
    ...
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_LCK, cg$errors.ROW_LCK), 'E', 'ORA', SQLCODE, 'cg$ept_locales.lck.resource_busy');
      cg$errors.raise_failure;
    WHEN no_data_found
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_DEL, cg$errors.ROW_DEL), 'E', 'ORA', SQLCODE, 'cg$$ept_locales.lck.no_data_found');
      cg$errors.raise_failure;
    WHEN OTHERS
    THEN
      cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$ept_locales.lck.others');
      cg$errors.raise_failure;
  END;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P10ID), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$ept_locales.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P15ROWVERSION), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$ept_locales.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P20CREATED_BY), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$ept_locales.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P25CREATED_ON), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$ept_locales.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P30UPDATED_BY), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$ept_locales.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P35UPDATED_ON), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$ept_locales.lck');
      any_modified := TRUE;
    END IF;
  END IF;
  ...
  THEN
    ...
    THEN
      cg$errors.push(cg$errors.get_text(cg$errors.API_ROW_MOD, cg$errors.ROW_MOD, P40MEANING), 'E', 'API', CG$ERRORS.API_MODIFIED, 'cg$ept_locales.lck');
      any_modified := TRUE;
    END IF;
  END IF;
...
-- Name:        cascade_update
--
-- Description: Updates all child tables affected by a change to ept_locales
--
-- Parameters:  cg$rec     Record of ept_locales current values
--              cg$old_rec Record of ept_locales previous values
--------------------------------------------------------------------------------
...
--              of each child table that references this table ept_locales
--
-- Parameters:  cg$old_rec     Record of ept_locales current values
--------------------------------------------------------------------------------
...
--              child table that references this table ept_locales
--
-- Parameters:  cg$new_rec  New values for ept_locales's domain key constraint columns
--              cg$new_ind  Indicates changed ept_locales's domain key constraint columns
--              cg$old_rec  Current values for ept_locales's domain key constraint columns
--------------------------------------------------------------------------------
...
-- Name:        cascade_delete
--
-- Description: Delete all child tables affected by a delete to ept_locales
--
-- Parameters:  cg$rec     Record of ept_locales current values
--------------------------------------------------------------------------------
...
EXCEPTION
  ...
  WHEN no_tabview
  THEN
    cg$errors.push( cg$errors.get_text(cg$errors.API_RV_TAB_NOT_FOUND, cg$errors.APIMSG_RV_TAB_NOT_FOUND, '"epp REF_CODES"','ept_locales')
                  , 'E'
                  , 'API'
                  , 'cg$errors.API_RV_TAB_NOT_FOUND'
                  , 'cg$ept_locales.v_domain.no_reftable_found'
                  );
    cg$errors.raise_failure;
  WHEN OTHERS
  THEN
    cg$errors.push(SQLERRM, 'E', 'ORA', SQLCODE, 'cg$ept_locales.v_domain.no_reftable_found');
    cg$errors.raise_failure;
END validate_domain;
...
--
-- Description: Update the Domain Constraint Key columns of ept_locales when the
--              Cascade Update rule is Cascades and the domain table has been
--              updated. Called from <Domain Table pkg>.domain_cascade_update().
--
-- Parameters:  cg$rec      New values for ept_locales's domain key constraint columns
--              cg$ind      Indicates changed ept_locales's domain key constraint columns
--              cg$old_rec  Current values for ept_locales's domain key constraint columns
--------------------------------------------------------------------------------
...
BEGIN
  ...
  IF (con_name = 'EPT_LOC_PK')
  THEN
    cg$errors.push( NVL(EPT_LOC_PK, cg$errors.get_text(cg$errors.API_PK_CON_VIOLATED, cg$errors.APIMSG_PK_VIOLAT, 'EPT_LOC_PK', 'ept_locales'))
                  , 'E'
                  , 'API'
                  , 'cg$errors.API_PK_CON_VIOLATED'
                  , loc
                  );
  ELSIF (con_name = 'EPT_LOC_CK' AND EPT_LOC_CK IS NOT NULL)
  THEN
    cg$errors.push('EPT-00301', 'E', 'API', cg$errors.API_CK_CON_VIOLATED, loc);
  ELSE
