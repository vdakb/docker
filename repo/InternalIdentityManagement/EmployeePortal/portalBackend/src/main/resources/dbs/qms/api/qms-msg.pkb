-- D:\Project\Bundeskriminalamt12c\InternalIdentityManagement\EmployeePortal\eppAplication\src\main\dbs\qms\api\qms-msg.pkb
--
-- Generated for Oracle Database 12c on Fri Apr 24 10:07:06 2020 by Server Generator 10.1.2.93.10

PROMPT Creating Package Implementation 'qms$message'
CREATE OR REPLACE PACKAGE BODY qms$message
AS
  -- ---------------------------------------------------------------------------
  -- private constants
  -- ---------------------------------------------------------------------------
  ...
  -- list of internal headstart messages
  -- message are hardcoded here to avoid recursive calling of error procedures if
  -- QMS-00101 or QMS-00102 are missing from the error tables.
  ...
  -- ---------------------------------------------------------------------------
  g_errorrec                   hil$message.row_type;
  ...
  BEGIN
    ...
    get_message(l_error, qms$severity_error_msg, p_language => l_language);
    g_error_string := l_error.msg_text;

    get_message(l_error, qms$severity_warning_msg, p_language => l_language);
    g_warning_string := l_error.msg_text;

    get_message(l_error, qms$severity_information_msg, p_language => l_language);
    g_information_string := l_error.msg_text;

    get_message(l_error, qms$severity_question_msg, p_language => l_language);
    g_question_string := l_error.msg_text;

    get_message(l_error, qms$severity_message_msg, p_language => l_language);
    g_message_string := l_error.msg_text;
  ...
  --
  -- Purpose  Initialise table with error codes that may not be displayed by retrieving
  --          the records from the database with suppress_always set to 'Y'.
  --          These will be passed to the client and cached in a global record_group on
  --          the client.
  --
  ...
  BEGIN
    ...
    LOOP
      ...
      p_message(l_rowindex) := r_suppresserror.code;
    END LOOP;
  ...
  IS
    ...
    IS
      ...
      AND    msp.code = mst.msp_code
      ;

    l_index NUMBER (5, 0) := 0;
  BEGIN
    p_error.delete;
    FOR r_native_error IN c_native_error
    LOOP
      l_index := l_index + 1;
      p_error(l_index).code             := r_native_error.code;
      p_error(l_index).severity         := r_native_error.severity;
      p_error(l_index).severity_desc    := get_severity_string(r_native_error.severity);
      p_error(l_index).suppress_warning := r_native_error.suppress_warning;
      p_error(l_index).suppress_always  := r_native_error.suppress_always;
      p_error(l_index).logging          := r_native_error.logging;
      p_error(l_index).msg_text         := r_native_error.text;
      p_error(l_index).help_text        := r_native_error.help_text;
      p_error(l_index).lang_code        := r_native_error.language;
    END LOOP;
  ...
  IS
  BEGIN
    cg$errors.pop_error(p_error);
  END;

  --
  -- Purpose  return true (and p_existing_msg_code then contains the message code)
  --          if specified message code or constraint exists in qms$message_properties.
  --
  ...
  BEGIN
    ...
    -- internal error message for problem/tracking
    IF p_language IS NULL
    THEN
      p_error.code             := qms$no_language_code;
      p_error.msg_text         := qms$no_language;
      p_error.severity         := 'E';
      p_error.severity_desc    := get_severity_string('E');
      p_error.suppress_warning := 'N';
      p_error.help_text        := null;
      p_error.lang_code        := qms$default_language;
      p_error.raise_error      := true;
    ELSE
      OPEN  c_error(p_code, p_language, p_constraint);
      FETCH c_error
      INTO  p_error.code
      ,     p_error.severity
      ...
      THEN
        ...
        p_error.msg_text         := qms$no_message;
        p_error.severity         := 'E';
        p_error.severity_desc    := get_severity_string('E');
        p_error.suppress_warning := 'N';
        p_error.help_text        := null;
        p_error.lang_code        := p_language;
        p_error.raise_error      := true;
        IF p_constraint IS NULL
        THEN
          p_error.msg_text := replace(p_error.msg_text, '<p1>', p_code);
        ELSE
          p_error.msg_text := replace(p_error.msg_text, '<p1>', p_constraint);
        END IF;
        p_error.msg_text := replace(p_error.msg_text, '<p2>', p_language);
      ELSE
        p_error.severity_desc := get_severity_string(p_error.severity);
  ...
  FUNCTION get_next_msg_code(p_prefix IN VARCHAR2)
    RETURN VARCHAR2
  IS
    CURSOR c_msp(b_msg_prefix IN VARCHAR2)
    IS
      SELECT NVL(MAX(TO_NUMBER(SUBSTR(msp.code, LENGTH(b_msg_prefix) + 2))), 0) max_number
      FROM   qms_message_properties msp
      WHERE  SUBSTR(msp.code, 1, LENGTH(b_msg_prefix) + 1) = b_msg_prefix||'-'
      -- select only codes where rest is numbers only
      AND    translate(SUBSTR(msp.code, LENGTH(b_msg_prefix) + 2), '0123456789', '9999999999')
           = SUBSTR('999999999999999', 1, LENGTH(SUBSTR(msp.code, LENGTH(b_msg_prefix) + 2)))
      ;
  ...
  BEGIN
     ...
     l_number_length := qms$errors.c_errorlength - LENGTH(p_prefix) - 1;
  ...
  IS
    ...
    IS
      ...
      WHERE  mst.msp_code = b_code
      AND    mst.language = b_language
      ;

    r_mst      c_mst%rowtype;
    l_existing VARCHAR2(200);
  BEGIN
    --
    -- check if message property exists
    --
    IF msg_code_exists( p_code       => p_code
                      , p_constraint => p_constraint
                      , p_existing   => l_existing  -- out parameter
                      )
    THEN
      -- message property exist
      OPEN  c_mst(l_existing, p_language);
  ...
  BEGIN
    ...
    add_language(p_code, p_language, p_text, p_help_text);
  END create_message;
  ...
  BEGIN
    ...
    -- this has to be done because the code is the primary key which is not
    -- updateable
    --
    OPEN   c_msp(b_code => p_code_old);
    FETCH  c_msp INTO r_msp;
    INSERT INTO qms_message_properties (
      code
    , description
    ...
    FOR r_mst IN c_mst(b_code => p_code_old)
    LOOP
      INSERT INTO qms_message_text (
        language
      , text
